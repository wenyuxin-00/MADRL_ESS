...
def plot_voltage_and_net_load_dashboard(rollout: Any, *, title: str | None=None):
    ...
    if not agent_df.empty:
        ...
        if not agent_profiles:
            agent_profiles = [str(profile) for profile in agent_df.get('agent_profile', pd.Series(dtype=object)).dropna().drop_duplicates().tolist()]
    axis_count = 3 + len(agent_profiles)
    figure, axes = plt.subplots(axis_count, 1, figsize=(14.0, max(8.5, 2.8 * axis_count)), sharex=True)
    axes = np.atleast_1d(axes)
    price_ax, voltage_ax, net_load_ax = axes[:3]
    figure.suptitle(title or f"{meta.get('controller', 'Rollout')} Import Price, Voltage, Net Load, and Storage State", fontsize=14)
    timestamps = pd.to_datetime(step_df['timestamp'])
    price_ax.plot(timestamps, np.asarray(step_df.get(IMPORT_PRICE_COLUMN, np.zeros(len(step_df))), dtype=np.float32), color='#111827', linewidth=1.6, label='Actual')
    if IMPORT_PRICE_PRED_COLUMN in step_df.columns:
        price_ax.plot(timestamps, np.asarray(step_df[IMPORT_PRICE_PRED_COLUMN], dtype=np.float32), color='#dc2626', linewidth=1.4, linestyle='--', label='Forecast')
    ...
    if not grid_df.empty:
        ...
        for bus_id, bus_df in bus_groups:
            ...
            if bus_id in agent_bus_ids:
                ...
                highlight_index += 1
                voltage_ax.plot(timestamps, values, color=color, linewidth=1.7, alpha=0.95, label=f'Agent bus {bus_id}')
            else:
                voltage_ax.plot(timestamps, values, color=background_color, linewidth=1.0, alpha=0.45)
    ...
    net_load_ax.plot(timestamps, np.asarray(step_df.get('base_net_load_total', np.zeros(len(step_df))), dtype=np.float32), color='#111827', linewidth=1.5, label='Raw net load')
    net_load_ax.plot(timestamps, np.asarray(step_df.get('base_net_load_effective_total', np.zeros(len(step_df))), dtype=np.float32), color='#0f766e', linewidth=1.5, label='Post-curtail net load')
    net_load_ax.plot(timestamps, np.asarray(step_df.get('net_load_total', np.zeros(len(step_df))), dtype=np.float32), color='#2563eb', linewidth=1.7, label='Post-action net load')
    ...
    for axis, profile in zip(axes[3:], agent_profiles, strict=False):
        ...
        soc_axis = axis.twinx()
        soc_axis.plot(agent_timestamps, agent_frame['soc'].to_numpy(dtype=np.float32), color='#111827', linewidth=1.2, label='SoC')
