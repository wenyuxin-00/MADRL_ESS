...
def build_compare_warning_banner(*rollouts: RolloutResult):
    ...
    for rollout in rollouts:
        ...
        if bool(rollout.meta.get('high_budget_refinement_warn', False)):
            items.append(f'<li><strong>{controller}</strong>: high-budget MISOCP refinement was used; compare economics are based on the returned/final dispatch, so the economic comparison should be interpreted with care.</li>')
        if bool(rollout.meta.get('formulation_tightening_required', False)):
            items.append(f'<li><strong>{controller}</strong>: formulation tightening is still required; safety and economic conclusions may be unstable.</li>')
        missing_optional_fields = _collect_compare_optional_missing_fields(rollout)
        if missing_optional_fields:
            joined = ', '.join((escape(field_name) for field_name in missing_optional_fields))
            items.append(f'<li><strong>{controller}</strong>: optional compare fields missing ({joined}); affected table cells will show N/A.</li>')
    if not items:
        html = "<div style='padding:10px 12px;border:1px solid #86efac;background:#f0fdf4;color:#166534;border-radius:8px;'>Compare warnings: none.</div>"
        return _HTML(html)
    html = "<div style='padding:10px 12px;border:1px solid #facc15;background:#fefce8;color:#854d0e;border-radius:8px;'><strong>Compare warnings</strong><ul style='margin:8px 0 0 18px;'>" + ''.join(items) + '</ul></div>'
    return _HTML(html)
