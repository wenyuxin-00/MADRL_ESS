...
from predictors.shared_data import load_madrl_shared_data_manifest,select_shared_data_episode_indices
...
def _cfg_runtime_split_payload(*,source:str,split:str,year:int,start_date:str|None,end_date:str|None,exclude_start_date:str|None,exclude_end_date:str|None,window_strategy:str='cfg_window')->dict[str,Any]:return{'source':str(source),'split':str(split),'year':int(year),'start_date':start_date,'end_date':end_date,'exclude_start_date':exclude_start_date,'exclude_end_date':exclude_end_date,'window_strategy':str(window_strategy)}
...
def build_env(cfg:Any,mode:str,dataset:Any|None=None,reward_fn:Any|None=None,forecaster:Any|None=None,obs_builder:Any|None=None)->Any:
	...
	if obs_builder is None:normalizer=build_observation_normalizer(cfg,dataset=dataset if split_name=='train'else None);obs_builder=DefaultObservationBuilder(local_features=cfg.obs.local_features,sequence_features=cfg.obs.sequence_features,future_horizon=cfg.env.future_horizon,adjacency_type=cfg.obs.adjacency_type,normalizer=normalizer,precomputed=split_precomputed_dir is not None)
