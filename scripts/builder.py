...
from predictors.shared_data import (
    load_madrl_shared_data_manifest,
...
def _shared_split_dir(cfg: Any, split: str) -> Path | None:
    ...
    if not manifest_path.exists():
        raise FileNotFoundError(f"shared_data_dir is set to '{shared_data_dir}', but split='{split}' is missing '{manifest_path.name}'.")
    return split_dir
...
def _cfg_runtime_split_payload(*, source: str, split: str, year: int, start_date: str | None, end_date: str | None, exclude_start_date: str | None, exclude_end_date: str | None, window_strategy: str='cfg_window') -> dict[str, Any]:
    return {'source': str(source), 'split': str(split), 'year': int(year), 'start_date': start_date, 'end_date': end_date, 'exclude_start_date': exclude_start_date, 'exclude_end_date': exclude_end_date, 'window_strategy': str(window_strategy)}
...
def _shared_data_metadata(cfg: Any) -> dict[str, Any] | None:
    ...
    return {'shared_data_dir': str(shared_data_dir), 'shared_data_signature': str(getattr(getattr(cfg, 'runtime', None), 'shared_data_signature', manifest.get('signature_hash', ''))), 'manifest': manifest, 'split_dirs': {split: str((shared_data_dir / split).resolve()) for split in ('train', 'test') if (shared_data_dir / split / 'manifest.json').exists()}}

def _subproc_vec_env_is_supported_in_current_process() -> tuple[bool, str | None]:
    main_module = sys.modules.get('__main__')
    main_file = getattr(main_module, '__file__', None)
    if 'ipykernel' in sys.modules:
        return (False, 'Jupyter/IPython kernels do not reliably support spawn-based vector environments.')
...
def build_env(cfg: Any, mode: str, dataset: Any | None=None, reward_fn: Any | None=None, forecaster: Any | None=None, obs_builder: Any | None=None) -> Any:
    ...
    if split_precomputed_dir is not None:
        ...
        dataset = build_dataset(cfg, mode=mode, override_start_date=split_controls.get('start_date'), override_end_date=split_controls.get('end_date'), override_exclude_start_date=split_controls.get('exclude_start_date'), override_exclude_end_date=split_controls.get('exclude_end_date'))
        ...
        selected_episode_indices = select_shared_data_episode_indices(shared_split_manifest, start_date=requested_start, end_date=requested_end) if split_name == 'test' else _full_manifest_episode_indices(shared_split_manifest)
        cfg.runtime.effective_split_controls = _cfg_runtime_split_payload(source='shared_data_manifest', split=split_name, year=int(split_controls.get('year', cfg.data.train_year if split_name == 'train' else cfg.data.test_year)), start_date=split_controls.get('start_date'), end_date=split_controls.get('end_date'), exclude_start_date=split_controls.get('exclude_start_date'), exclude_end_date=split_controls.get('exclude_end_date'), window_strategy=str(split_controls.get('window_strategy', 'cfg_window')))
    ...
    if split_precomputed_dir is None:
        ...
        cfg.runtime.effective_split_controls = _cfg_runtime_split_payload(source='cfg', split=split_name, year=int(selected_year), start_date=start_date, end_date=end_date, exclude_start_date=exclude_start_date, exclude_end_date=exclude_end_date)
...
def build_train_runner(cfg: Any, seed: int=0, env_name: str='GridEnv', number: int=1) -> TrainRunner:
    ...
    validate_and_finalize_model_config(cfg)
    runner = TrainRunner(cfg, train_env=train_env, eval_env=eval_env, env_name=env_name, number=number, seed=seed)
