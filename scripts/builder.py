...
def _cfg_runtime_split_payload(*,source:str,split:str,year:int,start_date:str|None,end_date:str|None,window_strategy:str='cfg_window',window_days:int=1,window_stride_days:int=1,episode_length:int|None=None,base_episode_length:int|None=None)->dict[str,Any]:return{'source':str(source),'split':str(split),'year':int(year),'start_date':start_date,'end_date':end_date,'window_strategy':str(window_strategy),'window_days':int(window_days),'window_stride_days':int(window_stride_days),'episode_length':None if episode_length is None else int(episode_length),'base_episode_length':None if base_episode_length is None else int(base_episode_length)}
...
def build_env(cfg:Any,mode:str,dataset:Any|None=None,reward_fn:Any|None=None,forecaster:Any|None=None,obs_builder:Any|None=None)->Any:
	...
	if obs_builder is None:normalizer=build_observation_normalizer(cfg,dataset=dataset if split_name=='train'else None);obs_builder=DefaultObservationBuilder(local_features=cfg.obs.local_features,sequence_features=cfg.obs.sequence_features,future_horizon=cfg.env.future_horizon,normalizer=normalizer,precomputed=split_precomputed_dir is not None,price_spread_scale_eur_per_kwh=float(getattr(cfg.obs,'wholesale_price_spread_scale_eur_per_kwh',.20)))
