...
from scripts.checkpoints import build_training_run_paths, resolve_checkpoint_to_load, slugify_checkpoint_token
...
def _apply_reward_controls(cfg, reward_controls: dict[str, Any] | None) -> None:
    ...
    _KNOWN_REWARD_KEYS = {'export_subsidy_eur_per_kwh', 'import_price_markup_eur_per_kwh', 'w_soc_pen', 'w_voltage_pen', 'w_line_pen', 'w_trafo_pen'}
    unknown_keys = set(controls.keys()) - _KNOWN_REWARD_KEYS
    if unknown_keys:
        raise ValueError(f'Unknown reward_controls key(s): {sorted(unknown_keys)}. Supported keys: {sorted(_KNOWN_REWARD_KEYS)}.')
    if 'w_soc_pen' in controls:
        cfg.reward.w_soc_pen = float(controls['w_soc_pen'])
    numeric_fields = ('export_subsidy_eur_per_kwh', 'import_price_markup_eur_per_kwh', 'w_voltage_pen', 'w_line_pen', 'w_trafo_pen')
...
def _resolve_save_dir(*, args, experiment_controls: dict[str, Any], data_controls: dict[str, Any], train_controls: dict[str, Any], checkpoint_controls: dict[str, Any]) -> Path:
    if args.save_dir is not None:
        return Path(args.save_dir).resolve()
    checkpoint_root = Path(checkpoint_controls.get('checkpoint_root') or get_checkpoint_root(project_root())).resolve()
    algorithm = str(experiment_controls.get('algorithm') or ('MATD3' if str(train_controls.get('profile', 'base')) == 'gpu_fast' else 'MADDPG'))
...
def main(argv: list[str] | None=None) -> int:
    ...
    save_dir = _resolve_save_dir(args=args, experiment_controls=experiment_controls, data_controls=data_controls, train_controls=train_controls, checkpoint_controls=checkpoint_controls)
