...
def _build_progress_payload(*, interaction_step: int, target_interactions: int, episodes_completed: int, total_steps: int, avg_reward: float, action_time_total: float, env_step_time_total: float, update_time_total: float, update_calls: int, run_start: float, started_at: datetime, status: str, error_message: str='') -> dict[str, object]:
    now = datetime.now().astimezone()
    elapsed = max(time.perf_counter() - run_start, 0.0)
    safe_elapsed = max(elapsed, 1e-06)
    remaining_seconds = _estimate_remaining_seconds(interaction_step=interaction_step, target_interactions=target_interactions, elapsed_seconds=elapsed)
...
class TrainRunner:
    ...
    def __init__(self, cfg: Any, train_env: Any, eval_env: Any, env_name: str='GridEnv', number: int=1, seed: int=0) -> None:
        ...
        self.progress_write_interval_seconds = max(0.0, float(getattr(self.cfg.train, 'progress_write_interval_seconds', 5.0)))
        ...
        self._safety_projection_calls_by_stage = {'rollout': 0, 'target': 0, 'actor': 0}
        self._safety_projection_samples_by_stage = {'rollout': 0, 'target': 0, 'actor': 0}
        self._safety_projection_time_s_by_stage = {'rollout': 0.0, 'target': 0.0, 'actor': 0.0}
    ...
    def _record_projection_diagnostics(self, diagnostics: dict[str, Any]) -> None:
        ...
        self._safety_pre_trafo_import_violation_kw_weighted += float(diagnostics.get('pre_trafo_import_violation_kw', 0.0)) * batch_size
        self._safety_pre_trafo_export_violation_kw_weighted += float(diagnostics.get('pre_trafo_export_violation_kw', 0.0)) * batch_size
        self._safety_post_trafo_import_violation_kw_weighted += float(diagnostics.get('post_trafo_import_violation_kw', 0.0)) * batch_size
        self._safety_post_trafo_export_violation_kw_weighted += float(diagnostics.get('post_trafo_export_violation_kw', 0.0)) * batch_size
        self._safety_max_abs_delta = max(self._safety_max_abs_delta, float(diagnostics.get('max_abs_delta', 0.0)))

    def _record_projection_event(self, *, stage: str, batch_size: int, elapsed_s: float, diagnostics: dict[str, Any] | None=None) -> None:
    ...
    def _record_projector_local_infeasible(self, residual_action_info: dict[str, torch.Tensor] | None) -> None:
    ...
    def build_safety_summary(self) -> dict[str, Any]:
        if not self._safe_projection_enabled():
            return {'enabled': False, 'algorithm': str(self.cfg.algo.name)}
        ...
        return {'enabled': True, 'algorithm': str(self.cfg.algo.name), 'projector_mode': str(getattr(self.cfg.safety, 'projector_mode', 'joint_linearized')), 'projection_batches': int(self._safety_projection_batches), 'projection_samples': int(self._safety_projection_samples), 'projection_diagnostic_samples': int(self._safety_diagnostic_samples), 'projected_fraction': float(self._safety_projected_fraction_weighted / diagnostic_denominator), 'mean_abs_action_delta': float(self._safety_mean_abs_delta_weighted / diagnostic_denominator), 'max_abs_action_delta': float(self._safety_max_abs_delta), 'mean_abs_action_delta_kw': float(self._safety_mean_abs_delta_kw_weighted / diagnostic_denominator), 'mean_pre_projection_violation': float(self._safety_pre_violation_weighted / diagnostic_denominator), 'mean_post_projection_violation': float(self._safety_post_violation_weighted / diagnostic_denominator), 'mean_pre_trafo_import_violation_kw': float(self._safety_pre_trafo_import_violation_kw_weighted / diagnostic_denominator), 'mean_pre_trafo_export_violation_kw': float(self._safety_pre_trafo_export_violation_kw_weighted / diagnostic_denominator), 'mean_post_trafo_import_violation_kw': float(self._safety_post_trafo_import_violation_kw_weighted / diagnostic_denominator), 'mean_post_trafo_export_violation_kw': float(self._safety_post_trafo_export_violation_kw_weighted / diagnostic_denominator), 'projection_time_s': projection_time_total, 'rollout_projection_calls': int(self._safety_projection_calls_by_stage['rollout']), 'target_projection_calls': int(self._safety_projection_calls_by_stage['target']), 'actor_projection_calls': int(self._safety_projection_calls_by_stage['actor']), 'rollout_projection_samples': int(self._safety_projection_samples_by_stage['rollout']), 'target_projection_samples': int(self._safety_projection_samples_by_stage['target']), 'actor_projection_samples': int(self._safety_projection_samples_by_stage['actor']), 'rollout_projection_time_s': float(self._safety_projection_time_s_by_stage['rollout']), 'target_projection_time_s': float(self._safety_projection_time_s_by_stage['target']), 'actor_projection_time_s': float(self._safety_projection_time_s_by_stage['actor']), 'env_fallback_action_pen_steps': int(self._safety_env_fallback_steps), 'env_fallback_action_pen_rate': float(self._safety_env_fallback_steps / env_denominator), 'env_fallback_action_pen_mean': float(self._safety_env_action_pen_total / env_denominator), 'projector_local_infeasible_count': int(self._projector_local_infeasible_count)}
    ...
    def _select_action_batch_with_info(self, obs_np: dict) -> tuple[np.ndarray, dict[str, np.ndarray] | None]:
        obs_t = to_torch_nested(obs_np, self.cfg.runtime.device)
        with torch.inference_mode():
            raw_action_t = torch.stack([agent.act_from_torch_obs(obs_t, noise_std=self.noise_std) for agent in self.agent_n], dim=1)
            action_info = None
            if self._safe_projection_enabled():
                projection_started = time.perf_counter()
                projected_action_t, diagnostics = self.safety_projector.project_actions_from_safety_local(obs_t['safety_local'], raw_action_t, return_diagnostics=True)
                self._record_projection_event(stage='rollout', batch_size=int(raw_action_t.shape[0]), elapsed_s=time.perf_counter() - projection_started, diagnostics=diagnostics)
                action_t, projector_residual_info = enforce_local_action_feasibility_torch(obs_t['safety_local'], projected_action_t, efficiency=float(self.cfg.env.efficiency), dt_hours=float(self.cfg.env.dt), soc_min=float(self.cfg.env.soc_min), soc_max=float(self.cfg.env.soc_max))
                ...
                action_t, action_info = enforce_local_action_feasibility_torch(obs_t['safety_local'], raw_action_t, efficiency=float(self.cfg.env.efficiency), dt_hours=float(self.cfg.env.dt), soc_min=float(self.cfg.env.soc_min), soc_max=float(self.cfg.env.soc_max))
            else:
    ...
    def _apply_controller_action_postprocessing(self, reward: np.ndarray, info_list: list[dict[str, Any]], action_info: dict[str, np.ndarray] | None) -> tuple[np.ndarray, list[dict[str, Any]]]:
        ...
        for env_idx, info in enumerate(info_list):
            ...
            if action_info is not None:
                env_action_info = {key: np.asarray(value[env_idx], dtype=np.float32) for key, value in action_info.items()}
            merged_info, action_penalty = merge_action_info_into_step_info(info, env_action_info, soc_pen_weight=float(self.cfg.reward.w_soc_pen), apply_action_penalty=self.apply_action_penalty)
    ...
    def rollout_once(self, obs_np: dict | None=None) -> dict:
        ...
        next_obs, reward, terminated, truncated, info_list = self.env.step(self.format_env_actions(action_batch))
        reward, info_list = self._apply_controller_action_postprocessing(reward, info_list, action_info)
        done = np.logical_or(terminated, truncated).astype(np.float32)
        return {'obs': obs_np, 'reset_info': reset_info, 'action_batch': action_batch, 'next_obs': next_obs, 'reward': reward, 'done': done, 'terminated': terminated, 'truncated': truncated, 'info_list': info_list}

    def _build_shared_update_ctx(self, batch: dict[str, Any]) -> dict[str, Any]:
        next_obs = batch['next_obs']
        with torch.no_grad():
            target_actor_actions_clean = torch.stack([agent._actor_target_call(next_obs) for agent in self.agent_n], dim=1)
        ...
        if self._safe_projection_enabled():
            ...
            with torch.no_grad():
                noise = (torch.randn_like(target_actor_actions_clean) * float(self.cfg.algo.policy_noise)).clamp(-float(self.cfg.algo.noise_clip), float(self.cfg.algo.noise_clip))
                target_actor_actions = (target_actor_actions_clean + noise).clamp(-float(self.cfg.model.max_action), float(self.cfg.model.max_action))
                projection_started = time.perf_counter()
                shared_ctx['projected_target_actions'] = self.safety_projector.project_actions_from_safety_local(next_obs['safety_local'], target_actor_actions)
                self._record_projection_event(stage='target', batch_size=int(target_actor_actions.shape[0]), elapsed_s=time.perf_counter() - projection_started)
        return shared_ctx
    ...
    def save_model(self, model_dir: str, episode: int) -> None:
        ...
        manifest = build_checkpoint_manifest(algorithm=self.cfg.algo.name, saved_episode_tag=episode, episodes_completed=self.episodes_completed, total_steps=self.total_steps, num_envs=self.cfg.train.num_envs, episode_limit=self.cfg.env.episode_limit, save_dir=algo_dir)
    ...
    def build_reward_summary(self) -> dict[str, Any]:
        ...
        grid_safety_keys = [str(meta.key) for meta in self.reward_component_meta if str(meta.key).startswith('r_safe_') and int(getattr(meta, 'sign', 0)) == -1]
        if grid_safety_keys and all((key in components for key in grid_safety_keys)):
            num_episodes = len(episode_indices)
            aggregates['grid_safety_penalty'] = [float(sum((self.episode_reward_components[key][episode_idx] for key in grid_safety_keys))) for episode_idx in range(num_episodes)]
        return {'episodes': episode_indices, 'episode_total_reward': [float(value) for value in self.episode_rewards], 'components': components, 'aggregates': aggregates}
    ...
    def run(self) -> int:
        ...
        def emit_progress(*, force: bool=False) -> None:
            ...
            should_write_progress = progress_state_path is not None and (force or self.progress_write_interval_seconds <= 0.0 or time.perf_counter() - last_progress_write_at >= self.progress_write_interval_seconds)
            ...
            if should_write_progress:
                write_progress_snapshot(_build_progress_payload(interaction_step=interaction_step, target_interactions=target_interactions, episodes_completed=episodes_completed, total_steps=self.total_steps, avg_reward=avg_reward, action_time_total=action_time_total, env_step_time_total=env_step_time_total, update_time_total=update_time_total, update_calls=update_calls, run_start=run_start, started_at=started_at, status='running'))
        if progress_state_path is not None:
            write_progress_snapshot(_build_progress_payload(interaction_step=0, target_interactions=target_interactions, episodes_completed=0, total_steps=0, avg_reward=0.0, action_time_total=0.0, env_step_time_total=0.0, update_time_total=0.0, update_calls=0, run_start=run_start, started_at=started_at, status='running'))
        try:
            ...
            if progress_state_path is not None:
                avg_reward = float(np.mean(self.episode_rewards[-50:])) if self.episode_rewards else 0.0
                write_progress_snapshot(_build_progress_payload(interaction_step=interaction_step, target_interactions=target_interactions, episodes_completed=episodes_completed, total_steps=self.total_steps, avg_reward=avg_reward, action_time_total=action_time_total, env_step_time_total=env_step_time_total, update_time_total=update_time_total, update_calls=update_calls, run_start=run_start, started_at=started_at, status=run_status, error_message=error_message))
            progress.close()
        ...
        self.perf_summary = {'seed': self.seed, 'runtime_mode': str(self.cfg.runtime.execution_mode), 'device': str(self.cfg.runtime.device), 'vec_env': self.vec_env_name, 'tensorboard_dir': self.tensorboard_dir, 'total_wall_time_s': total_elapsed, 'action_time_s': action_time_total, 'env_step_time_s': env_step_time_total, 'update_time_s': update_time_total, 'sample_time_s': sample_time_total, 'history_time_s': history_time_total, 'progress_io_time_s': progress_io_time_total, 'agent_update_time_s': agent_update_time_total, 'update_calls': update_calls, 'steps_per_sec': self.total_steps / total_elapsed, 'avg_action_ms_per_iter': 1000.0 * action_time_total / max(interaction_step, 1), 'avg_env_ms_per_iter': 1000.0 * env_step_time_total / max(interaction_step, 1), 'avg_update_ms_per_call': 1000.0 * update_time_total / max(update_calls, 1), 'projection_time_s': float(sum(self._safety_projection_time_s_by_stage.values())), 'rollout_projection_time_s': float(self._safety_projection_time_s_by_stage['rollout']), 'target_projection_time_s': float(self._safety_projection_time_s_by_stage['target']), 'actor_projection_time_s': float(self._safety_projection_time_s_by_stage['actor']), 'rollout_projection_calls': int(self._safety_projection_calls_by_stage['rollout']), 'target_projection_calls': int(self._safety_projection_calls_by_stage['target']), 'actor_projection_calls': int(self._safety_projection_calls_by_stage['actor'])}
