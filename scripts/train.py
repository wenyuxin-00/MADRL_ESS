...
def init_power_flow_tracking(runner:Any)->None:
	runner._power_flow_diag={'steps':0,'nonconverged_steps':0,'first_error':'','last_error':'','error_counts':{}}
def record_power_flow_diagnostics(runner:Any,info_list:list[dict[str,Any]])->None:
	if getattr(runner,'_power_flow_diag',None)is None:init_power_flow_tracking(runner)
	diag=runner._power_flow_diag
	for info in info_list:
		if'pf_converged'not in info:continue
		diag['steps']=int(diag.get('steps',0))+1
		if bool(info.get('pf_converged',False)):continue
		diag['nonconverged_steps']=int(diag.get('nonconverged_steps',0))+1
		error_text=str(info.get('pf_error')or'unknown')
		if not str(diag.get('first_error','')):diag['first_error']=error_text
		diag['last_error']=error_text
		error_counts=diag.setdefault('error_counts',{})
		error_counts[error_text]=int(error_counts.get(error_text,0))+1
def build_power_flow_summary(runner:Any)->dict[str,Any]:
	diag=dict(getattr(runner,'_power_flow_diag',{})or{})
	steps=int(diag.get('steps',0));nonconverged_steps=int(diag.get('nonconverged_steps',0));error_counts={str(key):int(value)for(key,value)in dict(diag.get('error_counts',{})or{}).items()}
	return{'tracked':bool(steps>0),'steps':steps,'nonconverged_steps':nonconverged_steps,'nonconverged_fraction':float(nonconverged_steps/max(steps,1)),'all_converged':bool(steps>0 and nonconverged_steps==0),'first_pf_error':str(diag.get('first_error','')or''),'last_pf_error':str(diag.get('last_error','')or''),'pf_error_counts':error_counts}
	...
	def run(self)->int:
		...
		try:
			...
			while interaction_step<target_interactions:
				...
				self._record_power_flow_diagnostics(info_list)
				for(env_idx,info)in enumerate(info_list):
