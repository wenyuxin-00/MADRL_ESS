...
def build_shared_update_ctx(runner:Any,batch:dict[str,Any])->dict[str,Any]:
	...
	if _safe_projection_enabled(runner):
		...
		with torch.no_grad():noise=(torch.randn_like(target_actor_actions_clean)*float(runner.cfg.algo.policy_noise)).clamp(-float(runner.cfg.algo.noise_clip),float(runner.cfg.algo.noise_clip));target_actor_actions=(target_actor_actions_clean+noise).clamp(-float(runner.cfg.model.max_action),float(runner.cfg.model.max_action));projection_started=time.perf_counter();projected_target_actions=runner.safety_projector.project_actions_from_safety_local(next_obs['safety_local'],target_actor_actions);shared_ctx['projected_target_actions']=projected_target_actions;_record_projection_event(runner,stage='target',batch_size=int(target_actor_actions.shape[0]),elapsed_s=time.perf_counter()-projection_started)
	return shared_ctx
...
def build_reward_summary(runner:Any)->dict[str,Any]:
	...
	return{'episodes':episode_indices,'episode_total_reward':[float(value)for value in runner.episode_rewards],'components':components,'aggregates':aggregates,'madrl_throughput_bonus_weight_final':float(getattr(runner,'_last_throughput_bonus_weight',0.0)),'training_contract':training_contract,'training_contract_signature':build_training_contract_signature(runner.cfg)}
class TrainRunner:
