...
def select_action_batch_with_info(runner:Any,obs_np:dict)->tuple[np.ndarray,dict[str,np.ndarray]|None]:
	...
	with torch.inference_mode():
		...
		if _safe_projection_enabled(runner):projection_started=time.perf_counter();projected_action_t,diagnostics=runner.safety_projector.project_actions_from_safety_local(obs_t['safety_local'],raw_action_t,return_diagnostics=True);_record_projection_event(runner,stage='rollout',batch_size=int(raw_action_t.shape[0]),elapsed_s=time.perf_counter()-projection_started,diagnostics=diagnostics);action_t,projector_residual_info=enforce_local_action_feasibility_torch(obs_t['safety_local'],projected_action_t,efficiency=float(runner.cfg.env.efficiency),dt_hours=float(runner.cfg.env.dt),soc_min=float(runner.cfg.env.soc_min),soc_max=float(runner.cfg.env.soc_max));_record_projector_local_infeasible(runner,projector_residual_info);action_info=compute_action_gap_metrics_torch(obs_t['safety_local'],raw_action_t,action_t);action_info=_override_soc_penalty_metrics(action_info,projector_residual_info)
		elif'safety_local'in obs_t:action_t,action_info=enforce_local_action_feasibility_torch(obs_t['safety_local'],raw_action_t,efficiency=float(runner.cfg.env.efficiency),dt_hours=float(runner.cfg.env.dt),soc_min=float(runner.cfg.env.soc_min),soc_max=float(runner.cfg.env.soc_max))
...
def build_shared_update_ctx(runner:Any,batch:dict[str,Any])->dict[str,Any]:
	...
	with torch.no_grad():target_actor_actions_clean=torch.stack([agent._actor_target_call(next_obs)for agent in runner.agent_n],dim=1)
	...
	if _safe_projection_enabled(runner):
		...
		with torch.no_grad():noise=(torch.randn_like(target_actor_actions_clean)*float(runner.cfg.algo.policy_noise)).clamp(-float(runner.cfg.algo.noise_clip),float(runner.cfg.algo.noise_clip));target_actor_actions=(target_actor_actions_clean+noise).clamp(-float(runner.cfg.model.max_action),float(runner.cfg.model.max_action));projection_started=time.perf_counter();shared_ctx['projected_target_actions']=runner.safety_projector.project_actions_from_safety_local(next_obs['safety_local'],target_actor_actions);_record_projection_event(runner,stage='target',batch_size=int(target_actor_actions.shape[0]),elapsed_s=time.perf_counter()-projection_started)
	return shared_ctx
def save_runner_model(runner:Any,model_dir:str,episode:int)->None:
	...
	manifest=build_checkpoint_manifest(algorithm=runner.cfg.algo.name,saved_episode_tag=episode,episodes_completed=runner.episodes_completed,total_steps=runner.total_steps,num_envs=runner.cfg.train.num_envs,episode_limit=runner.cfg.env.episode_limit,save_dir=algo_dir);write_checkpoint_manifest(algo_dir,manifest)
...
def build_reward_summary(runner:Any)->dict[str,Any]:
	episode_indices=list(range(1,len(runner.episode_rewards)+1));components={str(meta.key):{'label':str(getattr(meta,'label',meta.key)),'color':str(getattr(meta,'color','#111827')),'sign':int(getattr(meta,'sign',0)),'values':[float(value)for value in runner.episode_reward_components.get(str(meta.key),[])]}for meta in runner.reward_component_meta};aggregates:dict[str,list[float]]={};grid_safety_keys=[str(meta.key)for meta in runner.reward_component_meta if str(meta.key).startswith('r_safe_')and int(getattr(meta,'sign',0))==-1]
	...
	def run(self)->int:
		...
		try:
			...
			while interaction_step<target_interactions:
				action_start=time.perf_counter();action_batch,action_info=self._select_action_batch_with_info(obs);action_time_total+=time.perf_counter()-action_start;env_step_start=time.perf_counter();next_obs,reward,terminated,truncated,info_list=self.env.step([action_batch[:,agent_id].copy()for agent_id in range(self.cfg.env.num_agents)]);reward,info_list=self._apply_controller_action_postprocessing(reward,info_list,action_info);done=np.logical_or(terminated,truncated).astype(np.float32);env_step_time_total+=time.perf_counter()-env_step_start
