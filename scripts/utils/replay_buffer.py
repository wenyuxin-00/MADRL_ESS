...
class ReplayBuffer:
	...
	def sample_torch(self,device:torch.device|str,*,pin_memory:bool=False,non_blocking:bool=False)->dict[str,NestedArray]:
		...
		for(name,storage)in(('action',self.action_storage),('reward',self.reward_storage),('terminated',self.terminated_storage),('truncated',self.truncated_storage)):np.take(storage,indices,axis=0,out=staging[f"{name}_numpy"])
	...
	def _get_torch_staging_cache(self,device:torch.device,*,pin_memory:bool)->dict[str,object]:
		...
		obs_tensors,obs_numpy=_alloc_nested();next_obs_tensors,next_obs_numpy=_alloc_nested();scalars={name:torch.empty((self.batch_size,self.num_agents,1 if name!='action'else self.action_dim),dtype=torch.float32,pin_memory=use_pinned)for name in('action','reward','terminated','truncated')};cache={'obs_tensors':obs_tensors,'obs_numpy':obs_numpy,'next_obs_tensors':next_obs_tensors,'next_obs_numpy':next_obs_numpy,'action_tensor':scalars['action'],'action_numpy':scalars['action'].numpy(),'reward_tensor':scalars['reward'],'reward_numpy':scalars['reward'].numpy(),'terminated_tensor':scalars['terminated'],'terminated_numpy':scalars['terminated'].numpy(),'truncated_tensor':scalars['truncated'],'truncated_numpy':scalars['truncated'].numpy()};self._torch_staging_cache[key]=cache;return cache
def to_torch_batch(batch:dict[str,NestedArray],device:torch.device|str)->dict[str,NestedArray]:return{name:to_torch_nested(batch[name],device)for name in('obs','action','reward','next_obs','terminated','truncated')}
