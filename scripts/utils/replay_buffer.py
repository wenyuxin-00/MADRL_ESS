...
class ReplayBuffer:
	def __init__(self,cfg:object)->None:
		...
		self.buffer_size=int(cfg.train.buffer_size);self.batch_size=int(cfg.train.batch_size);self.num_agents=int(cfg.env.num_agents);self.action_dim=int(cfg.runtime.action_dim);self.observation_schema={key:tuple(shape)for(key,shape)in dict(cfg.runtime.observation_schema).items()};self.obs_storage=_allocate_nested_storage(self.observation_schema,self.buffer_size);self.next_obs_storage=_allocate_nested_storage(self.observation_schema,self.buffer_size);shape=self.buffer_size,self.num_agents;self.action_storage=np.zeros((*shape,self.action_dim),dtype=np.float32);self.reward_storage=np.zeros((*shape,1),dtype=np.float32);self.done_storage=np.zeros((*shape,1),dtype=np.float32);self.position=0;self.current_size=0;(self._torch_staging_cache):dict[tuple[object,...],dict[str,object]]={}
	def _sample_indices(self)->np.ndarray:return np.random.choice(self.current_size,size=self.batch_size,replace=self.current_size<self.batch_size)
	def store_transitions_batched(self,obs:NestedArray,action:np.ndarray,reward:np.ndarray,next_obs:NestedArray,done:np.ndarray)->None:
		action,reward,done=(np.asarray(value,dtype=np.float32)for value in(action,reward,done));num_envs=int(action.shape[0]);first_block=min(num_envs,self.buffer_size-self.position);blocks=[(slice(0,first_block),slice(self.position,self.position+first_block))]
		...
		for(source,target)in blocks:
			...
			_store_nested_range(self.obs_storage,obs,source,target);_store_nested_range(self.next_obs_storage,next_obs,source,target);self.action_storage[target],self.reward_storage[target],self.done_storage[target]=action[source],reward[source],done[source]
		self.position=(self.position+num_envs)%self.buffer_size;self.current_size=min(self.current_size+num_envs,self.buffer_size)
	def sample(self)->dict[str,NestedArray]:indices=self._sample_indices();return{'obs':_sample_nested(self.obs_storage,indices),'action':np.asarray(self.action_storage[indices],dtype=np.float32).copy(),'reward':np.asarray(self.reward_storage[indices],dtype=np.float32).copy(),'next_obs':_sample_nested(self.next_obs_storage,indices),'done':np.asarray(self.done_storage[indices],dtype=np.float32).copy()}
	def sample_torch(self,device:torch.device|str,*,pin_memory:bool=False,non_blocking:bool=False)->dict[str,NestedArray]:
		...
		for(name,storage)in(('action',self.action_storage),('reward',self.reward_storage),('done',self.done_storage)):np.take(storage,indices,axis=0,out=staging[f"{name}_numpy"])
	...
	def _get_torch_staging_cache(self,device:torch.device,*,pin_memory:bool)->dict[str,object]:
		...
		obs_tensors,obs_numpy=_alloc_nested();next_obs_tensors,next_obs_numpy=_alloc_nested();scalars={name:torch.empty((self.batch_size,self.num_agents,1 if name!='action'else self.action_dim),dtype=torch.float32,pin_memory=use_pinned)for name in('action','reward','done')};cache={'obs_tensors':obs_tensors,'obs_numpy':obs_numpy,'next_obs_tensors':next_obs_tensors,'next_obs_numpy':next_obs_numpy,'action_tensor':scalars['action'],'action_numpy':scalars['action'].numpy(),'reward_tensor':scalars['reward'],'reward_numpy':scalars['reward'].numpy(),'done_tensor':scalars['done'],'done_numpy':scalars['done'].numpy()};self._torch_staging_cache[key]=cache;return cache
def to_torch_batch(batch:dict[str,NestedArray],device:torch.device|str)->dict[str,NestedArray]:return{name:to_torch_nested(batch[name],device)for name in('obs','action','reward','next_obs','done')}
