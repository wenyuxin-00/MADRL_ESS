...
def normalize_internal_signal_name(signal_name: str) -> str:
    ...
    if normalized == LEGACY_PRICE_SIGNAL:
        raise LegacyPriceSchemaError(f"Legacy internal signal name 'price' is no longer supported. Use '{WHOLESALE_PRICE_SIGNAL}' at all internal interfaces.")
    return normalized

def derive_import_price(wholesale_price: float | np.ndarray, *, markup_eur_per_kwh: float) -> float | np.ndarray:
    return np.asarray(wholesale_price, dtype=np.float32) + np.float32(markup_eur_per_kwh)

def derive_import_price_seq(wholesale_price_seq: Sequence[float] | np.ndarray, *, markup_eur_per_kwh: float) -> np.ndarray:
...
def assert_no_legacy_price_schema(keys: Iterable[object], *, context: str) -> None:
    ...
    if legacy_keys:
        raise LegacyPriceSchemaError(f"{context} uses legacy price schema keys {legacy_keys}. Use '{WHOLESALE_PRICE_SIGNAL}', '{WHOLESALE_PRICE_SEQ_FIELD}', '{WHOLESALE_PRICE_PRED_COLUMN}', '{IMPORT_PRICE_COLUMN}', '{IMPORT_PRICE_SEQ_FIELD}', '{IMPORT_PRICE_PRED_COLUMN}', and '{IMPORT_PRICE_MARKUP_KEY}' instead.")
