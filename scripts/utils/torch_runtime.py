...
def configure_torch_runtime(runtime_or_cfg: Any | str | torch.device | None=None, *, device: str | torch.device | None=None, require_cuda: bool | None=None, seed: int | None=None, mode: str | None=None, worker_rank: int=0) -> TorchRuntimeState:
    ...
    resolved_mode = resolve_runtime_mode(mode if mode is not None else _runtime_attr(runtime_cfg, 'execution_mode', PERFORMANCE_RUNTIME_MODE))
    resolved_require_cuda = bool(_runtime_attr(runtime_cfg, 'require_cuda', False) if require_cuda is None else require_cuda)
    resolved_seed = int(_runtime_attr(runtime_cfg, 'seed', 0)) if seed is None and runtime_cfg is not None and hasattr(runtime_cfg, 'seed') else None if seed is None else int(seed)
    ...
    deterministic_algorithms = bool(_runtime_attr(runtime_cfg, 'use_deterministic_algorithms', strict_mode))
    pin_memory = bool(_runtime_attr(runtime_cfg, 'pin_memory', resolved_device.type == 'cuda'))
    non_blocking_transfers = bool(_runtime_attr(runtime_cfg, 'non_blocking_transfers', resolved_device.type == 'cuda'))
    enable_amp = bool(_runtime_attr(runtime_cfg, 'enable_amp', resolved_device.type == 'cuda' and resolved_mode == PERFORMANCE_RUNTIME_MODE))
    amp_dtype = str(_runtime_attr(runtime_cfg, 'amp_dtype', 'bfloat16'))
    enable_compile = bool(_runtime_attr(runtime_cfg, 'enable_compile', resolved_device.type == 'cuda' and resolved_mode == PERFORMANCE_RUNTIME_MODE))
    ...
    if enable_compile and hasattr(torch, '_dynamo') and hasattr(torch._dynamo, 'config'):
        try:
            ...
            pass
    state = TorchRuntimeState(device=resolved_device, mode=resolved_mode, seed=worker_seed, worker_rank=int(worker_rank), matmul_precision=matmul_precision, allow_tf32=allow_tf32, cudnn_benchmark=cudnn_benchmark and (not cudnn_deterministic), cudnn_deterministic=cudnn_deterministic, deterministic_algorithms=deterministic_algorithms, pin_memory=pin_memory, non_blocking_transfers=non_blocking_transfers, enable_amp=enable_amp, amp_dtype=amp_dtype, enable_compile=enable_compile, compile_mode=compile_mode, compile_fullgraph=compile_fullgraph, compile_dynamic=compile_dynamic)
