...
from scripts.utils.grid_notebook_compare_reports import (
...
from scripts.utils.grid_notebook_plotting import (
...
def _extract_compare_signature(bundle: Mapping[str, object]) -> dict[str, object]:
    ...
    if removed_reward_keys:
        raise ValueError(
            "Stored reward_controls use removed legacy key(s): "
            f"{removed_reward_keys}. Regenerate the checkpoint bundle with the current mainline schema."
        )
    ...
    return {
        ...
        "w_soc_pen": float(reward_controls.get("w_soc_pen", 0.0)),
