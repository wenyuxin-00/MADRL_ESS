...
def resolve_battery_controls(cfg,battery_controls:Mapping[str,object]|None=None)->dict[str,object]:
	...
	if not resolved['soc_min']<=resolved['train_init_soc_low']<=resolved['train_init_soc_high']<=resolved['soc_max']:raise ValueError(f"Invalid training initial SoC range: train_init_soc_low={resolved['train_init_soc_low']}, train_init_soc_high={resolved['train_init_soc_high']}, expected within [{resolved['soc_min']}, {resolved['soc_max']}].")
...
def apply_notebook_experiment_settings(cfg,*,prediction_mode:str,test_start_date:str|int|None,test_end_date:str|int|None,agent_profiles:list[str]|None=None,agent_bus_ids:list[int]|None=None,load_scale:float|list[float]|None=None,pv_scale:float|list[float]|None=None,future_horizon:int|None=None,battery_controls:Mapping[str,object]|None=None,forecast_controls:Mapping[str,object]|None=None,train_year:int|None=None,test_year:int|None=None)->dict[str,object]:
	...
	for(field_name,value)in{'battery_capacity':list(resolved_battery_controls['battery_capacity']),'max_charge_rate':resolved_battery_controls['max_charge_rate'],'efficiency':resolved_battery_controls['efficiency'],'init_soc':resolved_battery_controls['init_soc'],'train_init_soc_low':resolved_battery_controls['train_init_soc_low'],'train_init_soc_high':resolved_battery_controls['train_init_soc_high'],'soc_min':resolved_battery_controls['soc_min'],'soc_max':resolved_battery_controls['soc_max'],'soc_target':resolved_battery_controls['soc_target']}.items():setattr(cfg.env,field_name,value)
...
def _build_rollout_records(*,controller_label:str,episode_idx:int,step_in_episode:int,timestamp,info:dict[str,object],load_pred,pv_pred,wholesale_price_pred:float,import_price_pred:float,agent_profiles:list[str],bus_ids:list[int],agent_bus_set:set[int],dt_hours:float,export_subsidy:float,fixed_load_kw:float,fixed_generation_kw:float,global_step:int|None=None)->dict[str,object]:
	load_values=np.asarray(info['load'],dtype=np.float32);pv_values=np.asarray(info['pv'],dtype=np.float32);base_net_load=np.asarray(info.get('base_net_load',load_values-pv_values),dtype=np.float32);base_net_load_effective=np.asarray(info.get('base_net_load_effective',base_net_load),dtype=np.float32);battery_power=np.asarray(info['e_bat'],dtype=np.float32);battery_power_req=np.asarray(info.get('e_bat_req',battery_power),dtype=np.float32);net_load=np.asarray(info.get('net_load',base_net_load_effective+battery_power),dtype=np.float32);pv_raw=np.asarray(info.get('pv_raw',pv_values),dtype=np.float32);pv_effective=np.asarray(info.get('pv_effective',pv_raw),dtype=np.float32);pv_curtail=np.asarray(info.get('pv_curtail',pv_raw-pv_effective),dtype=np.float32);grid_import=np.asarray(info.get('grid_import_kw',np.maximum(net_load,.0)),dtype=np.float32);grid_export=np.asarray(info.get('grid_export_kw',np.maximum(-net_load,.0)),dtype=np.float32);battery_charge=np.clip(battery_power,.0,None).astype(np.float32);battery_discharge=np.maximum(-battery_power,.0).astype(np.float32);line_loading_pct=np.asarray(info.get('line_loading_pct',np.zeros(0,dtype=np.float32)),dtype=np.float32);trafo_loading_pct=np.asarray(info.get('trafo_loading_pct',np.zeros(0,dtype=np.float32)),dtype=np.float32);pp_root_p_array=np.asarray(info.get('trafo_p_signed_kw',np.zeros(1,dtype=np.float32)),dtype=np.float32).reshape(-1);pp_root_p_kw=float(pp_root_p_array.sum())if pp_root_p_array.size else float('nan');actual_price=np.float32(info[IMPORT_PRICE_COLUMN]);purchase_cost_per_agent=(grid_import*np.float32(dt_hours)*actual_price).astype(np.float32);export_subsidy_per_agent=(grid_export*np.float32(dt_hours)*np.float32(export_subsidy)).astype(np.float32);storage_charge_cost_per_agent=(battery_charge*np.float32(dt_hours)*actual_price).astype(np.float32);storage_discharge_revenue_per_agent=(battery_discharge*np.float32(dt_hours)*actual_price).astype(np.float32);storage_profit_per_agent=(storage_discharge_revenue_per_agent-storage_charge_cost_per_agent).astype(np.float32);storage_objective_per_agent=(-storage_profit_per_agent).astype(np.float32)
	def _agent_metric(name:str,default:float=.0)->np.ndarray:return np.asarray(info.get(name,np.full((len(agent_profiles),),default,dtype=np.float32)),dtype=np.float32)
	voltage_penalty_per_agent=_agent_metric('madrl_r_safe_v');line_penalty_per_agent=_agent_metric('madrl_r_safe_line');trafo_penalty_per_agent=_agent_metric('madrl_r_safe_trafo');objective_per_agent=(purchase_cost_per_agent-export_subsidy_per_agent+voltage_penalty_per_agent+line_penalty_per_agent+trafo_penalty_per_agent).astype(np.float32);agent_raw_net_load_kw,agent_effective_net_load_kw,agent_post_action_net_load_kw=float(np.sum(base_net_load)),float(np.sum(base_net_load_effective)),float(np.sum(net_load));feeder_raw_net_load_kw,feeder_effective_net_load_kw,feeder_post_action_net_load_kw=float(agent_raw_net_load_kw+fixed_load_kw-fixed_generation_kw),float(agent_effective_net_load_kw+fixed_load_kw-fixed_generation_kw),float(agent_post_action_net_load_kw+fixed_load_kw-fixed_generation_kw);voltage_penalty_total=float(np.mean(voltage_penalty_per_agent))if voltage_penalty_per_agent.size else .0;line_penalty_total=float(np.mean(line_penalty_per_agent))if line_penalty_per_agent.size else .0;trafo_penalty_total=float(np.mean(trafo_penalty_per_agent))if trafo_penalty_per_agent.size else .0;soc_penalty_total=float(np.sum(_agent_metric('madrl_r_soc_regularization')));purchase_cost_total,export_subsidy_total=float(np.sum(purchase_cost_per_agent)),float(np.sum(export_subsidy_per_agent));storage_charge_cost_eur=float(np.sum(storage_charge_cost_per_agent));storage_discharge_revenue_eur=float(np.sum(storage_discharge_revenue_per_agent));storage_profit_eur=float(np.sum(storage_profit_per_agent));storage_objective_eur=float(-storage_profit_eur);objective_total=float(purchase_cost_total-export_subsidy_total+voltage_penalty_total+line_penalty_total+trafo_penalty_total);vm_pu=np.asarray(info.get('vm_pu',np.zeros(len(bus_ids),dtype=np.float32)),dtype=np.float32)
	step_row={'controller':controller_label,'episode_idx':int(episode_idx),'step':int(step_in_episode),'timestamp':timestamp,WHOLESALE_PRICE_SIGNAL:float(info[WHOLESALE_PRICE_SIGNAL]),IMPORT_PRICE_COLUMN:float(info[IMPORT_PRICE_COLUMN]),WHOLESALE_PRICE_PRED_COLUMN:float(wholesale_price_pred),IMPORT_PRICE_PRED_COLUMN:float(import_price_pred),'base_net_load_total':agent_raw_net_load_kw,'base_net_load_effective_total':agent_effective_net_load_kw,'net_load_total':agent_post_action_net_load_kw,'agent_raw_net_load_kw':agent_raw_net_load_kw,'agent_effective_net_load_kw':agent_effective_net_load_kw,'agent_post_action_net_load_kw':agent_post_action_net_load_kw,'fixed_load_kw':fixed_load_kw,'fixed_generation_kw':fixed_generation_kw,'feeder_raw_net_load_kw':feeder_raw_net_load_kw,'feeder_effective_net_load_kw':feeder_effective_net_load_kw,'feeder_post_action_net_load_kw':feeder_post_action_net_load_kw,'root_net_exchange_kw':pp_root_p_kw,'pp_root_p_available':bool(pp_root_p_array.size),'load_total':float(np.sum(load_values)),'pv_raw_total':float(np.sum(pv_raw)),'pv_effective_total':float(np.sum(pv_effective)),'pv_curtail_total':float(np.sum(pv_curtail)),'grid_import_total':float(np.sum(grid_import)),'grid_export_total':float(np.sum(grid_export)),'battery_power_kw':float(np.sum(battery_power)),'battery_charge_total':float(np.sum(battery_charge)),'battery_discharge_total':float(np.sum(battery_discharge)),'purchase_cost_total':purchase_cost_total,'export_subsidy_total':export_subsidy_total,'objective_total':objective_total,'storage_charge_cost_eur':storage_charge_cost_eur,'storage_discharge_revenue_eur':storage_discharge_revenue_eur,'storage_profit_eur':storage_profit_eur,'storage_objective_eur':storage_objective_eur,'soc_penalty_total':soc_penalty_total,'voltage_penalty_total':voltage_penalty_total,'line_penalty_total':line_penalty_total,'trafo_penalty_total':trafo_penalty_total,'line_loading_pct_max':float(np.max(line_loading_pct))if line_loading_pct.size else .0,'trafo_loading_pct_max':float(np.max(trafo_loading_pct))if trafo_loading_pct.size else .0,'n_trafo_violations':int(info.get('n_trafo_violations',0)),'misocp_fallback':float(info.get('misocp_fallback',.0)),'simultaneous_step_flag':float(info.get('simultaneous_step_flag',.0))}
	if global_step is not None:step_row['global_step']=int(global_step)
	agent_rows,grid_rows=[],[];load_pred_values,pv_pred_values,soc_next=np.asarray(load_pred,dtype=np.float32),np.asarray(pv_pred,dtype=np.float32),np.asarray(info['soc_next'],dtype=np.float32);madrl_r_inc=_agent_metric('madrl_r_inc');madrl_r_action_penalty=_agent_metric('madrl_r_action_penalty');madrl_r_soc_regularization=_agent_metric('madrl_r_soc_regularization');madrl_r_throughput_bonus=_agent_metric('madrl_r_throughput_bonus');madrl_r_safe_total=_agent_metric('madrl_r_safe_total');madrl_r_total_internal=_agent_metric('madrl_r_total_internal');madrl_action_projection_gap=np.asarray(info.get('controller_action_gap',np.zeros(len(agent_profiles),dtype=np.float32)),dtype=np.float32);madrl_action_feasibility_gap=np.asarray(info.get('soc_penalty_unweighted',np.zeros(len(agent_profiles),dtype=np.float32)),dtype=np.float32);madrl_actor_action_mapping_gap=np.asarray(info.get('actor_action_mapping_gap',np.zeros(len(agent_profiles),dtype=np.float32)),dtype=np.float32);madrl_throughput_kwh=np.asarray(info.get('madrl_throughput_kwh',np.abs(battery_power)*np.float32(dt_hours)),dtype=np.float32);madrl_throughput_bonus_weight=float(info.get('madrl_throughput_bonus_weight',0.))
	for(agent_idx,profile)in enumerate(agent_profiles):
		agent_row={'controller':controller_label,'episode_idx':int(episode_idx),'step':int(step_in_episode),'timestamp':timestamp,'agent_id':agent_idx,'agent_profile':str(profile),'load':float(load_values[agent_idx]),'load_pred':float(load_pred_values[agent_idx]),'pv':float(pv_values[agent_idx]),'pv_pred':float(pv_pred_values[agent_idx]),'e_bat':float(battery_power[agent_idx]),'e_bat_req':float(battery_power_req[agent_idx]),'soc':float(soc_next[agent_idx]),'purchase_cost':float(purchase_cost_per_agent[agent_idx]),'export_subsidy':float(export_subsidy_per_agent[agent_idx]),'objective_total':float(objective_per_agent[agent_idx]),'storage_objective_eur':float(storage_objective_per_agent[agent_idx]),'storage_charge_cost_eur':float(storage_charge_cost_per_agent[agent_idx]),'storage_discharge_revenue_eur':float(storage_discharge_revenue_per_agent[agent_idx]),'storage_profit_eur':float(storage_profit_per_agent[agent_idx]),'madrl_r_inc':float(madrl_r_inc[agent_idx]),'madrl_r_action_penalty':float(madrl_r_action_penalty[agent_idx]),'madrl_r_soc_regularization':float(madrl_r_soc_regularization[agent_idx]),'madrl_r_throughput_bonus':float(madrl_r_throughput_bonus[agent_idx]),'madrl_r_safe_v':float(voltage_penalty_per_agent[agent_idx]),'madrl_r_safe_line':float(line_penalty_per_agent[agent_idx]),'madrl_r_safe_trafo':float(trafo_penalty_per_agent[agent_idx]),'madrl_r_safe_total':float(madrl_r_safe_total[agent_idx]),'madrl_r_total_internal':float(madrl_r_total_internal[agent_idx]),'madrl_action_projection_gap':float(madrl_action_projection_gap[agent_idx]),'madrl_action_feasibility_gap':float(madrl_action_feasibility_gap[agent_idx]),'madrl_actor_action_mapping_gap':float(madrl_actor_action_mapping_gap[agent_idx]),'madrl_throughput_kwh':float(madrl_throughput_kwh[agent_idx]),'madrl_throughput_bonus_weight':madrl_throughput_bonus_weight}
...
def _apply_post_action_reward_regularization(env,reward_array:np.ndarray,info:dict[str,object],action_info:dict[str,np.ndarray]|None)->tuple[np.ndarray,dict[str,object]]:
	reward_vector=np.asarray(reward_array,dtype=np.float32).reshape(-1)
	updated=dict(info);updated['reward']=reward_vector.astype(np.float32);return reward_vector.astype(np.float32),updated
def collect_controller_rollout(cfg,*,label:str,controller=None,controller_builder=None,action_fn=None,episode_indices:list[int]|None=None,soc_mode:str='reset')->RolloutResult:
	...
	try:
		...
		carried_soc:np.ndarray|None=None;global_step=0;selected_episode_start_timestamps:list[str]=[];selected_episode_end_timestamps:list[str]=[];expected_next_episode_timestamp:pd.Timestamp|None=None
		for episode_idx in effective_episode_indices:
			...
			if resolved_soc_mode=='continuous':
				episode_meta=dict(reset_info.get('episode_meta',{}));timestamps=[str(value)for value in list(episode_meta.get('timestamps')or[])]
				if not timestamps:raise ValueError(f"continuous SoC rollout requires timestamp metadata for episode_idx={episode_idx}. Re-run the source notebook after regenerating shared-data.")
				first_timestamp,last_timestamp=pd.Timestamp(timestamps[0]),pd.Timestamp(timestamps[-1])
				if expected_next_episode_timestamp is not None and first_timestamp!=expected_next_episode_timestamp:raise ValueError(f"continuous SoC rollout refuses to carry SoC across a timestamp gap: expected next episode to start at {expected_next_episode_timestamp.isoformat()}, got {first_timestamp.isoformat()} for episode_idx={episode_idx}. Re-run with contiguous selected_episode_indices.")
				if expected_next_episode_timestamp is None and selected_episode_start_timestamps:raise ValueError(f"continuous SoC rollout reached episode_idx={episode_idx} after a previous episode without bootstrap timestamp. Re-run with a contiguous date range.")
				selected_episode_start_timestamps.append(first_timestamp.isoformat());selected_episode_end_timestamps.append(last_timestamp.isoformat());bootstrap_timestamps=[str(value)for value in list(episode_meta.get('bootstrap_timestamps')or[])];expected_next_episode_timestamp=pd.Timestamp(bootstrap_timestamps[0])if bootstrap_timestamps else None
			...
			while not done:
				wholesale_price_pred=_aligned_wholesale_price_prediction(env,previous_raw_obs,raw_obs);import_price_pred=float(derive_import_price(wholesale_price_pred,markup_eur_per_kwh=float(getattr(env,'import_price_markup_eur_per_kwh',get_import_price_markup(env)))));load_pred=_aligned_prediction(previous_raw_obs,raw_obs,'load_seq');pv_pred=_aligned_prediction(previous_raw_obs,raw_obs,'pv_seq');timestamp=_step_timestamp(reset_info,step_in_episode)
...
def resolve_madrl_notebook_training(cfg,*,spec:Mapping[str,object],force_retrain_madrl:bool,root:str|Path|None=None,notebook_path:str|Path|None=None)->dict[str,object]:
	...
	root_path=_project_root_from_notebook_helpers()if root is None else Path(root).resolve()
	spec_dict={str(key):value for(key,value)in dict(spec).items()}
	notebook_hint=str(Path(notebook_path).as_posix())if notebook_path is not None else f"notebooks/madrl/{spec_dict.get('experiment_name','madrl')}.ipynb"
	prediction_mode=resolve_prediction_mode_from_forecast_backend(cfg.forecast.type)
	expected_saved_fields={'agent_profiles':list(cfg.data.agent_profiles),'agent_bus_ids':list(cfg.grid.agent_bus_ids),'load_scale':list(cfg.data.load_scale),'pv_scale':list(cfg.data.pv_scale),'future_horizon':int(cfg.env.future_horizon)}
	runtime_controls={}
	expected_shared_data_signature=''
	if prediction_mode==NORMAL_PREDICTION_MODE:
		expected_shared_data_dir_raw=str(getattr(cfg.runtime,'shared_data_dir',None)or'').strip()
		expected_shared_data_signature=str(getattr(cfg.runtime,'shared_data_signature',None)or'').strip()
		if not expected_shared_data_dir_raw or not expected_shared_data_signature:
			raise ValueError(f"Missing forecast shared-data runtime contract for {notebook_hint}. Expected canonical forecast shared-data contract loaded from 'notebooks/record/forecast/lstm/shared_data_record.json' with both 'shared_data_dir' and 'signature_hash'. Re-run notebooks/forecast/forecast_lstm.ipynb, then rerun {notebook_hint}.")
		expected_shared_data_dir=str(Path(expected_shared_data_dir_raw).resolve())
		if not Path(expected_shared_data_dir).exists():
			raise FileNotFoundError(f"Missing forecast shared-data package: '{expected_shared_data_dir}'. Expected canonical forecast shared-data directory for {notebook_hint}. Re-run notebooks/forecast/forecast_lstm.ipynb before rerunning {notebook_hint}.")
		runtime_controls={'shared_data_dir':expected_shared_data_dir,'shared_data_signature':expected_shared_data_signature}
	reward_controls={'action_boundary_penalty_weight':float(cfg.reward.action_boundary_penalty_weight),'soc_boundary_regularization_weight':float(cfg.reward.soc_boundary_regularization_weight),'throughput_bonus_eur_per_kwh_max':float(cfg.reward.throughput_bonus_eur_per_kwh_max),'soc_boundary_epsilon':float(cfg.reward.soc_boundary_epsilon),'soc_boundary_margin':float(cfg.reward.soc_boundary_margin),'w_voltage_pen':float(cfg.reward.w_voltage_pen),'w_line_pen':float(cfg.reward.w_line_pen),'w_trafo_pen':float(cfg.reward.w_trafo_pen),'export_subsidy_eur_per_kwh':float(cfg.reward.export_subsidy_eur_per_kwh),'import_price_markup_eur_per_kwh':float(cfg.reward.import_price_markup_eur_per_kwh),'storage_objective_mode':str(cfg.reward.storage_objective_mode),'storage_price_mode':str(cfg.reward.storage_price_mode),'storage_profit_weight':float(cfg.reward.storage_profit_weight)}
	...
	data_controls={'prediction_mode':prediction_mode,'agent_profiles':list(cfg.data.agent_profiles),'agent_bus_ids':list(cfg.grid.agent_bus_ids),'load_scale':list(cfg.data.load_scale),'pv_scale':list(cfg.data.pv_scale),'future_horizon':int(cfg.env.future_horizon),'train_year':int(cfg.data.train_year),'test_year':int(cfg.data.test_year),'test_start_date':str(cfg.data.test_start_date),'test_end_date':str(cfg.data.test_end_date)}
	battery_controls={'battery_capacity':list(cfg.env.battery_capacity),'max_charge_rate':float(cfg.env.max_charge_rate),'efficiency':float(cfg.env.efficiency),'init_soc':float(cfg.env.init_soc),'train_init_soc_low':float(cfg.env.train_init_soc_low),'train_init_soc_high':float(cfg.env.train_init_soc_high),'soc_min':float(cfg.env.soc_min),'soc_max':float(cfg.env.soc_max),'soc_target':float(cfg.env.soc_target)}
	train_controls={'profile':'base','model_family':str(cfg.model.family),'train_episodes':int(cfg.train.train_episodes),'max_train_steps':cfg.train.max_train_steps,'num_envs':int(cfg.train.num_envs),'vec_env_type':str(cfg.train.vec_env_type),'parallel_episode_sampling':str(cfg.train.parallel_episode_sampling),'batch_size':int(cfg.train.batch_size),'buffer_size':int(cfg.train.buffer_size),'update_interval':int(cfg.train.update_interval),'updates_per_step':int(cfg.train.updates_per_step),'learning_starts_transitions':cfg.train.learning_starts_transitions,'actor_learning_starts_transitions':cfg.train.actor_learning_starts_transitions,'n_step_return':int(cfg.train.n_step_return),'feasible_random_exploration_start':float(cfg.train.feasible_random_exploration_start),'feasible_random_exploration_end':float(cfg.train.feasible_random_exploration_end),'feasible_random_exploration_decay_steps':int(cfg.train.feasible_random_exploration_decay_steps),'discount_gamma':float(cfg.algo.gamma),'policy_update_freq':int(cfg.algo.policy_update_freq),'actor_lr':float(cfg.train.actor_lr),'critic_lr':float(cfg.train.critic_lr),'noise_std_init':float(cfg.train.noise_std_init),'noise_std_min':float(cfg.train.noise_std_min),'noise_decay_steps':float(cfg.train.noise_decay_steps),'use_noise_decay':bool(cfg.train.use_noise_decay),'show_progress':bool(cfg.train.show_progress),'progress_episode_interval':int(cfg.train.progress_episode_interval),'train_window_days':int(cfg.env.train_window_days),'window_stride_days':int(cfg.env.window_stride_days)}
	checkpoint_controls={'experiment_name':str(spec_dict['experiment_name'])}
	if force_retrain_madrl:
		launch=run_external_train_mainline(project_root=root_path,experiment_controls=experiment_controls,data_controls=data_controls,train_controls=train_controls,battery_controls=battery_controls,checkpoint_controls=checkpoint_controls,data_dir=root_path/'data',env_name=str(spec_dict['env_name']))
		train_result=dict(launch['result']);result_json_path=str(launch['launch_info']['result_json_path']);model_root=str(train_result['model_root'])
	else:
		loaded=load_madrl_training_result(algorithm=str(spec_dict['algorithm']),prediction_mode=prediction_mode,experiment_name=str(spec_dict['experiment_name']),root=root_path)
		train_result=dict(loaded['result']);result_json_path=str(loaded['result_json_path']);model_root=str(loaded['model_root'])
		stored_data=dict(train_result.get('data_controls',{}));stored_reward_controls=dict(dict(train_result.get('experiment_controls',{})).get('reward_controls',{}));stored_train_controls=dict(train_result.get('train_controls',{}))
		mismatches=[f"{field_name}: expected={expected_value!r}, got={stored_data.get(field_name)!r}"for(field_name,expected_value)in expected_saved_fields.items()if stored_data.get(field_name)!=expected_value]
		...
		for field_name in('train_window_days','window_stride_days','learning_starts_transitions','actor_learning_starts_transitions','n_step_return','feasible_random_exploration_start','feasible_random_exploration_end','feasible_random_exploration_decay_steps','discount_gamma'):
			expected_value=train_controls[field_name]
			if stored_train_controls.get(field_name)!=expected_value:mismatches.append(f"train_controls.{field_name}: expected={expected_value!r}, got={stored_train_controls.get(field_name)!r}")
		if str(train_result.get('prediction_mode'))!=prediction_mode:mismatches.append(f"prediction_mode: expected={prediction_mode!r}, got={train_result.get('prediction_mode')!r}")
...
def _reject_legacy_madrl_rollout_record(*,category:str,scheme_name:str,meta:Mapping[str,object],tables:Mapping[str,pd.DataFrame])->None:
	reject_legacy_storage_profit_tables(tables,context=f"Saved rollout record '{category}/{scheme_name}'",rerun_hint='Re-run the source notebook for this record.')
	reject_legacy_storage_profit_meta(meta,context=f"Saved rollout record '{category}/{scheme_name}' metadata",rerun_hint='Re-run the source notebook for this record.')
	legacy_columns=sorted({column for frame in tables.values() for column in getattr(frame,'columns',[]) if str(column) in {'r_terminal_soc_value'}})
	if legacy_columns:raise ValueError(f"Saved rollout record '{category}/{scheme_name}' uses old MADRL parquet schema with terminal SoC shaping column(s) {legacy_columns}. New contract expects no r_terminal_soc_value. Re-run the source notebook for this record.")
	if str(category).strip().lower()!='madrl':return
	soc_mode=str(dict(meta).get('soc_mode','reset')).strip().lower()
	if soc_mode!='continuous':raise ValueError(f"Saved MADRL rollout record '{scheme_name}' has soc_mode={soc_mode!r}. New compare contract expects soc_mode='continuous'. Re-run notebooks/madrl/train_base.ipynb, train_base_safe.ipynb, or train_projection_safe.ipynb to regenerate the record.")
	for field_name in('selected_episode_start_timestamps','selected_episode_end_timestamps'):
		if field_name not in dict(meta):raise ValueError(f"Saved MADRL rollout record '{scheme_name}' is missing meta['{field_name}']. New compare contract expects continuous SoC timestamp continuity metadata. Re-run notebooks/madrl/train_base.ipynb, train_base_safe.ipynb, or train_projection_safe.ipynb to regenerate the record.")
