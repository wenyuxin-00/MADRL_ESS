...
from scripts.utils.admm_mpc_rollout_packages import _augment_rollout_with_diagnostics, build_admm_mpc_rollout_package, load_admm_mpc_rollout_package, replay_admm_mpc_rollout_package, resolve_exact_admm_mpc_rollout_package_dir, save_admm_mpc_rollout_package
from scripts.utils.admm_mpc_solver import AdmmMpcStepResult, AdmmMpcSurrogateCache, AdmmMpcWarmStartCache, AdmmMpcWindowData, AdmmMpcWindowResult, _create_model, _compute_default_rho, _day_episode_length, _duplicate_last_shift_2d, _load_gurobi, _make_local_warm_start_cache, build_admm_mpc_surrogate_cache, build_admm_mpc_window_data, resolve_default_terminal_cost_weight, run_admm_mpc_step, solve_admm_mpc_window
...
class _AdmmMpcController:
    ...
    def act(self, obs, deterministic: bool=True):
        ...
        step_result = run_admm_mpc_step(self.env, raw_obs, self.cfg, surrogate_cache=self.surrogate_cache, warm_start_cache=self.warm_start_cache, rho_init=float(rho_value), rho_min=float(self.rho_min), rho_max=float(self.rho_max), rho_adaptation=self.rho_adaptation, max_iters=int(self.max_iters), max_iters_first_step=int(self.max_iters_first_step), primal_tol=float(self.primal_tol), dual_tol=float(self.dual_tol), terminal_cost_weight_eur_per_kwh2=terminal_weight)
        self.warm_start_cache = step_result.warm_start_cache
        self._record_step_diagnostics(step_result)
        safety_local = build_safety_local_numpy(soc=np.asarray(self.env.soc, dtype=np.float32), load_raw=np.asarray(window_data.load_seq[:, 0], dtype=np.float32), pv_raw=np.asarray(window_data.pv_seq[:, 0], dtype=np.float32), battery_capacity_kwh=np.asarray(self.env.agent_c_bat, dtype=np.float32), p_max_kw=np.asarray(self.env.agent_p_max, dtype=np.float32))
        action_info = compute_action_gap_metrics_numpy(safety_local, step_result.executed_action_array, step_result.executed_action_array)
