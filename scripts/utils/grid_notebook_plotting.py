...
def plot_global_misocp_validation(rollout: RolloutResult, *, figsize: tuple[float, float]=(11.0, 8.0)):
    ...
    series = [('max_vm_abs_err_pu', _VOLTAGE_ERR_TOL_PU, 'Max |V_misocp - V_pp| [p.u.]'), ('max_line_loading_abs_err_pct', _LINE_LOADING_ERR_TOL_PCT, 'Max |Line Loading| Error [pct-point]'), ('trafo_loading_abs_err_pct', _TRAFO_LOADING_ERR_TOL_PCT, '|Trafo Loading| Error [pct-point]'), ('root_p_abs_err_kw', _ROOT_POWER_ERR_TOL_KW, '|P_root| Error [kW]')]
...
def _prepare_agent_power_balance_frame(step_df: pd.DataFrame, *, controller_label: str, tolerance_kw: float=1.0) -> tuple[pd.DataFrame, float]:
    ...
    required_columns = {'load_total', 'battery_charge_total', 'pv_curtail_total', 'grid_import_total', 'grid_export_total', 'battery_discharge_total'}
    missing = sorted(required_columns.difference(prepared.columns))
    if missing:
        raise ValueError(f"Rollout '{controller_label}' is missing required agent power-balance columns: {missing}")
    if 'pv_raw_total' not in prepared.columns:
        ...
        prepared['pv_raw_total'] = prepared['pv_effective_total'].to_numpy(dtype=np.float32) + prepared['pv_curtail_total'].to_numpy(dtype=np.float32)
    positive_total = prepared['load_total'].to_numpy(dtype=np.float32) + prepared['battery_charge_total'].to_numpy(dtype=np.float32) + prepared['grid_export_total'].to_numpy(dtype=np.float32) + prepared['pv_curtail_total'].to_numpy(dtype=np.float32)
    negative_total = prepared['pv_raw_total'].to_numpy(dtype=np.float32) + prepared['grid_import_total'].to_numpy(dtype=np.float32) + prepared['battery_discharge_total'].to_numpy(dtype=np.float32)
    ...
    if max_abs_balance_residual_kw > float(tolerance_kw):
        raise ValueError(f"Rollout '{controller_label}' violates agent-only power balance by {max_abs_balance_residual_kw:.3f} kW (tolerance={float(tolerance_kw):.3f} kW).")
...
def _prepare_compare_net_load_frame(step_df: pd.DataFrame, *, controller_label: str) -> pd.DataFrame:
    if step_df.empty:
        raise ValueError(f"Rollout '{controller_label}' has no step_df.")
    prepared = step_df.copy()
    if not {'agent_raw_net_load_kw', 'agent_effective_net_load_kw', 'agent_post_action_net_load_kw'}.issubset(prepared.columns):
        ...
        prepared['agent_effective_net_load_kw'] = np.asarray(prepared.get('base_net_load_effective_total', prepared['base_net_load_total']), dtype=np.float32)
        prepared['agent_post_action_net_load_kw'] = prepared['net_load_total'].to_numpy(dtype=np.float32)
    if not {'feeder_raw_net_load_kw', 'feeder_effective_net_load_kw', 'feeder_post_action_net_load_kw'}.issubset(prepared.columns):
        ...
        prepared['feeder_raw_net_load_kw'] = prepared['agent_raw_net_load_kw'].to_numpy(dtype=np.float32) + fixed_load - fixed_generation
        prepared['feeder_effective_net_load_kw'] = prepared['agent_effective_net_load_kw'].to_numpy(dtype=np.float32) + fixed_load - fixed_generation
        prepared['feeder_post_action_net_load_kw'] = prepared['agent_post_action_net_load_kw'].to_numpy(dtype=np.float32) + fixed_load - fixed_generation
    return prepared

def _compare_plot_fontsizes() -> tuple[int, int, int, int]:
    return (COMPARE_PLOT_TITLE_FONTSIZE, COMPARE_PLOT_LABEL_FONTSIZE, COMPARE_PLOT_TICK_FONTSIZE, COMPARE_PLOT_LEGEND_FONTSIZE)

def _prepare_shared_price_frame(step_df: pd.DataFrame, *, controller_label: str, require_predicted: bool=False) -> pd.DataFrame:
    ...
    if step_df.empty or not required_columns.issubset(step_df.columns):
        ...
        raise ValueError(f"Rollout '{controller_label}' is missing shared price columns required for compare plotting: {missing}")
...
def _resolve_adjusted_price_prediction(step_df: pd.DataFrame, *, price_markup: float) -> np.ndarray:
    ...
    return derive_import_price_seq(step_df[WHOLESALE_PRICE_PRED_COLUMN].to_numpy(dtype=np.float64), markup_eur_per_kwh=float(price_markup))

def _prepare_shared_agent_signal_frame(agent_df: pd.DataFrame, signal_name: str, *, controller_label: str, require_predicted: bool=False) -> pd.DataFrame:
    ...
    if agent_df.empty or not required_columns.issubset(agent_df.columns):
        ...
        raise ValueError(f"Rollout '{controller_label}' is missing shared {signal_name} columns required for compare plotting: {missing}")
    ...
    frame = frame.sort_values(['timestamp', 'agent_profile']).drop_duplicates(subset=['timestamp', 'agent_profile'], keep='first').reset_index(drop=True)
    return frame

def _assert_matching_shared_actual_frame(reference_frame: pd.DataFrame, candidate_frame: pd.DataFrame, *, key_columns: list[str], value_column: str, controller_label: str, signal_label: str) -> None:
    ...
    if reference_keys.shape != candidate_keys.shape or not np.array_equal(reference_keys, candidate_keys):
        raise ValueError(f"Rollout '{controller_label}' has different {signal_label} timestamps/profiles; shared forecast-vs-actual plotting requires matching actual series across rollouts.")
    if not np.allclose(reference_frame[value_column].to_numpy(dtype=np.float64), candidate_frame[value_column].to_numpy(dtype=np.float64), rtol=1e-06, atol=1e-08, equal_nan=True):
...
def _align_shared_actual_frames(reference_frame: pd.DataFrame, candidate_frame: pd.DataFrame, *, key_columns: list[str], value_column: str, controller_label: str, signal_label: str) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    _assert_matching_shared_actual_frame(reference_aligned, candidate_aligned, key_columns=key_columns, value_column=value_column, controller_label=controller_label, signal_label=signal_label)
...
def _select_shared_forecast_reference_rollout(*rollouts: RolloutResult) -> RolloutResult:
    for rollout in rollouts:
        ...
        if {'timestamp', 'wholesale_price', IMPORT_PRICE_COLUMN, WHOLESALE_PRICE_PRED_COLUMN}.issubset(step_df.columns) and {'timestamp', 'agent_profile', 'load', 'load_pred', 'pv', 'pv_pred'}.issubset(agent_df.columns):
            return rollout
    return rollouts[0]
...
def plot_shared_forecast_vs_actual(*rollouts: RolloutResult, figsize: tuple[float, float] | None=None):
    ...
    price_reference = _prepare_shared_price_frame(reference_rollout.step_df, controller_label=reference_label)
    load_reference = _prepare_shared_agent_signal_frame(reference_rollout.agent_df, 'load', controller_label=reference_label)
    pv_reference = _prepare_shared_agent_signal_frame(reference_rollout.agent_df, 'pv', controller_label=reference_label)
    for rollout in rollouts[1:]:
        controller_label = str(rollout.meta.get('controller', 'unknown'))
        price_candidate = _prepare_shared_price_frame(rollout.step_df, controller_label=controller_label)
        load_candidate = _prepare_shared_agent_signal_frame(rollout.agent_df, 'load', controller_label=controller_label)
        pv_candidate = _prepare_shared_agent_signal_frame(rollout.agent_df, 'pv', controller_label=controller_label)
        price_reference, _ = _align_shared_actual_frames(price_reference, price_candidate, key_columns=['timestamp'], value_column=IMPORT_PRICE_COLUMN, controller_label=controller_label, signal_label='import_price')
        load_reference, _ = _align_shared_actual_frames(load_reference, load_candidate, key_columns=['timestamp', 'agent_profile'], value_column='load', controller_label=controller_label, signal_label='load')
        pv_reference, _ = _align_shared_actual_frames(pv_reference, pv_candidate, key_columns=['timestamp', 'agent_profile'], value_column='pv', controller_label=controller_label, signal_label='pv')
    ...
    price_prediction = _prepare_shared_price_frame(prediction_rollout.step_df, controller_label=prediction_label, require_predicted=True)
    load_prediction = _prepare_shared_agent_signal_frame(prediction_rollout.agent_df, 'load', controller_label=prediction_label, require_predicted=True)
    pv_prediction = _prepare_shared_agent_signal_frame(prediction_rollout.agent_df, 'pv', controller_label=prediction_label, require_predicted=True)
    price_reference, price_prediction = _align_shared_actual_frames(price_reference, price_prediction, key_columns=['timestamp'], value_column=IMPORT_PRICE_COLUMN, controller_label=prediction_label, signal_label='import_price')
    load_reference, load_prediction = _align_shared_actual_frames(load_reference, load_prediction, key_columns=['timestamp', 'agent_profile'], value_column='load', controller_label=prediction_label, signal_label='load')
    pv_reference, pv_prediction = _align_shared_actual_frames(pv_reference, pv_prediction, key_columns=['timestamp', 'agent_profile'], value_column='pv', controller_label=prediction_label, signal_label='pv')
    figure, axes = plt.subplots(3, 1, figsize=figsize or (20.0, 12.0), sharex=True)
    price_axis, load_axis, pv_axis = np.atleast_1d(axes)
    price_axis.plot(price_reference['timestamp'], price_reference[IMPORT_PRICE_COLUMN], color='#111827', linewidth=1.9, label='Actual import price')
    price_axis.plot(price_prediction['timestamp'], _resolve_adjusted_price_prediction(price_prediction, price_markup=float(prediction_rollout.meta.get(IMPORT_PRICE_MARKUP_KEY, 0.0))), color='#dc2626', linewidth=1.8, linestyle='--', label=f'Predicted import price ({prediction_label})')
    ...
    figure_handles = [Line2D([0], [0], color=COMPARE_PLOT_AGENT_PALETTE[idx % len(COMPARE_PLOT_AGENT_PALETTE)], linewidth=2.0, label=profile) for idx, profile in enumerate(profile_names)]
    figure_handles.extend([Line2D([0], [0], color='#111827', linewidth=2.0, linestyle='-', label='Actual'), Line2D([0], [0], color='#111827', linewidth=2.0, linestyle='--', label='Predicted')])
    for axis, signal_name, reference_frame, prediction_frame in ((load_axis, 'load', load_reference, load_prediction), (pv_axis, 'pv', pv_reference, pv_prediction)):
        for color_idx, profile in enumerate(profile_names):
            ...
            axis.plot(actual_frame['timestamp'], actual_frame[signal_name], color=color, linewidth=1.8)
            axis.plot(predicted_frame['timestamp'], predicted_frame[f'{signal_name}_pred'], color=color, linewidth=1.6, linestyle='--')
    ...
    figure.legend(handles=figure_handles, loc='upper center', ncol=min(len(figure_handles), 7), frameon=False, fontsize=legend_fontsize, bbox_to_anchor=(0.5, 1.02))
...
def plot_rollout_dashboard(rollout: RolloutResult, *, figsize: tuple[float, float] | None=None):
    ...
    for axis_idx, signal_name in enumerate(['pv', 'load'], start=1):
        ...
        for agent_idx, profile in enumerate(agent_profiles):
            ...
            axis.plot(agent_frame['timestamp'], agent_frame[signal_name], color=color, linewidth=1.4, label=f'{profile} actual')
            axis.plot(agent_frame['timestamp'], agent_frame[f'{signal_name}_pred'], color=color, linewidth=1.2, linestyle='--', label=f'{profile} forecast')
    ...
    for bus_id, frame in grid_df.loc[~grid_df['is_agent_bus']].groupby('bus_id'):
        voltage_axis.plot(frame['timestamp'], frame['vm_pu'], color=muted_color, linewidth=0.9, alpha=0.35, zorder=1)
    for color_idx, bus_id in enumerate(rollout.meta['agent_bus_ids']):
        frame = grid_df.loc[grid_df['bus_id'] == int(bus_id)]
        if frame.empty:
            continue
        voltage_axis.plot(frame['timestamp'], frame['vm_pu'], color=highlight_palette[color_idx % len(highlight_palette)], linewidth=2.1, alpha=0.95, label=f'Agent bus {bus_id}', zorder=3)
    voltage_axis.axhline(float(rollout.meta['v_min_pu']), color='#dc2626', linestyle='--', linewidth=1.1, label='V min')
    voltage_axis.axhline(float(rollout.meta['v_max_pu']), color='#ea580c', linestyle='--', linewidth=1.1, label='V max')
    ...
    net_load_axis.plot(step_df['timestamp'], step_df['base_net_load_total'], color='#111827', linewidth=1.6, label='Raw net load')
    if 'base_net_load_effective_total' in step_df.columns:
        net_load_axis.plot(step_df['timestamp'], step_df['base_net_load_effective_total'], color='#16a34a', linewidth=1.4, linestyle='-.', label='Post-curtail net load')
    net_load_axis.plot(step_df['timestamp'], step_df['net_load_total'], color='#2563eb', linewidth=1.5, linestyle='--', label='Post-action net load')
    ...
    if trafo_limit_kw is not None:
        ...
        if np.isfinite(trafo_limit_value) and trafo_limit_value > 0.0:
            net_load_axis.axhline(trafo_limit_value, color='#dc2626', linestyle=':', linewidth=1.2, label=f'Approx trafo +limit ({trafo_limit_value:.1f} kW)')
            net_load_axis.axhline(-trafo_limit_value, color='#dc2626', linestyle=':', linewidth=1.2, label=f'Approx trafo -limit ({trafo_limit_value:.1f} kW)')
    ...
    if 'pv_effective_total' in step_df.columns:
        pv_axis.plot(step_df['timestamp'], step_df['pv_raw_total'], color='#ea580c', linewidth=1.5, label='Raw PV')
        pv_axis.plot(step_df['timestamp'], step_df['pv_effective_total'], color='#16a34a', linewidth=1.5, linestyle='--', label='Effective PV')
        pv_axis.bar(step_df['timestamp'], step_df['pv_curtail_total'], width=0.008, color='#dc2626', alpha=0.35, label='Curtailment')
...
def plot_power_balance_bars(rollout: RolloutResult, *, figsize: tuple[float, float]=(18.0, 4.8)):
    step_df, _ = _prepare_agent_power_balance_frame(rollout.step_df, controller_label=str(rollout.meta.get('controller', 'unknown')))
    figure, axis = plt.subplots(1, 1, figsize=figsize)
    timestamps = step_df['timestamp']
    width = 0.008
    positive_specs = [('load_total', 'Load', '#111827'), ('battery_charge_total', 'Charge', '#dc2626'), ('grid_export_total', 'Grid export', '#f59e0b'), ('pv_curtail_total', 'Curtailment loss', '#fca5a5')]
    negative_specs = [('pv_raw_total', 'PV raw', '#16a34a'), ('grid_import_total', 'Grid import', '#2563eb'), ('battery_discharge_total', 'Discharge', '#7c3aed')]
    ...
    for column, label, color in positive_specs:
        ...
        axis.bar(timestamps, values, width=width, bottom=positive_bottom, color=color, alpha=0.78, label=label, **extra_kwargs)
...
def plot_test_voltage_profile(rollout: RolloutResult, *, figsize: tuple[float, float]=(18.0, 4.8)):
    ...
    for color_idx, bus_id in enumerate(rollout.meta['agent_bus_ids']):
        ...
        if frame.empty:
            continue
        axis.plot(frame['timestamp'], frame['vm_pu'], color=highlight_palette[color_idx % len(highlight_palette)], linewidth=2.1, alpha=0.95, label=f'Agent bus {bus_id}', zorder=3)
...
def plot_voltage_profile_comparison(*rollouts: RolloutResult, figsize: tuple[float, float] | None=None):
    if not rollouts:
        raise ValueError('At least one rollout is required.')
    n_rows = len(rollouts)
    figure, axes = plt.subplots(n_rows, 1, figsize=figsize or (20.0, max(4.0 * n_rows, 5.6)), sharex=True, sharey=True)
    axes = np.atleast_1d(axes)
    ...
    for axis_idx, (axis, rollout) in enumerate(zip(axes, rollouts, strict=False)):
        ...
        background_df = grid_df.loc[~grid_df['is_agent_bus'], ['timestamp', 'bus_id', 'vm_pu']].sort_values(['timestamp', 'bus_id'])
        if not background_df.empty:
            background_wide = background_df.pivot_table(index='timestamp', columns='bus_id', values='vm_pu', aggfunc='first').sort_index()
            if background_wide.shape[1] > 0:
                axis.plot(background_wide.index.to_numpy(), background_wide.to_numpy(dtype=np.float32), color=muted_color, linewidth=0.9, alpha=0.35, zorder=1)
        for color_idx, bus_id in enumerate(rollout.meta['agent_bus_ids']):
            frame = grid_df.loc[grid_df['bus_id'] == int(bus_id)]
            if frame.empty:
                continue
            axis.plot(frame['timestamp'], frame['vm_pu'], color=highlight_palette[color_idx % len(highlight_palette)], linewidth=2.1, alpha=0.95, label=f'Agent bus {bus_id}' if axis_idx == 0 else None, zorder=3)
        axis.axhline(float(rollout.meta['v_min_pu']), color='#dc2626', linestyle='--', linewidth=1.1, label='V min' if axis_idx == 0 else None)
        axis.axhline(float(rollout.meta['v_max_pu']), color='#ea580c', linestyle='--', linewidth=1.1, label='V max' if axis_idx == 0 else None)
...
def plot_price_prediction_comparison(*rollouts: RolloutResult, figsize: tuple[float, float]=(20.0, 5.0)):
    if not rollouts:
        raise ValueError('At least one rollout is required.')
    title_fontsize, label_fontsize, tick_fontsize, legend_fontsize = _compare_plot_fontsizes()
    reference_rollout = next((rollout for rollout in rollouts if str(rollout.meta.get('prediction_mode', '')) == NORMAL_PREDICTION_MODE), rollouts[0])
    ...
    figure, axis = plt.subplots(1, 1, figsize=figsize)
    axis.plot(reference_step_df['timestamp'], reference_step_df[IMPORT_PRICE_COLUMN], color='#111827', linewidth=1.8, label='Actual import price')
    ...
    for rollout in rollouts:
        ...
        if step_df.empty or WHOLESALE_PRICE_PRED_COLUMN not in step_df.columns:
            raise ValueError(f"Rollout '{rollout.meta.get('controller', 'unknown')}' is missing wholesale_price_pred for compare plotting.")
        ...
        for group in prediction_groups:
            ...
            if candidate_timestamps.shape == group_timestamps.shape and np.array_equal(candidate_timestamps, group_timestamps) and (candidate_pred.shape == group_pred.shape) and np.allclose(candidate_pred, group_pred, equal_nan=True):
                matched_group = group
                break
        if matched_group is None:
            prediction_groups.append({'timestamps': candidate_timestamps, 'pred': candidate_pred, 'controllers': [controller_name]})
        else:
            matched_group['controllers'].append(controller_name)
    if not prediction_groups:
        prediction_groups.append({'timestamps': pd.to_datetime(reference_step_df['timestamp']).to_numpy(), 'pred': reference_pred, 'controllers': [str(reference_rollout.meta.get('controller', 'unknown'))]})
    ...
    for group_idx, group in enumerate(prediction_groups):
        ...
        axis.plot(group['timestamps'], group['pred'], color=prediction_palette[group_idx % len(prediction_palette)], linewidth=1.8, linestyle='--', label=label)
...
def plot_net_load_comparison(*rollouts: RolloutResult, figsize: tuple[float, float] | None=None):
    if not rollouts:
        raise ValueError('At least one rollout is required.')
    n_rows = len(rollouts)
    figure, axes = plt.subplots(n_rows, 1, figsize=figsize or (20.0, max(4.2 * n_rows, 5.8)), sharex=True, sharey=True)
    axes = np.atleast_1d(axes)
    title_fontsize, label_fontsize, tick_fontsize, legend_fontsize = _compare_plot_fontsizes()
    for axis_idx, (feeder_axis, rollout) in enumerate(zip(axes, rollouts, strict=False)):
        step_df = _prepare_compare_net_load_frame(rollout.step_df, controller_label=str(rollout.meta.get('controller', 'unknown')))
        feeder_axis.plot(step_df['timestamp'], step_df['feeder_raw_net_load_kw'], color='#111827', linewidth=1.6, label='Feeder raw net load')
        feeder_axis.plot(step_df['timestamp'], step_df['feeder_effective_net_load_kw'], color='#16a34a', linewidth=1.4, linestyle='-.', label='Feeder post-curtail net load')
        feeder_axis.plot(step_df['timestamp'], step_df['feeder_post_action_net_load_kw'], color='#2563eb', linewidth=1.5, linestyle='--', label='Feeder post-action net load')
        if 'root_net_exchange_kw' in step_df.columns:
            feeder_axis.plot(step_df['timestamp'], step_df['root_net_exchange_kw'], color='#7c3aed', linewidth=1.7, label='Root net exchange')
        elif 'pp_root_p_kw' in step_df.columns:
            feeder_axis.plot(step_df['timestamp'], step_df['pp_root_p_kw'], color='#7c3aed', linewidth=1.7, label='Pandapower root exchange')
        trafo_limit_kw = rollout.meta.get('trafo_limit_kw')
        if trafo_limit_kw is not None:
            trafo_limit_value = float(trafo_limit_kw)
            if np.isfinite(trafo_limit_value) and trafo_limit_value > 0.0:
                feeder_axis.axhline(trafo_limit_value, color='#dc2626', linestyle=':', linewidth=1.2, label=f'Transformer S-limit ref (+P view) ({trafo_limit_value:.1f} kW)' if axis_idx == 0 else None)
                feeder_axis.axhline(-trafo_limit_value, color='#dc2626', linestyle=':', linewidth=1.2, label=f'Transformer S-limit ref (-P view) ({trafo_limit_value:.1f} kW)' if axis_idx == 0 else None)
...
def plot_power_balance_comparison(*rollouts: RolloutResult, figsize: tuple[float, float] | None=None):
    ...
    negative_specs = [('pv_raw_total', 'PV raw', '#16a34a'), ('grid_import_total', 'Grid import', '#2563eb'), ('battery_discharge_total', 'Discharge', '#7c3aed')]
    for axis_idx, (axis, rollout) in enumerate(zip(axes, rollouts, strict=False)):
        step_df, max_abs_balance_residual_kw = _prepare_agent_power_balance_frame(rollout.step_df, controller_label=str(rollout.meta.get('controller', 'unknown')))
        timestamps = step_df['timestamp'].to_numpy()
        positive_bottom = np.zeros(len(step_df), dtype=np.float32)
        for column, label, color in positive_specs:
            values = step_df[column].to_numpy(dtype=np.float32)
            extra_kwargs = {'hatch': '//', 'edgecolor': '#dc2626', 'linewidth': 1.0} if column == 'pv_curtail_total' else {}
            axis.bar(timestamps, values, width=0.008, bottom=positive_bottom, color=color, alpha=0.78, label=label if axis_idx == 0 else None, **extra_kwargs)
            positive_bottom = positive_bottom + values
        negative_bottom = np.zeros(len(step_df), dtype=np.float32)
        for column, label, color in negative_specs:
            values = step_df[column].to_numpy(dtype=np.float32)
            axis.bar(timestamps, -values, width=0.008, bottom=negative_bottom, color=color, alpha=0.78, label=label if axis_idx == 0 else None)
            negative_bottom = negative_bottom - values
        axis.axhline(0.0, color='#111827', linewidth=1.0)
        axis.set_ylabel('kW', fontsize=label_fontsize)
        axis.set_title(f"{rollout.meta['controller']} - agent-only balance (residual<={max_abs_balance_residual_kw:.3f} kW)", fontsize=title_fontsize)
        axis.grid(True, axis='y', alpha=0.25)
        axis.tick_params(axis='both', labelsize=tick_fontsize)
        if axis_idx == 0:
...
def plot_battery_power_and_soc_comparison(*rollouts: RolloutResult, figsize: tuple[float, float] | None=None):
    if not rollouts:
        raise ValueError('At least one rollout is required.')
    n_rows = len(rollouts)
    figure, axes = plt.subplots(n_rows, 1, figsize=figsize or (20.0, max(4.2 * n_rows, 6.5)), sharex=True, sharey=True)
    axes = np.atleast_1d(axes)
    title_fontsize, label_fontsize, tick_fontsize, legend_fontsize = _compare_plot_fontsizes()
    ...
    for rollout_idx, rollout in enumerate(rollouts):
        ...
        if not {'battery_charge_total', 'battery_discharge_total', 'timestamp'}.issubset(step_df.columns):
            raise ValueError(f"Rollout '{rollout.meta.get('controller', 'unknown')}' is missing battery power columns required for compare plotting.")
        step_df = step_df.copy()
        step_df['battery_net_power_kw'] = step_df['battery_discharge_total'].to_numpy(dtype=np.float32) - step_df['battery_charge_total'].to_numpy(dtype=np.float32)
        ...
        power_axis.bar(step_df['timestamp'], net_power, width=0.008, color=bar_colors, alpha=0.82)
        ...
        if agent_df.empty or 'soc' not in agent_df.columns:
            raise ValueError(f"Rollout '{rollout.meta.get('controller', 'unknown')}' is missing agent_df.soc required for compare plotting.")
        group_columns = [column for column in ['episode_idx', 'step', 'timestamp'] if column in agent_df.columns]
        if not group_columns:
            raise ValueError(f"Rollout '{rollout.meta.get('controller', 'unknown')}' is missing grouping columns required to aggregate SoC.")
        agent_id_column = 'agent_id' if 'agent_id' in agent_df.columns else None
        if agent_id_column is None:
            raise ValueError(f"Rollout '{rollout.meta.get('controller', 'unknown')}' is missing agent_df.agent_id required for compare plotting.")
        soc_summary = agent_df.groupby(group_columns, as_index=False).agg(mean=('soc', 'mean'))
        first_agent = True
        for _, agent_frame in agent_df.sort_values(group_columns + [agent_id_column]).groupby(agent_id_column, sort=True):
            soc_axis.plot(agent_frame['timestamp'], agent_frame['soc'], color=soc_agent_color, linewidth=1.1, alpha=0.45, label='Agent SoC' if first_agent and rollout_idx == 0 else None)
            first_agent = False
        soc_axis.plot(soc_summary['timestamp'], soc_summary['mean'], color=soc_mean_color, linewidth=1.7, label='Mean SoC' if rollout_idx == 0 else None)
        ...
        if rollout_idx == 0:
            legend_handles = [Patch(facecolor=discharge_color, alpha=0.82, label='Discharge (+)'), Patch(facecolor=charge_color, alpha=0.82, label='Charge (-)')]
            handles_2, labels_2 = soc_axis.get_legend_handles_labels()
            power_axis.legend(legend_handles + handles_2, ['Discharge (+)', 'Charge (-)'] + labels_2, loc='upper right', ncol=2, fontsize=legend_fontsize)
    axes[-1].set_xlabel('Timestamp', fontsize=label_fontsize)
    figure.tight_layout()
    return figure
...
def plot_rollout_comparison_dashboard(metrics_df: pd.DataFrame, *, figsize: tuple[float, float]=(16.0, 9.0)):
    ...
    metric_specs = [('purchase_cost_total', 'Purchase Cost'), ('export_subsidy_total', 'Export Subsidy'), ('objective_total', 'Objective Total'), ('voltage_violation_count', 'Voltage Violation Count'), ('trafo_penalty_total', 'Transformer Penalty'), ('line_penalty_total', 'Line Penalty')]
