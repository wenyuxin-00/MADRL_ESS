...
class AdmmMpcWindowData:import_price_eur_per_kwh:np.ndarray;load_seq:np.ndarray;pv_seq:np.ndarray;battery_capacity_kwh:np.ndarray;p_max_kw:np.ndarray;efficiency:float;energy_init_kwh:np.ndarray;energy_min_kwh:np.ndarray;energy_max_kwh:np.ndarray;export_subsidy_eur_per_kwh:float;dt_hours:float
@dataclass(frozen=True)
...
def build_admm_mpc_window_data(cfg,env,raw_obs)->AdmmMpcWindowData:
	...
	import_price=derive_import_price_seq(wholesale_price,markup_eur_per_kwh=float(get_import_price_markup(cfg)));export_subsidy=float(getattr(cfg.reward,'export_subsidy_eur_per_kwh',.079))
...
def _project_contribution_copies(values:np.ndarray,lower_bound_kw:np.ndarray,upper_bound_kw:np.ndarray)->np.ndarray:
	...
	if projected.shape[1]!=upper_bound.size:raise ValueError(f"values horizon {projected.shape[1]} must match upper_bound horizon {upper_bound.size}.")
	if not np.all(np.isfinite(lower_bound))or not np.all(np.isfinite(upper_bound)):raise ValueError('ADMM MPC transformer contribution bounds must be finite.')
	if np.any(upper_bound<lower_bound):raise ValueError('ADMM MPC transformer contribution upper_bound_kw must be greater than or equal to lower_bound_kw.')
	if projected.shape[0]<=0:raise ValueError('ADMM MPC transformer contribution projection requires at least one agent.')
	projected_sum=projected.sum(axis=0);clipped_sum=np.clip(projected_sum,lower_bound,upper_bound).astype(np.float32,copy=False);projected+=((clipped_sum-projected_sum)/np.float32(projected.shape[0])).astype(np.float32,copy=False)
...
class _ReusableAdmmLocalSolver:
	...
	def solve(self,*,window_data:AdmmMpcWindowData,alpha_kw:np.ndarray,z_kw:np.ndarray,u_kw:np.ndarray,rho:float)->tuple[np.ndarray,np.float32,np.float32]:
		...
		self._energy_init.RHS=float(window_data.energy_init_kwh[self.agent_idx]);objective=self.gp.QuadExpr()
		for step_idx in range(self.horizon):
			price_t=float(window_data.import_price_eur_per_kwh[step_idx]);self._pv_curtail_upper[step_idx].RHS=float(window_data.pv_seq[self.agent_idx,step_idx]);self._net_load_balance[step_idx].RHS=float(window_data.load_seq[self.agent_idx,step_idx]-window_data.pv_seq[self.agent_idx,step_idx]);objective+=float(window_data.dt_hours*price_t)*self.charge[step_idx];objective+=float(window_data.dt_hours*-price_t)*self.discharge[step_idx];objective+=float(window_data.dt_hours*_THROUGHPUT_TIEBREAKER_EUR_PER_KWH)*(self.charge[step_idx]+self.discharge[step_idx])
...
def run_admm_mpc_step(env,window_data:AdmmMpcWindowData,*,surrogate_cache:AdmmMpcSurrogateCache,rho_init:float,rho_min:float,rho_max:float,rho_adaptation:str|None,max_iters:int,max_iters_first_step:int,primal_tol:float,dual_tol:float)->AdmmMpcStepResult:
