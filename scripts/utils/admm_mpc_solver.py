...
def build_admm_mpc_surrogate_cache(cfg, env, *, horizon_steps: int) -> AdmmMpcSurrogateCache:
    ...
    if not isinstance(projector, JointGridSafetyProjector):
    ...
    if trafo_sensitivity.size == 0:
        alpha_window = np.zeros((int(projector.n_agents), int(horizon_steps)), dtype=np.float32)
    elif trafo_sensitivity.shape[0] != 1:
        raise ValueError(f"Notebook surrogate helpers expect exactly one transformer sensitivity row, got shape {trafo_sensitivity.shape}.")
    else:
        alpha_window = np.repeat(trafo_sensitivity[[0], : int(projector.n_agents)].T, int(horizon_steps), axis=1).astype(np.float32, copy=False)
...
def build_admm_mpc_window_data(cfg, env, raw_obs) -> AdmmMpcWindowData:
    wholesale_price = _as_float32(raw_obs[WHOLESALE_PRICE_SEQ_FIELD], name=WHOLESALE_PRICE_SEQ_FIELD, ndim=1).reshape(-1)
    load_seq = _as_float32(raw_obs["load_seq"], name="load_seq", ndim=2)
    pv_seq = _as_float32(raw_obs["pv_seq"], name="pv_seq", ndim=2)
    ...
    if horizon != wholesale_price.size:
        raise ValueError(f"load_seq horizon {horizon} must match {WHOLESALE_PRICE_SEQ_FIELD} horizon {wholesale_price.size}.")
    battery_capacity_kwh = _as_float32(env.agent_c_bat, name="agent_c_bat", ndim=1).reshape(-1)
    p_max_kw = _as_float32(env.agent_p_max, name="agent_p_max", ndim=1).reshape(-1)
    if battery_capacity_kwh.size != n_agents or p_max_kw.size != n_agents:
        raise ValueError(f"Battery arrays must match agent dimension {n_agents}, got {battery_capacity_kwh.size} and {p_max_kw.size}.")
    import_price = derive_import_price_seq(wholesale_price, markup_eur_per_kwh=float(get_import_price_markup(cfg)))
    export_subsidy = float(getattr(cfg.reward, "export_subsidy_eur_per_kwh", 0.079))
    min_gap = float(np.min(import_price - np.float32(export_subsidy)))
    if min_gap < -1e-09:
    ...
    if energy_init_kwh.size != n_agents:
        raise ValueError(f"env.soc agent dimension {energy_init_kwh.size} must match load_seq agents {n_agents}.")
    return AdmmMpcWindowData(
        ...
        energy_init_kwh=energy_init_kwh,
...
def resolve_default_terminal_cost_weight(window_data: AdmmMpcWindowData, *, multiplier: float = 1.0) -> np.ndarray:
    ...
    return np.asarray(float(multiplier) * 2.0 * mean_import_price / np.maximum(np.asarray(window_data.battery_capacity_kwh, dtype=np.float32), 1.0), dtype=np.float32)


def _project_contribution_copies(values: np.ndarray, lower_bound_kw: np.ndarray) -> np.ndarray:
    projected = _as_float32(values, name="values", ndim=2).copy()
    lower_bound = _as_float32(lower_bound_kw, name="lower_bound_kw", ndim=1).reshape(-1)
    if projected.shape[1] != lower_bound.size:
        raise ValueError(f"values horizon {projected.shape[1]} must match lower_bound horizon {lower_bound.size}.")
    if projected.shape[0] > 0:
        projected += np.maximum(lower_bound - projected.sum(axis=0), 0.0).astype(np.float32, copy=False) / projected.shape[0]
...
class _ReusableAdmmLocalSolver:
    def __init__(self, *, agent_idx: int, horizon: int, p_max_kw: float, dt_hours: float, efficiency: float, energy_min_kwh: float, energy_max_kwh: float) -> None:
        ...
        self.gp = gp
        self.grb = gp.GRB
        self.model = gp.Model(f"admm_mpc_local_agent_{self.agent_idx}")
        ...
        self.n_pos = self.model.addVars(self.horizon, lb=0.0, name="n_pos_kw")
        self.energy = self.model.addVars(self.horizon + 1, lb=float(energy_min_kwh), ub=float(energy_max_kwh), name="energy_kwh")
        self._energy_init = self.model.addConstr(self.energy[0] == 0.0, name="energy_init")
        self._pv_curtail_upper = []
        self._net_load_balance = []
        dt = float(dt_hours)
        eff = max(float(efficiency), 1e-06)
        for step_idx in range(self.horizon):
            self._pv_curtail_upper.append(self.model.addConstr(self.pv_curtail[step_idx] <= 0.0, name=f"curtail_ub_{step_idx}"))
            self._net_load_balance.append(self.model.addConstr(self.net_load[step_idx] - self.pv_curtail[step_idx] - self.charge[step_idx] + self.discharge[step_idx] == 0.0, name=f"net_load_balance_{step_idx}"))
            self.model.addConstr(self.n_pos[step_idx] >= self.net_load[step_idx], name=f"n_pos_lb_{step_idx}")
    ...
    def solve(self, *, window_data: AdmmMpcWindowData, alpha_kw: np.ndarray, z_kw: np.ndarray, u_kw: np.ndarray, rho: float, terminal_ref_kwh: float, terminal_cost_weight: float) -> tuple[np.ndarray, np.float32, np.float32]:
        alpha = _as_float32(alpha_kw, name="alpha_kw", ndim=1).reshape(-1)
        z = _as_float32(z_kw, name="z_kw", ndim=1).reshape(-1)
        u = _as_float32(u_kw, name="u_kw", ndim=1).reshape(-1)
        if alpha.size != self.horizon or z.size != self.horizon or u.size != self.horizon:
            raise ValueError(f"alpha/z/u must have horizon {self.horizon}, got {alpha.size}, {z.size}, {u.size}.")
        self._energy_init.RHS = float(window_data.energy_init_kwh[self.agent_idx])
        import_minus_subsidy = np.maximum(np.asarray(window_data.import_price_eur_per_kwh, dtype=np.float32) - float(window_data.export_subsidy_eur_per_kwh), 0.0)
        objective = self.gp.QuadExpr()
        for step_idx in range(self.horizon):
            self._pv_curtail_upper[step_idx].RHS = float(window_data.pv_seq[self.agent_idx, step_idx])
            self._net_load_balance[step_idx].RHS = float(window_data.load_seq[self.agent_idx, step_idx] - window_data.pv_seq[self.agent_idx, step_idx])
            objective += float(window_data.dt_hours * window_data.export_subsidy_eur_per_kwh) * self.net_load[step_idx]
            objective += float(window_data.dt_hours * import_minus_subsidy[step_idx]) * self.n_pos[step_idx]
        ...
        if float(terminal_cost_weight) > 0.0:
            objective += float(terminal_cost_weight) * (self.energy[self.horizon] * self.energy[self.horizon] - 2.0 * float(terminal_ref_kwh) * self.energy[self.horizon])
        ...
        if int(self.model.Status) not in {int(self.grb.OPTIMAL), int(self.grb.SUBOPTIMAL)}:
            raise RuntimeError(f"ADMM MPC local model for agent {self.agent_idx} failed with status={int(self.model.Status)}.")
        net_load_kw = np.asarray([self.net_load[t].X for t in range(self.horizon)], dtype=np.float32)
        return (
            (alpha * net_load_kw).astype(np.float32, copy=False),
            np.float32(float(self.charge[0].X) - float(self.discharge[0].X)),
            np.float32(float(self.pv_curtail[0].X)),
        )
...
def _first_step_action(env, *, battery_power_kw: np.ndarray, pv_curtail_kw: np.ndarray, pv_step_kw: np.ndarray) -> np.ndarray:
    ...
    safety_local = build_safety_local_numpy(soc=np.asarray(env.soc, dtype=np.float32), load_raw=np.zeros_like(requested_battery_power_kw), pv_raw=np.zeros_like(requested_battery_power_kw), battery_capacity_kwh=np.asarray(env.agent_c_bat, dtype=np.float32), p_max_kw=np.asarray(env.agent_p_max, dtype=np.float32))
    battery_lower, battery_upper, p_max_kw, _ = _local_bounds_numpy(safety_local, efficiency=float(env.eff), dt_hours=float(env.dt), soc_min=float(env.soc_min), soc_max=float(env.soc_max))
    battery_action = np.clip(np.clip(requested_battery_power_kw, battery_lower, battery_upper) / np.maximum(p_max_kw, 1e-06), -1.0, 1.0).astype(np.float32, copy=False)
    pv_raw_kw = np.maximum(np.asarray(pv_step_kw, dtype=np.float32).reshape(-1), 0.0)
    pv_effective_kw = np.maximum(pv_raw_kw - np.asarray(pv_curtail_kw, dtype=np.float32).reshape(-1), 0.0).astype(np.float32, copy=False)
...
def run_admm_mpc_step(env, window_data: AdmmMpcWindowData, *, surrogate_cache: AdmmMpcSurrogateCache, rho_init: float, rho_min: float, rho_max: float, rho_adaptation: str | None, max_iters: int, max_iters_first_step: int, primal_tol: float, dual_tol: float, terminal_cost_weight_eur_per_kwh2: np.ndarray) -> AdmmMpcStepResult:
    ...
    alpha_window = _as_float32(surrogate_cache.alpha_netload_window_kw, name="alpha_netload_window_kw", ndim=2)
    if alpha_window.shape != (n_agents, horizon):
        raise ValueError(f"alpha_netload_window_kw shape {alpha_window.shape} must match {(n_agents, horizon)}.")
    terminal_weight = _as_float32(terminal_cost_weight_eur_per_kwh2, name="terminal_cost_weight_eur_per_kwh2", ndim=1).reshape(-1)
    if terminal_weight.size != n_agents:
        raise ValueError(f"terminal_cost_weight_eur_per_kwh2 must provide {n_agents} values, got {terminal_weight.size}.")
    lower_bound = np.full((horizon,), -float(surrogate_cache.trafo_limit_kw) - float(surrogate_cache.trafo_base_kw), dtype=np.float32)
    baseline_contrib = (alpha_window * (np.asarray(window_data.load_seq, dtype=np.float32) - np.asarray(window_data.pv_seq, dtype=np.float32))).astype(np.float32, copy=False)
    z_kw = _project_contribution_copies(baseline_contrib, lower_bound)
    ...
    final_battery_power = final_pv_curtail = None
    ...
    try:
        for iteration in range(1, iteration_budget + 1):
            local_results = [solver.solve(window_data=window_data, alpha_kw=alpha_window[agent_idx], z_kw=z_kw[agent_idx], u_kw=u_kw[agent_idx], rho=float(rho), terminal_ref_kwh=float(window_data.energy_ref_kwh[agent_idx]), terminal_cost_weight=float(terminal_weight[agent_idx])) for agent_idx, solver in enumerate(local_solvers)]
            ...
            z_next = _project_contribution_copies(contribution_kw + u_kw, lower_bound)
            u_kw = (u_kw + contribution_kw - z_next).astype(np.float32, copy=False)
            ...
            if use_residual_balancing and rho > 0.0:
                ...
                if not np.isclose(rho, rho_before):
                    u_kw = (u_kw * np.float32(rho_before / max(rho, 1e-09))).astype(np.float32, copy=False)
            z_kw = z_next
    finally:
        for solver in local_solvers:
            solver.dispose()
    if final_battery_power is None or final_pv_curtail is None:
