...
_CFG_FLOAT_RTOL = 1e-06
_CFG_FLOAT_ATOL = 1e-06
...
def _normalize_battery_controls_from_cfg(cfg: Any) -> dict[str, Any]:
    ...
    return {'battery_capacity': [float(value) for value in capacity_values], 'max_charge_rate': float(getattr(getattr(cfg, 'env', None), 'max_charge_rate', np.nan)), 'efficiency': float(getattr(getattr(cfg, 'env', None), 'efficiency', np.nan)), 'init_soc': float(getattr(getattr(cfg, 'env', None), 'init_soc', np.nan)), 'soc_min': float(getattr(getattr(cfg, 'env', None), 'soc_min', np.nan)), 'soc_max': float(getattr(getattr(cfg, 'env', None), 'soc_max', np.nan)), 'soc_target': float(getattr(getattr(cfg, 'env', None), 'soc_target', np.nan))}
...
def build_rollout_cfg_snapshot_from_cfg(cfg: Any, *, prediction_mode: str | None=None) -> dict[str, Any]:
    ...
    resolved_prediction_mode = grid_nb.normalize_prediction_mode(prediction_mode) if prediction_mode is not None else grid_nb.resolve_prediction_mode_from_forecast_backend(forecast_backend)
    return {'test_start_date': str(getattr(getattr(cfg, 'data', None), 'test_start_date', '')), 'test_end_date': str(getattr(getattr(cfg, 'data', None), 'test_end_date', '')), 'prediction_mode': str(resolved_prediction_mode), 'forecast_backend': forecast_backend, 'agent_profiles': [str(value) for value in list(getattr(getattr(cfg, 'data', None), 'agent_profiles', []))], 'agent_bus_ids': [int(value) for value in list(getattr(getattr(cfg, 'grid', None), 'agent_bus_ids', []))], 'load_scale': [float(value) for value in list(getattr(getattr(cfg, 'data', None), 'load_scale', []))], 'pv_scale': [float(value) for value in list(getattr(getattr(cfg, 'data', None), 'pv_scale', []))], 'battery_controls': _normalize_battery_controls_from_cfg(cfg), 'future_horizon': int(getattr(getattr(cfg, 'env', None), 'future_horizon', 0)), 'episode_limit': int(getattr(getattr(cfg, 'env', None), 'episode_limit', 0)), 'v_min_pu': float(getattr(getattr(cfg, 'grid', None), 'v_min_pu', np.nan)), 'v_max_pu': float(getattr(getattr(cfg, 'grid', None), 'v_max_pu', np.nan)), 'price_protocol_version': int(PRICE_PROTOCOL_VERSION), IMPORT_PRICE_MARKUP_KEY: float(getattr(getattr(cfg, 'reward', None), IMPORT_PRICE_MARKUP_KEY, 0.0)), 'export_subsidy_eur_per_kwh': float(getattr(getattr(cfg, 'reward', None), 'export_subsidy_eur_per_kwh', np.nan))}
...
def assert_rollout_cfg_snapshot_matches(expected_snapshot: dict[str, Any], actual_snapshot: dict[str, Any], *, mismatch_prefix: str='Rollout package') -> None:
    ...
    _assert_float_sequence_match(mismatch_prefix, 'load_scale', expected_snapshot['load_scale'], actual_snapshot['load_scale'])
    _assert_float_sequence_match(mismatch_prefix, 'pv_scale', expected_snapshot['pv_scale'], actual_snapshot['pv_scale'])
    ...
    for field_name in ('max_charge_rate', 'efficiency', 'init_soc', 'soc_min', 'soc_max', 'soc_target'):
        _assert_float_match(mismatch_prefix, f'battery_controls.{field_name}', float(expected_snapshot['battery_controls'][field_name]), float(actual_snapshot['battery_controls'][field_name]))
...
def save_rollout_package(package: dict[str, Any], target_dir: str | Path) -> Path:
    ...
    manifest['tables'] = {'step_df': save_dataframe_npz(pd.DataFrame(package['step_df']).copy(), target_path / 'step_df.npz'), 'agent_df': save_dataframe_npz(pd.DataFrame(package['agent_df']).copy(), target_path / 'agent_df.npz'), 'grid_df': save_dataframe_npz(pd.DataFrame(package['grid_df']).copy(), target_path / 'grid_df.npz'), 'summary_df': save_dataframe_npz(pd.DataFrame(package['summary_df']).copy(), target_path / 'summary_df.npz')}
...
def load_rollout_package(target_dir: str | Path, *, package_label: str, version: int, regenerate_hint: str) -> dict[str, Any]:
    target_path = Path(target_dir).resolve()
    required_paths = {'manifest': target_path / 'manifest.json', 'diagnostics': target_path / 'diagnostics.json', 'step_df': target_path / 'step_df.npz', 'agent_df': target_path / 'agent_df.npz', 'grid_df': target_path / 'grid_df.npz', 'summary_df': target_path / 'summary_df.npz'}
    missing_files = [path.name for path in required_paths.values() if not path.exists()]
    if missing_files:
        raise FileNotFoundError(f"{package_label} is incomplete at {target_path}: missing {', '.join(missing_files)}.")
    manifest = json.loads(required_paths['manifest'].read_text(encoding='utf-8'))
    actual_version = int(manifest.get('rollout_package_version', -1))
    ...
    return {'manifest': manifest, 'diagnostics': json.loads(required_paths['diagnostics'].read_text(encoding='utf-8')), 'step_df': load_dataframe_npz(required_paths['step_df'], metadata=tables['step_df']), 'agent_df': load_dataframe_npz(required_paths['agent_df'], metadata=tables['agent_df']), 'grid_df': load_dataframe_npz(required_paths['grid_df'], metadata=tables['grid_df']), 'summary_df': load_dataframe_npz(required_paths['summary_df'], metadata=tables['summary_df'])}
...
def _serialize_dataframe_for_npz(frame: pd.DataFrame) -> tuple[dict[str, Any], dict[str, np.ndarray]]:
    ...
    for column_index, column_name in enumerate(table.columns):
        ...
        arrays[key] = values
        schema.append({'name': str(column_name), 'dtype': str(series.dtype), 'storage': str(storage), 'key': key})
