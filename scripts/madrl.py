...
from models.assembly import Actor, Critic, build_actors, build_critics, to_torch_obs
...
def _signed_reward_component(info: dict[str, Any], key: str, sign: float) -> float:
    return float(sign * np.sum(np.asarray(info.get(key, 0.0), dtype=np.float32)))
...
def _plot_reward_curves(rewards: pd.DataFrame, run_dir: str | Path, prefix: str) -> Any:
    ...
    component_columns = [key for key in REWARD_COMPONENT_COLUMNS if key in rewards.columns]
    if component_columns:
        fig, axes = plt.subplots(2, 1, figsize=(8, 5.4), sharex=True, gridspec_kw={"height_ratios": [1.2, 1.0]})
    else:
        fig, ax = plt.subplots(figsize=(8, 3.2)); axes = [ax]
    ...
    if component_columns:
        ...
        axes[0].set_title("Episode total reward; component columns missing; retrain required for component curves")
...
def plot_madrl_outputs(result: dict[str, Any], run_dir: str | Path) -> dict[str, Any]:
    ...
    if not rewards.empty:
        figures[f"{prefix}_learning_curve"] = _plot_reward_curves(rewards, run_dir, prefix)
    return figures
