...
def build_training_contract(cfg)->dict:
	...
	from predictors.shared_data import SHARED_DATA_SCHEMA_VERSION
...
def validate_checkpoint_training_contract(manifest:dict,*,expected_signature:str|None=None,entrypoint:str='scripts.checkpoints.resolve_checkpoint_to_load')->dict:
	...
	if removed_reward_keys:raise ValueError(f"{entrypoint} received a checkpoint training contract from the SoC-aware mapping experiment containing removed field(s) {removed_reward_keys}. Expected action_boundary_penalty_weight and no action_mapping_contract under the restored MADRL baseline contract. Re-run notebooks/madrl/train_base.ipynb.")
	required_fields=('reward_contract','rollout_soc_contract','train_window_days','window_stride_days','train_episode_limit','test_episode_limit','shared_data_schema_version','shared_data_signature','observation_feature_set','observation_normalization_signature','storage_objective_mode','storage_price_mode','storage_profit_weight','action_boundary_penalty_weight','soc_boundary_regularization_weight','throughput_bonus_eur_per_kwh_max','soc_boundary_epsilon','soc_boundary_margin','export_subsidy_eur_per_kwh','import_price_markup_eur_per_kwh','w_voltage_pen','w_line_pen','w_trafo_pen','train_init_soc_low','train_init_soc_high')
	missing_fields=[field_name for field_name in required_fields if field_name not in training_contract]
	if missing_fields:raise ValueError(f"{entrypoint} received an incomplete checkpoint training contract missing {missing_fields}. New contract expects shared-data, observation-normalization, Step 3 reward, and continuous-SoC fields. Re-run notebooks/madrl/train_base.ipynb.")
