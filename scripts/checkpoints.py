...
def build_training_run_label(
    *,
    ...
    budget_token = build_checkpoint_budget_token(
...
def build_training_run_paths(
    ...
    *,
    ...
    prediction_token = slugify_checkpoint_token(prediction_mode, default="perfect")
    experiment_token = slugify_checkpoint_token(experiment_name, default="grid_mainline")
    run_label = build_training_run_label(
        algorithm=algorithm_token,
        prediction_mode=prediction_token,
...
def find_latest_training_run(
    ...
    *,
    ...
    base_dir = (
        checkpoint_root
        / str(algorithm).strip()
        / slugify_checkpoint_token(prediction_mode, default="perfect")
        / slugify_checkpoint_token(experiment_name, default="grid_mainline")
    )
    ...
    candidates = sorted(
        (path for path in base_dir.iterdir() if path.is_dir() and path.name != "_meta"),
        key=_run_sort_key,
    )
    if not candidates:
