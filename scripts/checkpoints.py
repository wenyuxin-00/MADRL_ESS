...
def build_checkpoint_manifest(*,algorithm:str,saved_episode_tag:int,episodes_completed:int,total_steps:int,num_envs:int,episode_limit:int,save_dir)->dict:return{'algorithm':algorithm,'saved_episode_tag':int(saved_episode_tag),'episodes_completed':int(episodes_completed),'total_steps':int(total_steps),'num_envs':int(num_envs),'episode_limit':int(episode_limit),'save_dir':str(Path(save_dir).resolve())}
...
def resolve_checkpoint_to_load(model_dir,algorithm:str,episode_tag:int|None=None)->dict:
	algo_dir=get_algorithm_checkpoint_dir(model_dir,algorithm);manifest=None
	if episode_tag is None:
		try:
			manifest=load_latest_checkpoint_manifest(model_dir,algorithm);candidate_tag=int(manifest['saved_episode_tag'])
			if checkpoint_tag_exists(algo_dir,candidate_tag):episode_tag=candidate_tag
		except FileNotFoundError:manifest=None
	if episode_tag is None:episode_tag=infer_latest_checkpoint_tag(model_dir,algorithm)
	...
	if manifest is not None:resolved.update({'episodes_completed':int(manifest.get('episodes_completed',episode_tag)),'total_steps':int(manifest.get('total_steps',0)),'num_envs':int(manifest.get('num_envs',0)),'episode_limit':int(manifest.get('episode_limit',0)),'save_dir':manifest.get('save_dir',str(algo_dir))})
	return resolved
