...
def build_safety_local_numpy(
    *,
    ...
    return np.column_stack(
        [
            np.asarray(soc, dtype=np.float32),
            np.asarray(load_raw, dtype=np.float32),
            np.asarray(pv_raw, dtype=np.float32),
            np.asarray(battery_capacity_kwh, dtype=np.float32),
...
def _canonicalize_actions_numpy(
    ...
    *,
    default_pv_action: float = 1.0,
    require_full_action_dim: bool = False,
...
def _canonicalize_actions_torch(
    ...
    *,
    ...
    if actions.shape[-1] == 1 and not require_full_action_dim:
        pv_column = torch.full(
            (*actions.shape[:-1], 1),
            float(default_pv_action),
            dtype=actions.dtype,
...
def _local_bounds_numpy(
    ...
    *,
    ...
    charge_limit_kw = np.minimum(
        p_max_kw,
        np.maximum(0.0, (energy_max - energy_now) / (eff * dt)),
    )
    discharge_limit_kw = np.minimum(
        p_max_kw,
...
def compute_action_gap_metrics_numpy(
    safety_local: np.ndarray,
    requested_actions: np.ndarray | list[np.ndarray],
    executed_actions: np.ndarray | list[np.ndarray],
) -> dict[str, np.ndarray]:
    ...
    if safety_local.shape[0] != requested.shape[0]:
        raise ValueError(
            "safety_local and action arrays should share the same agent dimension, "
            f"got {safety_local.shape} vs {requested.shape}."
        )
    ...
    metrics = {
        ...
        "action_gap": np.abs(executed - requested).astype(np.float32),
        "controller_action_gap": np.sum(np.abs(executed - requested), axis=-1).astype(np.float32),
        "battery_action_req": battery_action_req,
        "battery_action_exec": battery_action_exec,
        "pv_action_req": pv_action_req,
        "pv_action_exec": pv_action_exec,
        "battery_power_req_kw": battery_power_req_kw,
        "battery_power_exec_kw": battery_power_exec_kw,
        "pv_effective_req_kw": pv_effective_req_kw,
...
def compute_action_gap_metrics_torch(
    ...
    if int(safety_local.shape[0]) != int(requested.shape[0]):
        raise ValueError(
            "safety_local batch dimension should match requested_actions batch dimension, "
            f"got {tuple(safety_local.shape)} vs {tuple(requested.shape)}."
        )
    ...
    metrics = {
        ...
        "action_gap": (executed - requested).abs(),
        "controller_action_gap": (executed - requested).abs().sum(dim=-1),
        "battery_action_req": battery_action_req,
        "battery_action_exec": battery_action_exec,
        "pv_action_req": pv_action_req,
        "pv_action_exec": pv_action_exec,
...
def enforce_local_action_feasibility_torch(
    ...
    *,
    ...
    battery_lower_kw, battery_upper_kw, p_max_kw, _ = _local_bounds_torch(
        safety_local,
...
def validate_executed_actions_numpy(
    ...
    *,
    ...
    if np.any(action_array < -1.0 - _ACTION_TOL) or np.any(action_array > 1.0 + _ACTION_TOL):
        raise ValueError(
            "Environment received action values outside [-1, 1]. "
            "Controllers must output already-feasible normalized actions."
        )

    battery_lower_kw, battery_upper_kw, p_max_kw, pv_raw_kw = _local_bounds_numpy(
        safety_local,
        efficiency=efficiency,
        dt_hours=dt_hours,
        soc_min=soc_min,
    ...
    invalid_battery = np.logical_or(
        battery_power_kw < (battery_lower_kw - _ACTION_TOL),
        battery_power_kw > (battery_upper_kw + _ACTION_TOL),
    )
    if np.any(invalid_battery):
        bad_agent = int(np.nonzero(invalid_battery)[0][0])
        raise ValueError(
            "Environment received a locally infeasible battery action "
            f"for agent {bad_agent}: requested {battery_power_kw[bad_agent]:.4f} kW, "
            f"allowed range [{battery_lower_kw[bad_agent]:.4f}, {battery_upper_kw[bad_agent]:.4f}] kW."
        )

    pv_utilization = 0.5 * (action_array[:, 1] + 1.0)
    pv_effective_kw = pv_utilization * pv_raw_kw
    invalid_pv = np.logical_or(
        pv_effective_kw < -_ACTION_TOL,
        pv_effective_kw > (pv_raw_kw + _ACTION_TOL),
    )
    if np.any(invalid_pv):
        bad_agent = int(np.nonzero(invalid_pv)[0][0])
        raise ValueError(
            "Environment received a locally infeasible PV action "
