...
class MADRLController:
	...
	def _format_action(action:object)->np.ndarray:
		...
		if action_array.ndim==0:return action_array.reshape(1)
		return action_array
	def _postprocess_joint_actions(self,obs:dict,actions:list[np.ndarray])->tuple[list[np.ndarray],dict[str,np.ndarray]|None]:
		if not actions:return actions,None
		has_batch_dim=np.asarray(obs['local']).ndim==3;obs_batch=obs if has_batch_dim else add_batch_dim(obs)
		if has_batch_dim:action_batch=np.stack(actions,axis=1).astype(np.float32)
		else:action_batch=np.stack(actions,axis=0).astype(np.float32)[None,...]
		obs_t=to_torch_nested(obs_batch,self.device);action_t=torch.as_tensor(action_batch,device=self.device,dtype=torch.float32)
		...
		if action_info_np is not None:single_env_info={key:np.asarray(value[0],dtype=np.float32)for(key,value)in action_info_np.items()}
		return[executed_np[0,agent_id].copy()for agent_id in range(executed_np.shape[1])],single_env_info
	def act(self,obs:dict,deterministic:bool=True)->list[np.ndarray]:noise_std=.0 if deterministic else self.noise_std;raw_actions=[self._format_action(agent.choose_action(obs,noise_std=noise_std))for agent in self.agent_n];actions,action_info=self._postprocess_joint_actions(obs,raw_actions);self.last_action_info=action_info;return actions
