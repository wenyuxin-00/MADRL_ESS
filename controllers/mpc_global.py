...
class GlobalMisocpSolver:
    ...
    def solve(self, *, import_price_seq: np.ndarray, load_seq: np.ndarray, pv_seq: np.ndarray, soc_init: np.ndarray) -> MisocpSolveResult:
        ...
        result = MisocpSolveResult(battery, curtailed, float(perf_counter() - started), float(model.ObjVal), int(model.Status))
        model.dispose()
        return result
...
class MisocpController:
    ...
    def __call__(self, env, window: dict[str, np.ndarray]):
        if int(env.cur_step) == 0:
            ...
            self.plan_start_step = int(env.cur_step); self.battery_power_kw = result.battery_power_kw; self.pv_curtail_kw = result.pv_curtail_kw; self.solve_time_sec = float(result.solve_time_sec)
