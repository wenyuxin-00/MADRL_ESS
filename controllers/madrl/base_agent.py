...
class MADDPG(BaseAgent):
    ...
    def train_on_batch(self, batch: dict, agent_n: list, shared_ctx: dict | None = None) -> None:
        ...
        self._optimizer_step(self.critic_optimizer, critic_loss, self.critic.parameters())
        new_action = action.clone()
        new_action[:, self.agent_id] = self._actor_call(obs)
        actor_loss = -self._critic_call(obs, new_action).float().mean()
        self._optimizer_step(self.actor_optimizer, actor_loss, self.actor.parameters())
        self._soft_update()
...
class MATD3(BaseAgent):
    ...
    def train_on_batch(self, batch: dict, agent_n: list, shared_ctx: dict | None = None) -> None:
        ...
        with torch.no_grad():
            ...
            target_q = reward[:, self.agent_id] + self.gamma * (1 - done[:, self.agent_id]) * torch.min(q1_next, q2_next)
...
class MATD3SafePOC(MATD3):
    ...
    def _get_projected_target_actions(self, next_obs: dict, agent_n: list, shared_ctx: dict | None) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            projected = self.safety_projector.project_actions_from_safety_local(next_obs["safety_local"], next_action)
    ...
    def train_on_batch(self, batch: dict, agent_n: list, shared_ctx: dict | None = None) -> None:
        ...
        actor_loss.backward(retain_graph=self.agent_id < (len(agent_n) - 1))
