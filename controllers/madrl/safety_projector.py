...
def _canonicalize_actions_numpy(actions: np.ndarray | list[np.ndarray], *, default_pv_action: float=1.0, require_full_action_dim: bool=False) -> tuple[np.ndarray, bool]:
    ...
    if action_array.shape[-1] == 1 and not require_full_action_dim:
        action_array = np.concatenate([action_array, np.full((action_array.shape[0], 1), float(default_pv_action), dtype=np.float32)], axis=-1)
...
def _canonicalize_actions_torch(actions: torch.Tensor, *, default_pv_action: float=1.0, require_full_action_dim: bool=False) -> tuple[torch.Tensor, bool]:
    ...
    if actions.shape[-1] == 1 and not require_full_action_dim:
        actions = torch.cat([actions, torch.full((*actions.shape[:-1], 1), float(default_pv_action), dtype=actions.dtype, device=actions.device)], dim=-1)
...
def _local_bounds_numpy(safety_local: np.ndarray, *, efficiency: float, dt_hours: float, soc_min: float, soc_max: float) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    capacity_kwh = np.maximum(np.asarray(safety_local[..., 3], dtype=np.float32), _SAFETY_EPS)
    p_max_kw = np.maximum(np.asarray(safety_local[..., 4], dtype=np.float32), _SAFETY_EPS)
    pv_raw_kw = np.maximum(np.asarray(safety_local[..., 2], dtype=np.float32), 0.0)
    energy_now = soc * capacity_kwh
    eff = max(float(efficiency), _SAFETY_EPS)
    dt = max(float(dt_hours), _SAFETY_EPS)
...
def compute_action_gap_metrics_numpy(safety_local: np.ndarray, requested_actions: np.ndarray | list[np.ndarray], executed_actions: np.ndarray | list[np.ndarray]) -> dict[str, np.ndarray]:
    ...
    p_max_kw = np.maximum(safety_local[:, 4], _SAFETY_EPS)
    pv_raw_kw = np.maximum(safety_local[:, 2], 0.0)
    battery_action_req, battery_action_exec = requested[:, 0].astype(np.float32), executed[:, 0].astype(np.float32)
    ...
    pv_effective_req_kw = (0.5 * (requested[:, 1].astype(np.float32) + 1.0) * pv_raw_kw).astype(np.float32)
    pv_effective_exec_kw = (0.5 * (pv_action_exec + 1.0) * pv_raw_kw).astype(np.float32)
    metrics = {'battery_power_req_kw': battery_power_req_kw, 'battery_power_exec_kw': battery_power_exec_kw, 'pv_action_exec': pv_action_exec, 'pv_effective_req_kw': pv_effective_req_kw, 'pv_effective_exec_kw': pv_effective_exec_kw, 'pv_curtail_req_kw': (pv_raw_kw - pv_effective_req_kw).astype(np.float32), 'pv_curtail_exec_kw': (pv_raw_kw - pv_effective_exec_kw).astype(np.float32), 'soc_penalty_unweighted': (np.abs(battery_power_req_kw - battery_power_exec_kw) / (p_max_kw + _SAFETY_EPS)).astype(np.float32)}
    metrics['action_penalty_unweighted'] = metrics['soc_penalty_unweighted']
    return {key: np.asarray(value[0], dtype=np.float32) for key, value in metrics.items()} if squeezed else metrics
...
def compute_action_gap_metrics_torch(safety_local: torch.Tensor, requested_actions: torch.Tensor, executed_actions: torch.Tensor) -> dict[str, torch.Tensor]:
    ...
    p_max_kw = torch.clamp(safety_local[..., 4], min=_SAFETY_EPS)
    pv_raw_kw = torch.clamp(safety_local[..., 2], min=0.0)
    battery_power_req_kw = requested[..., 0] * p_max_kw
    battery_power_exec_kw = executed[..., 0] * p_max_kw
    pv_action_exec = executed[..., 1]
    pv_effective_req_kw = 0.5 * (requested[..., 1] + 1.0) * pv_raw_kw
    pv_effective_exec_kw = 0.5 * (pv_action_exec + 1.0) * pv_raw_kw
    metrics = {'battery_power_req_kw': battery_power_req_kw, 'battery_power_exec_kw': battery_power_exec_kw, 'pv_action_exec': pv_action_exec, 'pv_effective_req_kw': pv_effective_req_kw, 'pv_effective_exec_kw': pv_effective_exec_kw, 'pv_curtail_req_kw': pv_raw_kw - pv_effective_req_kw, 'pv_curtail_exec_kw': pv_raw_kw - pv_effective_exec_kw, 'soc_penalty_unweighted': (battery_power_req_kw - battery_power_exec_kw).abs() / torch.clamp(p_max_kw, min=_SAFETY_EPS)}
    metrics['action_penalty_unweighted'] = metrics['soc_penalty_unweighted']
    return {key: value.squeeze(0) for key, value in metrics.items()} if squeezed else metrics
...
def enforce_local_action_feasibility_torch(safety_local: torch.Tensor, raw_actions: torch.Tensor, *, efficiency: float, dt_hours: float, soc_min: float, soc_max: float) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
    ...
    executed_actions = torch.stack([torch.clamp(requested_actions[..., 0] * p_max_kw, min=battery_lower_kw, max=battery_upper_kw) / torch.clamp(p_max_kw, min=_SAFETY_EPS), torch.clamp(requested_actions[..., 1], min=-1.0, max=1.0)], dim=-1)
    metrics = compute_action_gap_metrics_torch(safety_local, requested_actions, executed_actions)
    return (executed_actions.squeeze(0), metrics) if squeezed else (executed_actions, metrics)
...
def merge_action_info_into_step_info(info: dict[str, Any], action_info: dict[str, np.ndarray] | None, *, soc_pen_weight: float=0.0, action_pen_weight: float | None=None, apply_action_penalty: bool=False) -> tuple[dict[str, Any], np.ndarray]:
    ...
    for source_key, target_key in {'battery_power_req_kw': 'e_bat_req', 'battery_power_exec_kw': 'e_bat', 'pv_effective_req_kw': 'pv_effective_req', 'pv_effective_exec_kw': 'pv_effective', 'pv_curtail_req_kw': 'pv_curtail_req', 'pv_curtail_exec_kw': 'pv_curtail', 'pv_action_exec': 'pv_action'}.items():
        if source_key in action_info:
            updated[target_key] = np.asarray(action_info[source_key], dtype=np.float32)
    n_agents = int(np.asarray(updated.get('e_bat', np.zeros(0, dtype=np.float32))).reshape(-1).shape[0])
    penalty = (float(soc_pen_weight) * np.asarray(action_info['soc_penalty_unweighted'], dtype=np.float32)).astype(np.float32) if apply_action_penalty else np.zeros((n_agents,), dtype=np.float32)
    updated['r_soc_pen'] = updated['r_action_pen'] = penalty
...
def validate_executed_actions_numpy(safety_local: np.ndarray, actions: np.ndarray | list[np.ndarray], *, efficiency: float, dt_hours: float, soc_min: float, soc_max: float) -> None:
    ...
    pv_effective_kw = 0.5 * (action_array[:, 1] + 1.0) * pv_raw_kw
...
class JointGridSafetyProjector(nn.Module):
    ...
    def project_actions_from_safety_local(self, safety_local: torch.Tensor, raw_actions: torch.Tensor, *, return_diagnostics: bool=False) -> torch.Tensor | tuple[torch.Tensor, dict[str, float | int | bool]]:
        ...
        battery_lower_kw, battery_upper_kw, p_max_kw, pv_raw_kw = _local_bounds_torch(safety_local, efficiency=self.efficiency, dt_hours=self.dt_hours, soc_min=self.soc_min, soc_max=self.soc_max)
        ...
        return (
            ...
            {
                'enabled': True,
                'batch_size': int(battery_action.shape[0]),
                'projected_fraction': float(projected_any.float().mean().item()),
