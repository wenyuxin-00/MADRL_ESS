"""Joint safety projector used by the isolated MATD3 safe PoC path."""
...
def is_safe_poc_algorithm(cfg_or_name: Any) -> bool:
    """Return whether the experiment is using the isolated safe MATD3 branch."""
...
class JointGridSafetyProjector(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        trafo_margin_pct: float,
        linearization_delta_kw: float,
    ) -> None:
        super().__init__()
    ...
    def from_cfg(cls, cfg: Any, *, device: torch.device | str | None = None) -> "JointGridSafetyProjector":
        """Build one fixed linearized projector from the configured grid topology."""
        ...
        voltage_sensitivity = np.concatenate(
            [net_voltage_sensitivity, net_voltage_sensitivity],
            axis=1,
        )
        line_loading_sensitivity = np.concatenate(
        ...
        projector = cls(
            ...
            linearization_delta_kw=delta_kw,
        )
        if device is not None:
            projector = projector.to(device=torch.device(device))
        return projector

    def _extract_safety_local(
        self,
        ...
        *,
        ...
        if "safety_local" not in obs_t:
            raise KeyError(
                "Observation does not contain 'safety_local'. "
                "Enable the safety-aware observation schema before using MATD3_SAFE_POC."
            )
    ...
    def _local_bounds_kw(self, safety_local: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        charge_limit_kw = torch.minimum(
            p_max_kw,
            torch.clamp((energy_max - energy_now) / (eff * dt), min=0.0),
        )
        discharge_limit_kw = torch.minimum(
            p_max_kw,
    ...
    def _approximate_violation(
        ...
        *,
        x_kw: torch.Tensor,
        voltage_affine: torch.Tensor,
        line_affine: torch.Tensor,
        ...
        if line_sensitivity.numel() > 0:
            ...
            line_violation = torch.sum(
                torch.clamp(line_loading - (self.line_limit_pct - self.line_margin_pct), min=0.0),
                dim=-1,
            )
    ...
    def _project_rows_sequential(
        self,
        x_kw: torch.Tensor,
        rows: torch.Tensor,
        bounds: torch.Tensor,
        row_norm_sq: torch.Tensor,
    ) -> torch.Tensor:
    ...
    def _combined_bounds(
        ...
        *,
        ...
        bounds = [
            ...
            voltage_affine - voltage_low_limit,
        ]
    ...
    @staticmethod
    def _clamp_joint_action_kw(
        x_kw: torch.Tensor,
        *,
        battery_lower_kw: torch.Tensor,
        battery_upper_kw: torch.Tensor,
        pv_curtail_upper_kw: torch.Tensor,
        n_agents: int,
    ) -> torch.Tensor:
    ...
    def project_actions_from_safety_local(
        self,
        safety_local: torch.Tensor,
        raw_actions: torch.Tensor,
        *,
        return_diagnostics: bool = False,
    ) -> torch.Tensor | tuple[torch.Tensor, dict[str, float | int | bool]]:
        ...
        if int(safety_local.shape[0]) != int(raw_actions.shape[0]):
            raise ValueError(
                "safety_local batch dimension must match raw_actions batch dimension for safety projection."
            )
        ...
        x_kw = self._clamp_joint_action_kw(
            raw_kw,
            battery_lower_kw=battery_lower_kw,
            battery_upper_kw=battery_upper_kw,
            pv_curtail_upper_kw=pv_raw_kw,
            n_agents=self.n_agents,
        )

        voltage_affine, line_affine, trafo_affine = self._affine_terms(
            safety_local,
            dtype=work_dtype,
            device=device,
        )
        ...
        if return_diagnostics:
            pre_violation = self._approximate_violation(
                x_kw=raw_kw,
                voltage_affine=voltage_affine,
                line_affine=line_affine,
        ...
        combined_constraint_rows = self._as_work_dtype(
            self.combined_constraint_rows,
            dtype=work_dtype,
            device=device,
        )
        combined_constraint_row_norm_sq = self._as_work_dtype(
            self.combined_constraint_row_norm_sq,
            dtype=work_dtype,
            device=device,
        )
        ...
        for _ in range(self.projection_iters):
            ...
            if self.projector_mode == "joint_linearized_fast":
                combined_bounds = self._combined_bounds(
                    voltage_affine=voltage_affine,
                    line_affine=line_affine,
                    trafo_affine=trafo_affine,
                ...
                x_kw = self._project_rows_batched(
                    x_kw,
                    combined_constraint_rows,
                    combined_bounds,
                    combined_constraint_row_norm_sq,
                )
                continue
        ...
        projected_battery_action = torch.clamp(
            projected_battery_kw / torch.clamp(p_max_kw, min=_SAFETY_EPS),
            min=-1.0,
            max=1.0,
        )
        projected_pv_utilization = torch.ones_like(projected_pv_curtail_kw, dtype=work_dtype)
        valid_pv_mask = pv_raw_kw > _SAFETY_EPS
        projected_pv_utilization[valid_pv_mask] = (
            1.0 - projected_pv_curtail_kw[valid_pv_mask] / torch.clamp(pv_raw_kw[valid_pv_mask], min=_SAFETY_EPS)
        )
        ...
        post_violation = self._approximate_violation(
            x_kw=x_kw,
            voltage_affine=voltage_affine,
            line_affine=line_affine,
            trafo_affine=trafo_affine,
        ...
        projected_action_slice = (
            ...
            if squeezed
            else projected_actions.to(dtype=work_dtype)
        )
        delta = (projected_action_slice - raw_action_slice).abs()
        delta_kw = (x_kw - raw_kw).abs()
        projected_any = torch.any(delta.reshape(delta.shape[0], -1) > 1e-5, dim=-1)
    ...
    def project_actions(
        ...
        *,
        ...
        """Project one batch of joint actions into the approximate safe feasible set."""

        safety_local, _ = self._extract_safety_local(obs_t, dtype=torch.float32)
        return self.project_actions_from_safety_local(
            safety_local,
            raw_actions,
