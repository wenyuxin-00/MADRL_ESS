...
class MISOCPResult:
    ...
    physical_tiebreaker_eur: float = float('nan')
    physical_tiebreaker_weight: float = 0.0
    model_size: ModelSize | None = None
    simultaneous_charge_discharge_kw: np.ndarray | None = None
    simultaneous_agent_steps: int = 0
    simultaneous_step_ratio: float = 0.0
    max_simultaneous_kw: float = 0.0
    sol_count: int = 0
    node_count: float = 0.0
    iter_count: float = 0.0
    bar_iter_count: float = 0.0
...
class GlobalMISOCPProblem:
    ...
    def solve(self, import_price_seq: np.ndarray, load_seq: np.ndarray, pv_seq: np.ndarray, soc_init: np.ndarray, export_subsidy: float | None=None, *, verbose: bool=False, time_limit_sec: float | None=None, solve_config: GurobiSolveConfig | None=None) -> MISOCPResult:
        resolved_config = self._resolve_solve_config(solve_config, time_limit_sec=time_limit_sec)
        return self._solve_sequences(import_price_seq=import_price_seq, load_seq=load_seq, pv_seq=pv_seq, soc_init=soc_init, export_subsidy=export_subsidy, verbose=verbose, solve_config=resolved_config, expected_horizon=self.horizon, enforce_terminal_soc=True, solve_mode='single_window')
    ...
    def solve_full_horizon(self, import_price_seq: np.ndarray, load_seq: np.ndarray, pv_seq: np.ndarray, soc_init: np.ndarray, export_subsidy: float | None=None, *, timestamps: tuple[str, ...] | list[str] | None=None, episode_offsets: np.ndarray | list[int] | tuple[int, ...] | None=None, episode_lengths: np.ndarray | list[int] | tuple[int, ...] | None=None, verbose: bool=False, time_limit_sec: float | None=None, solve_config: GurobiSolveConfig | None=None) -> MISOCPResult:
        ...
        resolved_config = self._resolve_solve_config(solve_config, time_limit_sec=time_limit_sec)
        result = self._solve_sequences(import_price_seq=import_price_seq, load_seq=load_seq, pv_seq=pv_seq, soc_init=soc_init, export_subsidy=export_subsidy, verbose=verbose, solve_config=resolved_config, expected_horizon=None, enforce_terminal_soc=True, solve_mode='single_window')
    ...
    def solve_adaptive_full_horizon(self, full_input: FullHorizonProblemInput, export_subsidy: float | None=None, *, primary_window_steps: int=_ADAPTIVE_PRIMARY_WINDOW_STEPS, fallback_window_steps: int=_ADAPTIVE_FALLBACK_WINDOW_STEPS, verbose: bool=False, time_limit_sec: float | None=None, solve_config: GurobiSolveConfig | None=None) -> MISOCPResult:
        ...
        if int(full_input.horizon_steps) <= int(max(primary_window_steps, 1)):
            primary = self._solve_sequences(import_price_seq=full_input.import_price_seq, load_seq=full_input.load_seq, pv_seq=full_input.pv_seq, soc_init=full_input.soc_init, export_subsidy=export_subsidy, verbose=verbose, solve_config=resolved_config, expected_horizon=None, enforce_terminal_soc=True, solve_mode='single_window')
            primary = replace(primary, horizon_steps=int(full_input.horizon_steps), solve_mode='single_window', episode_offsets=full_input.episode_offsets.astype(np.int32, copy=False), episode_lengths=full_input.episode_lengths.astype(np.int32, copy=False))
            if primary.has_solution:
                return primary
    ...
    def solve_chunked_windows(self, full_input: FullHorizonProblemInput, export_subsidy: float | None=None, *, chunk_steps: int, verbose: bool=False, time_limit_sec: float | None=None, solve_config: GurobiSolveConfig | None=None) -> MISOCPResult:
        primary_solve_config = self._resolve_solve_config(solve_config, time_limit_sec=time_limit_sec)
        return self._solve_over_windows(full_input, windows=self._window_slices(full_input, chunk_steps=int(chunk_steps)), export_subsidy=export_subsidy, verbose=verbose, primary_solve_config=primary_solve_config, solve_mode='chunked_window')

    def solve_episode_chunks(self, full_input: FullHorizonProblemInput, export_subsidy: float | None=None, *, verbose: bool=False, time_limit_sec: float | None=None, solve_config: GurobiSolveConfig | None=None) -> MISOCPResult:
        primary_solve_config = self._resolve_solve_config(solve_config, time_limit_sec=time_limit_sec)
        ...
        return self._solve_over_windows(full_input, windows=episode_windows, export_subsidy=export_subsidy, verbose=verbose, primary_solve_config=primary_solve_config, solve_mode='chunked_episode')
    ...
    def _solve_over_windows(self, full_input: FullHorizonProblemInput, *, windows: list[tuple[int, int]], export_subsidy: float | None, verbose: bool, primary_solve_config: GurobiSolveConfig, solve_mode: str) -> MISOCPResult:
        ...
        for chunk_idx, (start, end) in enumerate(windows):
            ...
            if not result.has_solution:
                return replace(result, solve_mode=solve_mode, horizon_steps=int(full_input.horizon_steps), episode_offsets=full_input.episode_offsets.astype(np.int32, copy=False), episode_lengths=full_input.episode_lengths.astype(np.int32, copy=False))
            chunk_results.append(result)
            assert result.energy_mwh is not None
            current_soc = (np.asarray(result.energy_mwh[:, -1], dtype=np.float32) / np.maximum(self.capacity_mwh, _ROOT_VM_EPS)).astype(np.float32)
    ...
    def _solve_sequences(self, import_price_seq: np.ndarray, load_seq: np.ndarray, pv_seq: np.ndarray, soc_init: np.ndarray, export_subsidy: float | None, *, verbose: bool, solve_config: GurobiSolveConfig, expected_horizon: int | None, enforce_terminal_soc: bool, solve_mode: str) -> MISOCPResult:
        ...
        bundle = self._build_sequence_model(import_price_seq=import_price_seq, load_seq=load_seq, pv_seq=pv_seq, soc_init=soc_init, export_subsidy=export_subsidy, enforce_terminal_soc=enforce_terminal_soc, solve_mode=solve_mode)
        model = bundle.model
        ...
        has_solution = status_code == int(GRB.OPTIMAL) or (status_code == int(GRB.TIME_LIMIT) and int(getattr(model, 'SolCount', 0)) > 0)
        time_limit_feasible = status_code == int(GRB.TIME_LIMIT) and int(getattr(model, 'SolCount', 0)) > 0
        model_size = _model_size_from_model(model)
        sol_count = int(getattr(model, 'SolCount', 0))
        node_count = float(getattr(model, 'NodeCount', 0.0))
        iter_count = float(getattr(model, 'IterCount', 0.0))
        bar_iter_count = float(getattr(model, 'BarIterCount', 0.0))
        if not has_solution:
...
class GlobalSOCPMPCController:
    ...
    def act(self, obs: dict, deterministic: bool=True) -> list[np.ndarray]:
        ...
        first_battery_charge_kw = _first_step_grid(result.battery_charge_mw, scale=1000.0)
        first_battery_discharge_kw = _first_step_grid(result.battery_discharge_mw, scale=1000.0)
        first_battery_power_kw = None if first_battery_charge_kw is None or first_battery_discharge_kw is None else (first_battery_charge_kw - first_battery_discharge_kw).astype(np.float32)
        first_pv_curtail_kw = _first_step_grid(result.pv_curtail_mw, scale=1000.0)
        first_vm_pu = _first_step_grid(result.bus_vm_pu)
        first_line_loading_pct = _first_step_grid(result.line_loading_pct)
        first_trafo_loading_pct = _first_step_grid(result.trafo_loading_pct)
        root_import_kw = _first_step_scalar(result.root_import_mw, scale=1000.0)
        root_export_kw = _first_step_scalar(result.root_export_mw, scale=1000.0)
        root_p_kw = _first_step_scalar(result.root_p_kw)
        root_q_kvar = _first_step_scalar(result.root_q_kvar)
        if result.has_solution and first_battery_power_kw is not None and first_pv_curtail_kw is not None:
            battery_power_kw = self._clip_local_battery_power_kw(first_battery_power_kw)
            pv_curtail_kw = np.minimum(np.maximum(first_pv_curtail_kw, 0.0), np.maximum(np.asarray(self.env.get_signal_step('pv'), dtype=np.float32), 0.0)).astype(np.float32)
        else:
...
def _model_size_from_model(model: Any) -> ModelSize:
    return ModelSize(num_vars=int(getattr(model, 'NumVars', 0)), num_binary_vars=int(getattr(model, 'NumBinVars', 0)), num_linear_constraints=int(getattr(model, 'NumConstrs', 0)), num_quadratic_constraints=int(getattr(model, 'NumQConstrs', 0)))


def _sum_model_sizes(model_sizes: list[ModelSize | None]) -> ModelSize:
    valid = [model_size for model_size in model_sizes if model_size is not None]
    return ModelSize(num_vars=int(sum((model_size.num_vars for model_size in valid))), num_binary_vars=int(sum((model_size.num_binary_vars for model_size in valid))), num_linear_constraints=int(sum((model_size.num_linear_constraints for model_size in valid))), num_quadratic_constraints=int(sum((model_size.num_quadratic_constraints for model_size in valid))))
...
def _simultaneous_metrics(battery_charge_mw: np.ndarray, battery_discharge_mw: np.ndarray, p_max_mw: np.ndarray, threshold_ratio: float) -> tuple[np.ndarray, int, float, float]:
    simultaneous_kw = (1000.0 * np.minimum(np.asarray(battery_charge_mw, dtype=np.float32), np.asarray(battery_discharge_mw, dtype=np.float32))).astype(np.float32)
    threshold_kw = (1000.0 * float(max(threshold_ratio, 0.0)) * np.asarray(p_max_mw, dtype=np.float32).reshape(-1, 1)).astype(np.float32)
    simultaneous_agent_steps = int(np.count_nonzero(simultaneous_kw > threshold_kw))
    total_agent_steps = int(simultaneous_kw.shape[0] * simultaneous_kw.shape[1]) if simultaneous_kw.ndim == 2 else 0
    return (simultaneous_kw, simultaneous_agent_steps, float(simultaneous_agent_steps) / max(1, total_agent_steps), float(np.max(simultaneous_kw)) if simultaneous_kw.size else 0.0)


def _first_step_grid(values: np.ndarray | None, *, scale: float = 1.0) -> np.ndarray | None:
    return None if values is None else np.asarray(values[:, 0] * scale, dtype=np.float32)


def _first_step_scalar(values: np.ndarray | None, *, scale: float = 1.0) -> float:
    return float('nan') if values is None else float(values[0] * scale)
