"""Gurobi-based full-grid MISOCP helpers used by notebook comparisons."""
...
try:  # Keep the module importable outside the MADRL_ESS conda environment.
...
_ADAPTIVE_TARGET_MIP_GAP = 5e-3


@dataclass(frozen=True)
class NetworkModel:
    """Static branch-flow representation extracted from the SimBench feeder."""
    ...
    def from_cfg(cls, cfg: Any, *, agent_bus_ids: list[int]) -> "NetworkModel":
        ...
        if trafo_hv_bus != root_bus_id:
            raise ValueError(
                "Global SOCP-MPC expects the transformer HV bus to match the ext_grid root bus, "
                f"got ext_grid={root_bus_id} and trafo_hv_bus={trafo_hv_bus}."
            )
            ...
            raw_edges.append(
                {
                    "u": from_bus,
                    "v": to_bus,
                    "r_pu": float(r_total_ohm / z_base_ohm),
                    "x_pu": float(x_total_ohm / z_base_ohm),
                    "l_max_pu": l_max_pu,
                    "is_trafo": False,
                }
            )
        ...
        (
            ...
            branch_l_max_pu,
            branch_is_trafo,
            incoming_branch_by_bus,
            outgoing_branches_by_bus,
        ) = cls._build_tree(bus_ids, bus_pos, raw_edges, root_bus_id)

        root_outgoing = np.asarray(outgoing_branches_by_bus[bus_pos[root_bus_id]], dtype=np.int32)
        if root_outgoing.size != 1:
            raise ValueError(
                "Global SOCP-MPC expects exactly one outgoing root branch, "
                f"got {int(root_outgoing.size)}."
            )
        ...
        line_branch_indices = np.asarray(
            [idx for idx, is_trafo in enumerate(branch_is_trafo.tolist()) if not bool(is_trafo)],
            dtype=np.int32,
        )
        ...
        return cls(
            ...
            trafo_lv_bus_id=trafo_lv_bus,
            root_vm_pu=float(root_vm_pu),
            loading_limit_scale=float(max(float(grid_cfg.line_max_loading_pct), 0.0) / 100.0),
            bus_ids=bus_ids,
            bus_pos=bus_pos,
            v_base_kv=v_base_kv.astype(np.float32, copy=False),
            p_base_mw=p_base_mw.astype(np.float32, copy=False),
            q_base_mvar=q_base_mvar.astype(np.float32, copy=False),
            branch_parent_pos=branch_parent_pos.astype(np.int32, copy=False),
            branch_child_pos=branch_child_pos.astype(np.int32, copy=False),
            branch_r_pu=branch_r_pu.astype(np.float32, copy=False),
            branch_x_pu=branch_x_pu.astype(np.float32, copy=False),
            branch_is_trafo=branch_is_trafo.astype(bool, copy=False),
            branch_l_max_pu=branch_l_max_pu.astype(np.float32, copy=False),
            incoming_branch_by_bus=incoming_branch_by_bus.astype(np.int32, copy=False),
            outgoing_branches_by_bus=outgoing_branches_by_bus,
            root_outgoing_branches=root_outgoing.astype(np.int32, copy=False),
            trafo_branch_index=trafo_branch_index,
            line_branch_indices=line_branch_indices,
            agent_bus_positions=agent_bus_positions.astype(np.int32, copy=False),
        )
    ...
    def _build_tree(
        ...
        raw_edges: list[dict[str, object]],
        root_bus_id: int,
    ) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, tuple[tuple[int, ...], ...]]:
        ...
        if len(branch_parent_pos) != len(bus_ids) - 1:
            raise ValueError(
                "Global SOCP-MPC requires a tree topology; "
                f"got {len(branch_parent_pos)} branches for {len(bus_ids)} buses."
            )

        return (
            np.asarray(branch_parent_pos, dtype=np.int32),
            np.asarray(branch_child_pos, dtype=np.int32),
            np.asarray(branch_r_pu, dtype=np.float32),
            np.asarray(branch_x_pu, dtype=np.float32),
            np.asarray(branch_l_max_pu, dtype=np.float32),
            np.asarray(branch_is_trafo, dtype=bool),
            incoming_branch_by_bus,
            tuple(tuple(edge_idx for edge_idx in outgoing) for outgoing in outgoing_lists),
        )


@dataclass(frozen=True)
class ModelSize:
    """Compact model-size summary used by notebook debugging."""
...
class GurobiSolveConfig:
    """Notebook-friendly Gurobi knob bundle for MISOCP solves."""
...
@dataclass
class MISOCPResult:
    """Full-window solve result returned by the shared MISOCP core.

    Export subsidy fields store positive revenue values; net_cost = purchase - subsidy.
    """
...
def default_primary_solve_config() -> GurobiSolveConfig:
    """Return the notebook-oriented primary solve configuration."""

    return GurobiSolveConfig(
        time_limit_sec=_ADAPTIVE_PRIMARY_TIME_LIMIT_SEC,
        mip_gap=_ADAPTIVE_TARGET_MIP_GAP,
...
def default_retry_solve_config(primary: GurobiSolveConfig | None = None) -> GurobiSolveConfig:
    """Return the retry configuration used when the primary attempt finds no incumbent."""

    base = primary or default_primary_solve_config()
    return GurobiSolveConfig(
        time_limit_sec=_ADAPTIVE_RETRY_TIME_LIMIT_SEC,
        mip_gap=float(base.mip_gap),
        threads=base.threads,
        presolve=base.presolve,
        cuts=1,
        heuristics=0.20,
        mip_focus=1 if base.mip_focus is None else int(base.mip_focus),
    )


class GlobalMISOCPProblem:
    """Shared full-grid MISOCP solver used by compare and debug notebooks."""
    ...
    def from_env(
        ...
        *,
        ...
        network = NetworkModel.from_cfg(
            cfg,
            agent_bus_ids=list(getattr(env._grid_core, "agent_bus_ids", cfg.grid.agent_bus_ids)),
        )
        return cls(
            network=network,
            n_agents=int(env.n),
            ...
            capacity_mwh=np.asarray(env.agent_c_bat, dtype=np.float32) / 1000.0,
            p_max_mw=np.asarray(env.agent_p_max, dtype=np.float32) / 1000.0,
            soc_min=float(env.soc_min),
            soc_max=float(env.soc_max),
            soc_target=float(getattr(cfg.env, "soc_target", env.soc_max)),
    ...
    def _resolve_solve_config(
        self,
        solve_config: GurobiSolveConfig | None,
        *,
        time_limit_sec: float | None = None,
    ) -> GurobiSolveConfig:
    ...
    def _make_chunk_summary(
        ...
        *,
        ...
        return {
            ...
            "window_steps": int(end_step - start_step),
    ...
    def _run_window_with_retry(
        ...
        *,
        ...
        retry_solve_config: GurobiSolveConfig | None,
        ...
        retry_result = self._solve_sequences(
            ...
            export_subsidy=export_subsidy,
            verbose=verbose,
            solve_config=retry_solve_config,
            export_debug=export_debug,
    ...
    def solve(
        ...
        pv_seq: np.ndarray,
        soc_init: np.ndarray,
        export_subsidy: float | None = None,
        *,
        verbose: bool = False,
        time_limit_sec: float | None = None,
        solve_config: GurobiSolveConfig | None = None,
    ...
    def solve_full_horizon(
        ...
        export_subsidy: float | None = None,
        *,
        timestamps: tuple[str, ...] | list[str] | None = None,
        episode_offsets: np.ndarray | list[int] | tuple[int, ...] | None = None,
        episode_lengths: np.ndarray | list[int] | tuple[int, ...] | None = None,
        verbose: bool = False,
        time_limit_sec: float | None = None,
        solve_config: GurobiSolveConfig | None = None,
        export_debug: bool = False,
        ...
        return replace(
            ...
            solve_mode="single_window",
            episode_offsets=resolved_offsets.astype(np.int32, copy=False),
            episode_lengths=resolved_lengths.astype(np.int32, copy=False),
        )
    ...
    def solve_chunked_windows(
        ...
        *,
        ...
        time_limit_sec: float | None = None,
        solve_config: GurobiSolveConfig | None = None,
        retry_solve_config: GurobiSolveConfig | None = None,
        export_debug: bool = False,
        debug_tag: str | None = None,
    ) -> MISOCPResult:
        primary_solve_config = self._resolve_solve_config(solve_config, time_limit_sec=time_limit_sec)
        return self._solve_over_windows(
            full_input,
            windows=self._window_slices(full_input, chunk_steps=int(chunk_steps)),
            export_subsidy=export_subsidy,
            verbose=verbose,
        ...
        *,
        ...
        episode_windows = [
            ...
            for start, length in zip(
                np.asarray(full_input.episode_offsets, dtype=np.int32).tolist(),
                np.asarray(full_input.episode_lengths, dtype=np.int32).tolist(),
                strict=False,
            )
        ]
        return self._solve_over_windows(
            ...
            export_debug=export_debug,
            debug_tag=debug_tag,
            solve_mode="chunked_episode",
        )

    def solve_adaptive_full_horizon(
        self,
        full_input: FullHorizonProblemInput,
        export_subsidy: float | None = None,
        *,
        verbose: bool = False,
        primary_window_steps: int = _ADAPTIVE_PRIMARY_WINDOW_STEPS,
        fallback_window_steps: int = _ADAPTIVE_FALLBACK_WINDOW_STEPS,
        solve_config: GurobiSolveConfig | None = None,
        retry_solve_config: GurobiSolveConfig | None = None,
        export_debug: bool = False,
        debug_tag: str | None = None,
        ...
        if horizon_steps <= int(primary_window_steps):
            single_result, primary_result, retry_result, _ = self._run_window_with_retry(
                ...
                verbose=verbose,
                export_debug=export_debug,
                debug_tag=None if debug_tag is None else f"{debug_tag}_single_window",
                enforce_terminal_soc=True,
                solve_mode="single_window",
                primary_solve_config=primary_config,
            ...
            single_result = replace(
                single_result,
                debug_artifacts={**single_debug_artifacts, **dict(single_result.debug_artifacts)},
                horizon_steps=horizon_steps,
                ...
                no_retry_or_fallback_used=bool(primary_result.has_solution),
                chunk_retry_count=0,
            )
            if single_result.has_solution:
                return single_result

        chunked_result = self.solve_chunked_windows(
            full_input,
            export_subsidy=export_subsidy,
            chunk_steps=int(fallback_window_steps),
            verbose=verbose,
            solve_config=primary_config,
            retry_solve_config=retry_config,
            export_debug=export_debug,
            debug_tag=None if debug_tag is None else f"{debug_tag}_chunked_window",
        )
        ...
        return replace(
            chunked_result,
            debug_artifacts=combined_debug,
            no_retry_or_fallback_used=False,
        )

    def _solve_over_windows(
        self,
        full_input: FullHorizonProblemInput,
        *,
        windows: list[tuple[int, int]],
        ...
        for chunk_idx, (start, end) in enumerate(windows):
            chosen_result, primary_result, retry_result, attempt_used = self._run_window_with_retry(
                ...
                verbose=verbose,
                export_debug=export_debug,
                debug_tag=None if debug_tag is None else f"{debug_tag}_chunk{chunk_idx:03d}",
                enforce_terminal_soc=bool(chunk_idx == n_chunks - 1),
                solve_mode=solve_mode,
                primary_solve_config=primary_solve_config,
                retry_solve_config=retry_solve_config,
            )
            ...
            chunk_summary = self._make_chunk_summary(
                chosen_result,
                chunk_idx=chunk_idx,
            ...
            if not chosen_result.has_solution:
                ...
                return replace(
                    ...
                    solve_mode=solve_mode,
                    horizon_steps=int(full_input.horizon_steps),
                    episode_offsets=full_input.episode_offsets.astype(np.int32, copy=False),
                    episode_lengths=full_input.episode_lengths.astype(np.int32, copy=False),
            ...
            current_soc = (
                np.asarray(chosen_result.energy_mwh[:, -1], dtype=np.float32)
                / np.maximum(self.capacity_mwh, _ROOT_VM_EPS)
            ).astype(np.float32)

        return self._concatenate_chunk_results(
            chunk_results,
    ...
    def build_full_horizon_input(self, env: Any) -> FullHorizonProblemInput:
        ...
        for episode_idx in range(int(env.num_available_episodes)):
            ...
            if pv.shape != load.shape:
                raise ValueError(
                    f"Episode {episode_idx} pv signal should match load shape {load.shape}, got {pv.shape}."
                )
    ...
    def build_partial_mip_start(self, result: MISOCPResult) -> dict[str, np.ndarray]:
        ...
        if (
            result.battery_charge_mw is None
        ...
        if result.root_import_mw is None:
            ...
            u_grid = (np.asarray(result.root_import_mw, dtype=np.float32).reshape(-1) > 1e-6).astype(np.float32)
        return {
    ...
    def _apply_gurobi_solve_config(self, model: Any, solve_config: GurobiSolveConfig) -> None:
        model.Params.TimeLimit = float(
            _TIME_LIMIT_SEC if solve_config.time_limit_sec is None else solve_config.time_limit_sec
        )
        ...
        try:
            ...
            for key, variable_grid, expected_shape in matrix_specs:
                ...
                if values.shape != expected_shape:
                    raise ValueError(
                        f"{key} warm start shape {values.shape} does not match expected shape {expected_shape}."
                    )
    ...
    def _solve_sequences(
        ...
        *,
        ...
        debug_artifacts = self._maybe_export_debug_artifacts(
            model,
        ...
        if not has_solution:
            return MISOCPResult(
                ...
                mip_gap=float("nan"),
                best_bound=float(model.ObjBound) if hasattr(model, "ObjBound") else float("nan"),
                objective_value=float(model.ObjVal) if sol_count > 0 else float("nan"),
                agent_purchase_cost_eur=float("nan"),
                agent_export_subsidy_eur=float("nan"),
                agent_net_cost_eur=float("nan"),
                feeder_purchase_cost_eur=float("nan"),
                feeder_export_subsidy_eur=float("nan"),
                feeder_net_cost_eur=float("nan"),
                throughput_regularization_eur=float("nan"),
                throughput_regularization_weight=float(self.throughput_regularization_eur_per_kwh),
                physical_tiebreaker_eur=float("nan"),
                physical_tiebreaker_weight=float(self.branch_current_tiebreaker_eur_per_pu_step),
                ...
                bar_iter_count=bar_iter_count,
                sanity_warning=str(mip_start_warning or ""),
                horizon_steps=horizon_steps,
                solve_mode=solve_mode,
            )
        ...
        root_q_kvar = (
            np.sum(branch_q_pu[self.network.root_outgoing_branches.tolist(), :], axis=0)
            * self.network.s_base_mva
            * 1000.0
        ).astype(np.float32)
        ...
        trafo_loading_pct = (
            ...
            * np.sqrt(
                np.maximum(
                    (root_p_kw / (self.network.s_base_mva * 1000.0)) ** 2
                    + (root_q_kvar / (self.network.s_base_mva * 1000.0)) ** 2,
                    0.0,
                )
            )
        ...
        physical_tiebreaker_eur = float(
            self.dt_hours
            * self.branch_current_tiebreaker_eur_per_pu_step
            * np.sum(np.asarray(branch_i2_pu, dtype=np.float64))
        )
        (
            simultaneous_charge_discharge_kw,
            simultaneous_agent_steps,
            simultaneous_step_ratio,
            max_simultaneous_kw,
        ) = self._simultaneous_metrics(battery_charge_mw, battery_discharge_mw)
        ...
        return MISOCPResult(
            ...
            solve_time_sec=float(model.Runtime),
            mip_gap=float(model.MIPGap) if hasattr(model, "MIPGap") else float("nan"),
            best_bound=float(model.ObjBound) if hasattr(model, "ObjBound") else float("nan"),
            objective_value=float(model.ObjVal),
            agent_purchase_cost_eur=agent_purchase_cost_eur,
            agent_export_subsidy_eur=agent_export_subsidy_eur,
            agent_net_cost_eur=agent_net_cost_eur,
            feeder_purchase_cost_eur=feeder_purchase_cost_eur,
            feeder_export_subsidy_eur=feeder_export_subsidy_eur,
            feeder_net_cost_eur=feeder_net_cost_eur,
            throughput_regularization_eur=throughput_regularization_eur,
            throughput_regularization_weight=float(self.throughput_regularization_eur_per_kwh),
            ...
            pv_curtail_mw=pv_curtail_mw,
            energy_mwh=energy_mwh,
            branch_p_pu=branch_p_pu,
            branch_q_pu=branch_q_pu,
            branch_i2_pu=branch_i2_pu,
            bus_v_sq=bus_v_sq,
            root_import_mw=root_import_mw.astype(np.float32),
            root_export_mw=root_export_mw.astype(np.float32),
            root_p_kw=root_p_kw,
            root_q_kvar=root_q_kvar,
            bus_vm_pu=bus_vm_pu,
            ...
            simultaneous_agent_steps=simultaneous_agent_steps,
            simultaneous_step_ratio=simultaneous_step_ratio,
    ...
    def _concatenate_chunk_results(
        ...
        *,
        ...
        return MISOCPResult(
            ...
            time_limit_feasible=time_limit_feasible,
            solve_time_sec=float(sum(float(result.solve_time_sec) for result in chunk_results)),
            mip_gap=float(max(finite_gaps)) if finite_gaps else float("nan"),
            best_bound=float("nan"),
            objective_value=float(sum(float(result.objective_value) for result in chunk_results)),
            agent_purchase_cost_eur=float(sum(float(result.agent_purchase_cost_eur) for result in chunk_results)),
            agent_export_subsidy_eur=float(sum(float(result.agent_export_subsidy_eur) for result in chunk_results)),
            agent_net_cost_eur=float(sum(float(result.agent_net_cost_eur) for result in chunk_results)),
            feeder_purchase_cost_eur=float(sum(float(result.feeder_purchase_cost_eur) for result in chunk_results)),
            feeder_export_subsidy_eur=float(sum(float(result.feeder_export_subsidy_eur) for result in chunk_results)),
            feeder_net_cost_eur=float(sum(float(result.feeder_net_cost_eur) for result in chunk_results)),
            throughput_regularization_eur=float(
                sum(float(result.throughput_regularization_eur) for result in chunk_results)
            ),
            throughput_regularization_weight=float(reference.throughput_regularization_weight),
            physical_tiebreaker_eur=float(sum(float(result.physical_tiebreaker_eur) for result in chunk_results)),
            physical_tiebreaker_weight=float(reference.physical_tiebreaker_weight),
            model_size=ModelSize(
                num_vars=max(int(size.num_vars) for size in model_sizes),
                num_binary_vars=max(int(size.num_binary_vars) for size in model_sizes),
                num_linear_constraints=max(int(size.num_linear_constraints) for size in model_sizes),
                num_quadratic_constraints=max(int(size.num_quadratic_constraints) for size in model_sizes),
            ),
            debug_artifacts=debug_artifacts,
            agent_net_grid_mw=np.concatenate([result.agent_net_grid_mw for result in chunk_results], axis=1),
            agent_import_mw=np.concatenate([result.agent_import_mw for result in chunk_results], axis=1),
            agent_export_mw=np.concatenate([result.agent_export_mw for result in chunk_results], axis=1),
            battery_charge_mw=np.concatenate([result.battery_charge_mw for result in chunk_results], axis=1),
            battery_discharge_mw=np.concatenate([result.battery_discharge_mw for result in chunk_results], axis=1),
            pv_curtail_mw=np.concatenate([result.pv_curtail_mw for result in chunk_results], axis=1),
            energy_mwh=np.concatenate(
                [
                    chunk_results[0].energy_mwh,
                    *[result.energy_mwh[:, 1:] for result in chunk_results[1:]],
                ],
                axis=1,
            ),
            branch_p_pu=np.concatenate([result.branch_p_pu for result in chunk_results], axis=1),
            branch_q_pu=np.concatenate([result.branch_q_pu for result in chunk_results], axis=1),
            branch_i2_pu=np.concatenate([result.branch_i2_pu for result in chunk_results], axis=1),
            bus_v_sq=np.concatenate([result.bus_v_sq for result in chunk_results], axis=1),
            root_import_mw=np.concatenate([result.root_import_mw for result in chunk_results], axis=0),
            root_export_mw=np.concatenate([result.root_export_mw for result in chunk_results], axis=0),
            root_p_kw=np.concatenate([result.root_p_kw for result in chunk_results], axis=0),
            root_q_kvar=np.concatenate([result.root_q_kvar for result in chunk_results], axis=0),
            bus_vm_pu=np.concatenate([result.bus_vm_pu for result in chunk_results], axis=1),
            line_loading_pct=np.concatenate([result.line_loading_pct for result in chunk_results], axis=1),
            trafo_loading_pct=np.concatenate([result.trafo_loading_pct for result in chunk_results], axis=1),
            simultaneous_charge_discharge_kw=np.concatenate(
                [result.simultaneous_charge_discharge_kw for result in chunk_results], axis=1
            ),
            simultaneous_agent_steps=int(sum(int(result.simultaneous_agent_steps) for result in chunk_results)),
            simultaneous_step_ratio=float(
                np.mean(
                    np.any(
                        np.concatenate(
                            [result.simultaneous_charge_discharge_kw for result in chunk_results],
                            axis=1,
                        )
                        > 0.0,
                        axis=0,
            ...
            max_simultaneous_kw=float(max(float(result.max_simultaneous_kw) for result in chunk_results)),
            sol_count=int(sum(int(result.sol_count) for result in chunk_results)),
            node_count=_finite_sum(node_counts),
            iter_count=_finite_sum(iter_counts),
            bar_iter_count=_finite_sum(bar_iter_counts),
            sanity_warning=str(reference.sanity_warning),
            horizon_steps=int(np.sum(episode_lengths)),
    ...
    def run_transformer_sanity_check(self) -> dict[str, object]:
        """Return a non-blocking transformer check for notebook debugging."""
        ...
        return {
            ...
            "abs_error_pu": np.nan,
            "misocp_vm_pu": float("nan") if result.first_step_vm_pu is None else float(result.first_step_vm_pu[int(self.network.bus_pos[self.network.trafo_lv_bus_id])]),
            "pandapower_vm_pu": float("nan"),
            "warning": "Use notebook or rollout validation for detailed pandapower comparison.",
        }
...
class GlobalSOCPMPCController(BaseController):
    """Notebook-facing controller wrapper around the shared Gurobi MISOCP solver."""
    ...
    def __init__(
        ...
        cfg: Any,
        *,
        problem: GlobalMISOCPProblem | None = None,
        solve_verbose: bool = False,
        export_debug: bool = False,
        debug_tag_prefix: str = "global_misocp",
    ) -> None:
        if gp is None or GRB is None:  # pragma: no cover - depends on active interpreter
            raise RuntimeError(f"{_GUROBI_ERROR_PREFIX}: gurobipy import failed")
    ...
    def act(self, obs: dict, deterministic: bool = True) -> list[np.ndarray]:
        ...
        if result.has_solution and result.first_step_battery_power_kw is not None and result.first_step_pv_curtail_kw is not None:
            ...
            pv_curtail_kw = np.minimum(
                np.maximum(result.first_step_pv_curtail_kw, 0.0),
                np.maximum(np.asarray(self.env.get_signal_step("pv"), dtype=np.float32), 0.0),
            ).astype(np.float32)
        else:
        ...
        safety_local = (
            self.env._current_safety_local()
            if hasattr(self.env, "_current_safety_local")
            else build_safety_local_numpy(
                soc=np.asarray(self.env.soc, dtype=np.float32),
                load_raw=np.asarray(self.env.get_signal_step("load"), dtype=np.float32),
                pv_raw=np.asarray(self.env.get_signal_step("pv"), dtype=np.float32),
        ...
        action_info.update(
            {
                "misocp_fallback": np.asarray(float(not result.has_solution), dtype=np.float32),
                "misocp_time_limit_feasible": np.asarray(float(result.time_limit_feasible), dtype=np.float32),
                "solve_time_sec": np.asarray(result.solve_time_sec, dtype=np.float32),
                "root_import_kw": np.asarray(result.first_step_root_import_kw, dtype=np.float32),
                "root_export_kw": np.asarray(result.first_step_root_export_kw, dtype=np.float32),
                "simultaneous_charge_discharge_kw_total": np.asarray(result.first_step_simultaneous_kw_total, dtype=np.float32),
                "simultaneous_agent_count": np.asarray(float(result.first_step_simultaneous_agent_count), dtype=np.float32),
                "simultaneous_step_flag": np.asarray(float(result.first_step_simultaneous_agent_count > 0), dtype=np.float32),
            }
        )
        ...
        diagnostic = {
            ...
            "status_label": str(result.status_label),
            "solve_time_sec": float(result.solve_time_sec),
            "misocp_fallback": float(not result.has_solution),
            ...
            "root_import_kw": float(result.first_step_root_import_kw),
            "root_export_kw": float(result.first_step_root_export_kw),
            "root_p_kw": float(result.first_step_root_p_kw),
            "root_q_kvar": float(result.first_step_root_q_kvar),
            ...
            "battery_charge_kw": None if result.first_step_battery_charge_kw is None else result.first_step_battery_charge_kw.copy(),
            "battery_discharge_kw": None if result.first_step_battery_discharge_kw is None else result.first_step_battery_discharge_kw.copy(),
            "misocp_vm_pu": None if result.first_step_vm_pu is None else result.first_step_vm_pu.copy(),
            "misocp_line_loading_pct": None if result.first_step_line_loading_pct is None else result.first_step_line_loading_pct.copy(),
            "misocp_trafo_loading_pct": None if result.first_step_trafo_loading_pct is None else result.first_step_trafo_loading_pct.copy(),
            ...
            "sanity_warning": str(result.sanity_warning or self.sanity_warning or ""),
        }
    ...
    def _solve_step(self, raw_obs: dict[str, np.ndarray]) -> MISOCPResult:
        ...
        return self.problem.solve(
            ...
            export_subsidy=float(getattr(self.cfg.reward, "export_subsidy_eur_per_kwh", self.problem.export_subsidy_default)),
...
def _extract_grid_values(var_grid: Any, n_rows: int, n_cols: int) -> np.ndarray:
    return np.asarray(
        [[float(var_grid[row_idx, col_idx].X) for col_idx in range(n_cols)] for row_idx in range(n_rows)],
        dtype=np.float32,
    )
...
def _compute_agent_net_grid_mw(
    load_seq_kw: np.ndarray,
    pv_raw_seq_kw: np.ndarray,
    battery_charge_mw: np.ndarray,
    battery_discharge_mw: np.ndarray,
    pv_curtail_mw: np.ndarray,
) -> np.ndarray:
    ...
    return (
        (load_seq_kw - pv_raw_seq_kw) / 1000.0
        + battery_charge_mw
        - battery_discharge_mw
        + pv_curtail_mw
    ).astype(np.float32)
...
__all__ = [
    ...
    "_GUROBI_ERROR_PREFIX",
    "_LINE_LOADING_ERR_TOL_PCT",
    "_ROOT_POWER_ERR_TOL_KW",
    "_SIMULTANEOUS_THRESHOLD_RATIO",
    "_SIMULTANEOUS_WARNING_RATIO",
    "_THROUGHPUT_REGULARIZATION_EUR_PER_KWH",
