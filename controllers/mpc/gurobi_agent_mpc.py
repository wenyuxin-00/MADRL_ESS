...
LOCAL_MPC_TIME_LIMIT_SEC = 5.0
LOCAL_MPC_MIP_GAP = 0.0001
...
def _empty_full_horizon_result(horizon: int, *, solve_time_sec: float=0.0, net_load_floor_kw: np.ndarray | None=None) -> LocalMPCFullHorizonResult:
    empty_horizon = np.zeros((max(int(horizon), 0),), dtype=np.float32)
    empty_energy = np.zeros((max(int(horizon) + 1, 0),), dtype=np.float32)
    floor = None if net_load_floor_kw is None else np.asarray(net_load_floor_kw, dtype=np.float32).reshape(-1).copy()
    return LocalMPCFullHorizonResult(charge_kw=empty_horizon.copy(), discharge_kw=empty_horizon.copy(), pv_curtail_kw=empty_horizon.copy(), pv_effective_kw=empty_horizon.copy(), pv_utilization=empty_horizon.copy(), signed_battery_kw=empty_horizon.copy(), net_load_kw=empty_horizon.copy(), energy_kwh=empty_energy, objective_eur=0.0, solve_time_sec=float(solve_time_sec), feasible=False, grid_import_kw=empty_horizon.copy(), grid_export_kw=empty_horizon.copy(), net_load_floor_kw=floor)
...
def _sanitize_problem_inputs(*, import_price_seq: np.ndarray, load_seq: np.ndarray, pv_seq: np.ndarray, soc: float, battery_capacity_kwh: float, p_max_kw: float, dt_hours: float, efficiency: float, soc_min: float, soc_max: float) -> dict[str, Any] | None:
    ...
    return {'prices': prices, 'pv': pv, 'net_load': net_load, 'horizon': horizon, 'capacity': capacity, 'power_limit': power_limit, 'dt': dt, 'eff': eff, 'energy_now': energy_now}

def _shift_primal_solution_start(solution: _LocalMPCPrimalSolution, *, current_energy_kwh: float) -> _LocalMPCPrimalSolution:
    def _shift_control(values: np.ndarray) -> np.ndarray:
        array = np.asarray(values, dtype=np.float32).reshape(-1)
        return array.copy() if array.size == 0 else np.concatenate([array[1:], np.zeros((1,), dtype=np.float32)]).astype(np.float32, copy=False)
    energy = np.asarray(solution.energy_kwh, dtype=np.float32).reshape(-1)
    shifted_energy = energy.copy()
    if shifted_energy.size:
        shifted_energy[0] = np.float32(current_energy_kwh)
        if shifted_energy.size > 2:
            shifted_energy[1:-1] = energy[2:]
        if shifted_energy.size > 1:
            shifted_energy[-1] = energy[-2]
...
class _ReusableLocalMPCSolver:
    ...
    def _solve_prepared(self, *, prepared: dict[str, Any] | None, net_load_floor_kw: np.ndarray | None=None, pv_curtail_upper_kw: np.ndarray | None=None) -> LocalMPCFullHorizonResult:
        if prepared is None:
            return _empty_full_horizon_result(self.horizon, net_load_floor_kw=net_load_floor_kw)
        ...
        if not self._status_has_usable_solution():
            return _empty_full_horizon_result(self.horizon, solve_time_sec=solve_time_sec, net_load_floor_kw=floor)
...
def solve_local_gurobi_mpc_full_horizon(*, import_price_seq: np.ndarray, load_seq: np.ndarray, pv_seq: np.ndarray, soc: float, battery_capacity_kwh: float, p_max_kw: float, dt_hours: float, efficiency: float, soc_min: float, soc_max: float, export_subsidy_eur_per_kwh: float, net_load_floor_kw: np.ndarray | None=None, pv_curtail_upper_kw: np.ndarray | None=None) -> LocalMPCFullHorizonResult:
    ...
    if prepared is None:
        return _empty_full_horizon_result(0, net_load_floor_kw=net_load_floor_kw)
    ...
    try:
        return solver._solve_prepared(
            prepared=prepared,
            net_load_floor_kw=None if net_load_floor_kw is None else np.asarray(net_load_floor_kw, dtype=np.float32)[:int(prepared['horizon'])],
            pv_curtail_upper_kw=None if pv_curtail_upper_kw is None else np.asarray(pv_curtail_upper_kw, dtype=np.float32)[:int(prepared['horizon'])],
        )
    finally:
        solver.dispose()
...
def solve_local_gurobi_mpc_action(*, import_price_seq: np.ndarray, load_seq: np.ndarray, pv_seq: np.ndarray, soc: float, battery_capacity_kwh: float, p_max_kw: float, dt_hours: float, efficiency: float, soc_min: float, soc_max: float, export_subsidy_eur_per_kwh: float) -> float:
    ...
    if not result.feasible or result.signed_battery_kw.size == 0:
        return 0.0
    return 0.0 if abs(float(result.signed_battery_kw[0])) < 1e-08 else float(result.signed_battery_kw[0])
