...
LOCAL_MPC_TIME_LIMIT_SEC=5.
LOCAL_MPC_MIP_GAP=.0001
...
class _ReusableLocalMPCSolver:
	...
	def _apply_problem_data(self,*,prices:np.ndarray,net_load:np.ndarray,pv_available_kw:np.ndarray,energy_now:float,net_load_floor_kw:np.ndarray|None=None,pv_curtail_upper_kw:np.ndarray|None=None)->None:
		...
		for step_idx in range(self.horizon):
			price_t=float(prices[step_idx]);energy_tiebreaker=float(self.horizon-step_idx)*ENERGY_TIME_TIEBREAKER_EPS_EUR_PER_KWH if price_t<.0 else .0;throughput_penalty=BATTERY_THROUGHPUT_TIEBREAKER_EPS_EUR_PER_KWH*self.dt;gross_flow_limit_kw=abs(float(net_load[step_idx]))+self.power_limit+float(curtail_upper[step_idx])
			...
			self._pv_curtail_upper_constrs[step_idx].RHS=float(curtail_upper[step_idx]);self._grid_balance_constrs[step_idx].RHS=float(net_load[step_idx]);self._grid_import_ub_constrs[step_idx].RHS=gross_flow_limit_kw;self._grid_export_ub_constrs[step_idx].RHS=gross_flow_limit_kw;self.charge[step_idx].Obj=throughput_penalty;self.discharge[step_idx].Obj=throughput_penalty;self.pv_curtail[step_idx].Obj=.0;self.energy[step_idx+1].Obj=-energy_tiebreaker;self.model.chgCoeff(self._fallback_import_gate_constrs[step_idx],self.grid_mode[step_idx],-gross_flow_limit_kw);self._fallback_import_gate_constrs[step_idx].RHS=.0;self.model.chgCoeff(self._fallback_export_gate_constrs[step_idx],self.grid_mode[step_idx],gross_flow_limit_kw);self._fallback_export_gate_constrs[step_idx].RHS=gross_flow_limit_kw;self.grid_import[step_idx].Obj=(price_t+GRID_EXPORT_TIEBREAKER_EPS_EUR_PER_KWH)*self.dt;self.grid_export[step_idx].Obj=(-self.export_subsidy_eur_per_kwh+GRID_EXPORT_TIEBREAKER_EPS_EUR_PER_KWH)*self.dt;self.grid_mode[step_idx].Obj=.0
