"""Visualisation helpers for grid-aware environments.

Two main functions:

* :func:`plot_node_voltages` — time-series line chart of per-agent bus
  voltages, with horizontal dashed lines at the v_min / v_max limits and
  violation regions highlighted.

* :func:`plot_battery_and_grid` — combined chart: voltage in the top row,
  then one row per agent showing battery power (bar) and SoC (line twin axis).
  Reuses data from the standard ``episode_recorder`` format for the battery
  rows and from ``grid_recorder`` for the voltage row.
"""
