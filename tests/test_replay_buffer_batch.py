...
def test_replay_buffer_stores_and_samples_canonical_batch(tmp_path):
    ...
    try:
        ...
        assert set(batch.keys()) == {"obs", "action", "reward", "next_obs", "terminated", "truncated", "bootstrap_discount"}
        ...
        assert batch["bootstrap_discount"].shape == (cfg.train.batch_size, cfg.env.num_agents, 1)
        ...
        assert direct_torch_batch["bootstrap_discount"].dtype == torch.float32
...
def test_replay_buffer_samples_n_step_by_parallel_env_lane(tmp_path):
    case_dir = make_case_dir(tmp_path, "buffer_n_step")
    cfg = make_smoke_config(case_dir, algorithm="MADDPG")
    cfg.train.batch_size = 1
    cfg.train.n_step_return = 3
    cfg.algo.gamma = 0.5
    env = build_env(cfg, mode="train")
    try:
        cfg.runtime.observation_schema = dict(env.observation_schema)
        ...
        obs, _ = env.reset(episode_idx=0)
        batched_obs = stack_nested([obs, obs])
        batched_next_obs = stack_nested([obs, obs])
        action = np.zeros((2, cfg.env.num_agents, cfg.runtime.action_dim), dtype=np.float32)
        terminated = np.zeros((2, cfg.env.num_agents, 1), dtype=np.float32)
        truncated = np.zeros((2, cfg.env.num_agents, 1), dtype=np.float32)
        for step in range(4):
            reward = np.zeros((2, cfg.env.num_agents, 1), dtype=np.float32)
            reward[0, :, 0] = 10.0 + step
            reward[1, :, 0] = 100.0 + step
            buffer.store_transitions_batched(batched_obs, action, reward, batched_next_obs, terminated, truncated)

        buffer._sample_indices = lambda: np.array([0], dtype=np.int64)
        batch = buffer.sample()

        expected = 10.0 + 0.5 * 11.0 + 0.25 * 12.0
        np.testing.assert_allclose(batch["reward"][0, :, 0], expected, rtol=1e-6)
        np.testing.assert_allclose(batch["bootstrap_discount"][0, :, 0], 0.125, rtol=1e-6)
    finally:
        env.close()
