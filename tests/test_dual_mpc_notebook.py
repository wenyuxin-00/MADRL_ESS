...
def test_dual_mpc_notebook_code_cells_compile():
    repo_root = Path(__file__).resolve().parents[1]
    notebook_path = repo_root / "notebooks" / "madrl" / "Dual_mpc.ipynb"
    assert not (repo_root / "notebooks" / "madrl" / "Dual.ipynb").exists()
...
def test_dual_mpc_notebook_contains_compare_and_dual_rollout_calls():
    repo_root = Path(__file__).resolve().parents[1]
    notebook_path = repo_root / "notebooks" / "madrl" / "Dual_mpc.ipynb"
