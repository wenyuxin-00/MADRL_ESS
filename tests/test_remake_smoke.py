...
from scripts.madrl import REWARD_COMPONENT_COLUMNS, _plot_reward_curves
...
def test_madrl_reward_curve_plot_handles_components_and_legacy_cache(tmp_path: Path) -> None:
    import matplotlib
    ...
    plt.close(fig)
    legacy = pd.DataFrame({"scheme": ["madrl_base", "madrl_base"], "episode": [1, 2], "total_reward": [1.0, 2.0]})
    fig = _plot_reward_curves(legacy, tmp_path, "legacy")
    assert len(fig.axes) == 1
    assert "component columns missing" in fig.axes[0].get_title()
    assert (tmp_path / "figures" / "legacy_learning_curve.png").exists()
    plt.close(fig)
