...
def test_local_mpc_notebook_contains_config_and_cache_calls():
    repo_root = Path(__file__).resolve().parents[1]
    notebook_path = repo_root / "notebooks" / "madrl" / "local_MPC.ipynb"
    ...
    assert "SAVE_LOCAL_MPC_ROLLOUT = True" in joined_source
    assert "LOCAL_MPC_PERFECT_TAG_OVERRIDE = None" in joined_source
    assert "LOCAL_MPC_LSTM_TAG_OVERRIDE = None" in joined_source
    ...
    assert "grid_nb.collect_local_mpc_rollout(" in joined_source
    assert "local_mpc_nb.LOCAL_MPC_PERFECT_LABEL" in joined_source
    assert "local_mpc_nb.LOCAL_MPC_LSTM_LABEL" in joined_source
    assert "build_local_mpc_rollout_package(" in joined_source
    assert "save_local_mpc_rollout_package(" in joined_source
    assert "PROJECT_ROOT / \"artifacts\" / \"local_mpc_cached_rollout\"" in joined_source
