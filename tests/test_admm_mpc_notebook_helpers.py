...
def test_rho_is_clamped_in_solver():
    cfg = _make_cfg(future_horizon=5, n_agents=1)
    env = _make_env(n_agents=1, future_horizon=5)
    raw_obs = env.obs_builder.build_raw(env)
    surrogate_cache = _make_surrogate_cache(horizon=5, n_agents=1)
    window_data = admm_mpc_nb.build_admm_mpc_window_data(cfg, env, raw_obs)
...
def test_plotting_helpers_accept_admm_mpc_style_rollout(monkeypatch):
    ...
    rollout = admm_mpc_nb.collect_admm_mpc_rollout(cfg, prediction_mode="normal")
    fig_1 = grid_nb.plot_rollout_dashboard(rollout)
...
def test_compare_helpers_accept_admm_mpc_and_local_mpc_rollouts(monkeypatch):
    ...
    single_agent_rollout = collect_local_mpc_rollout(
        cfg,
        prediction_mode="normal",
        label="Local MPC + LSTM Forecast",
    )

    metrics_df = compare_rollout_metrics(admm_rollout, single_agent_rollout)
