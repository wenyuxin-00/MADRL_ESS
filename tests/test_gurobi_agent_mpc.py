...
def test_local_gurobi_solver_prevents_simultaneous_import_and_export():
    ...
    assert not (result.grid_import_kw[0] > 1e-6 and result.grid_export_kw[0] > 1e-6)
...
def test_local_gurobi_solver_handles_price_below_subsidy():
    power = gurobi_agent_mpc.solve_local_gurobi_mpc_action(
...
def test_export_variable_matches_negative_grid_flow():
    result = gurobi_agent_mpc.solve_local_gurobi_mpc_full_horizon(
    ...
    assert result.net_load_kw[0] < 0.0
    assert result.grid_export_kw[0] == pytest.approx(abs(result.net_load_kw[0]), abs=1e-6)
    assert result.grid_import_kw[0] == pytest.approx(0.0, abs=1e-6)
...
def test_grid_export_variable_is_zero_for_import_case():
    result = gurobi_agent_mpc.solve_local_gurobi_mpc_full_horizon(
        import_price_seq=np.array([0.20], dtype=np.float32),
        load_seq=np.array([1.0], dtype=np.float32),
        pv_seq=np.array([0.0], dtype=np.float32),
    ...
    assert result.net_load_kw[0] > 0.0
    assert result.grid_export_kw[0] == pytest.approx(0.0, abs=1e-6)
