...
def test_env_step_respects_soc_bounds(tmp_path):
    ...
    try:
        ...
        _, _, _, info = env.step([np.array([-1.0], dtype=np.float32) for _ in range(cfg.env.num_agents)])
