...
def _derive_training_budget(cfg: ExperimentConfig) -> dict[str, int | str | None]:
    train_episode_limit, test_episode_limit, num_envs = (max(1, int(resolve_train_episode_limit(cfg))), max(1, int(resolve_test_episode_limit(cfg))), max(1, int(cfg.train.num_envs)))
    resolved_steps = int(cfg.train.resolved_max_train_steps(train_episode_limit)); target_parallel_iters = resolved_steps // num_envs
    completed_rounds, partial_steps_per_env = (target_parallel_iters // train_episode_limit, target_parallel_iters % train_episode_limit)
    return {'episode_limit': train_episode_limit, 'test_episode_limit': test_episode_limit, 'future_horizon': int(cfg.env.future_horizon), 'max_train_steps': int(cfg.train.max_train_steps) if cfg.train.max_train_steps is not None else None, 'resolved_train_steps': resolved_steps, 'parallel_rollout_iterations': int(target_parallel_iters), 'expected_completed_episodes_floor': int(completed_rounds * num_envs), 'partial_steps_per_env_at_stop': int(partial_steps_per_env), 'budget_source': 'max_train_steps' if cfg.train.max_train_steps is not None else 'train_episodes * train_episode_limit'}
