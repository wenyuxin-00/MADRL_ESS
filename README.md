...
- �?`LSTM` 预测 `wholesale_price / load / pv`
- �?`GridEnv` 中训练和评估多智能体控制�?
- 在同一测试窗口下比较三�?`MADRL` 方案与两�?`MPC` 基线

这份 README 不再以旧训练入口为中心，而是按当前代码状态，优先说明�?

1. 现在真正对比的三�?`MADRL` 方案是什�?
2. 环境、动作、约束、奖励和测试目标的数学模型是什�?
3. 三种方案的差异发生在哪一�?

## 当前代码中的三种 MADRL 方案

| 方案�?| Notebook | Algorithm | 电网安全惩罚 | 本地 SoC 约束 | 安全投影�?| 核心机制说明 |
| --- | --- | --- | --- | --- | --- | --- |
| `MADRL + No Safety` | `notebooks/madrl/train_base.ipynb` | `MATD3` | 否，默认 `w_voltage_pen = w_line_pen = w_trafo_pen = 0` | �?| �?| 训练和评估前都保留本地电池可行性裁剪，并保�?`r_soc_pen`；这里只是“不对电网越限额外罚分”，不是“完全无约束�?|
| `MADRL + Safety Penalty` | `notebooks/madrl/train_base_safe.ipynb` | `MATD3` | 是，默认三项安全惩罚权重均为 `10.0` | �?| �?| 动作链与 `MADRL + No Safety` 相同，但把电压、线路、变压器越限写进优化目标 |
| `MADRL + Safety Projection` | `notebooks/madrl/train_projection_safe.ipynb` | `MATD3_SAFE_POC` | 是，默认三项安全惩罚权重均为 `10.0` | �?| �?| �?`rollout`、`target-Q` �?`actor-loss` 三处都引�?joint safety projector，并�?projector 后再做一次本�?SoC 可行�?|

`MADRL + No Safety` 这个名字需要特别注意：它的含义是“无电网安全惩罚”，不是“智能体动作完全不受约束”。当前代码里三种方案都会保留�?

- 本地电池 `SoC` 可行性裁�?`enforce_local_action_feasibility_torch`
- 储能越限惩罚 `r_soc_pen`
- 默认 `w_soc_pen = 10.0`
...
- 预测窗口长度�?$H$，观测中包含未来一�?`wholesale_price / load / pv` 序列
...
- $P^{bat,req}_{i,t}$ 是控制器申请的储能功�?
- $P^{pv,eff}_{i,t}$ 是实际被利用的光伏功�?
- $P^{pv,curt}_{i,t}$ 是弃光功�?
...
由于放电�?$P^{bat,exec}_{i,t}$ 为负值，所�?$\Delta E_{i,t}$ 也为负，电池能量会下降。于是：
...
�?`MADRL + Safety Projection` 中，`r_soc_pen` 的来源略有不同：它主要记�?`projected -> executed` 的残差，而不�?`raw -> executed` 的差值�?
...
- $r_{purchase\_cost}$：购电成�?
- $r_{export\_subsidy}$：向上级电网售电补贴
- $r_{soc\_pen}$：储能本地可行性惩�?
- $r_{safe\_v}$：电压越限惩�?
- $r_{safe\_line}$：线路越限惩�?
...
2. `soc_penalty_total`、`voltage_penalty_total`、`line_penalty_total`、`trafo_penalty_total` 都是带权后的惩罚累计值，不是未加权原�?violation
...
其余三项安全惩罚总量也是类似的“带�?violation 累计”�?

### 6. `MADRL + Safety Projection` �?projector 数学描述

`MATD3_SAFE_POC` �?joint safety projector 工作在物理量 `kW` 空间，而不是归一化动作空间。对 $N$ 个智能体，定义：
...
- $S_v$�?S_l$�?S_{\tau}$ 是对节点电压、线路负载率、变压器负载率的灵敏度矩�?
...
在代码里，projector 输出的联合动作会先从 `kW` 空间转换回归一化动作，再经过一次本�?`SoC` feasibility clamp，然后才真正送入环境。这也是当前 `projection_safe` 路径和另外两种方案最大的实现差异�?
...
- 算法：`MATD3`
- 交互动作链：`raw action -> local SoC feasibility -> env`
- �?`MADRL + No Safety` 的主要差别不在动作链，而在 reward
...
- 不仅 rollout 前会投影，`target-Q` �?`actor-loss` 也会使用 projector 处理 joint action
- `r_soc_pen` 在该方案下主要记�?`projected -> executed` 残差，因此更�?projector 健壮性诊断，而不�?base 方案里的原始申请惩罚
...
| --- | --- | --- | --- |
| rollout 前动作处�?| 本地 `SoC` 可行�?| 本地 `SoC` 可行�?| �?projector，再本地 `SoC` 可行�?|
...
| 电网安全信息进入优化 | 不进�?| 通过 reward 惩罚进入 | 通过 reward 惩罚和动作投影同时进�?|

## 对比与复现入�?

当前对比入口�?`notebooks/madrl/compare.ipynb`。它会在同一测试窗口下评估五个控制器�?
...
这份 README 主体只详细展开前三�?`MADRL` 方案；两�?`MPC` 基线主要作为统一测试窗口下的参考对象�?
...
- `configs/experiment_config.py`：默认配�?
- `envs/grid_env.py`：环境、动作映射、储能动力学
- `envs/rewards/NormalReward.py`：购电成本、售电补贴和电网安全惩罚
- `controllers/action_feasibility.py`：本�?`SoC` 可行化与 `r_soc_pen`
...
- 当前文档只描述现有主线代码，不再保留旧版 reward、旧�?notebook 或不存在的入口�?
- 所有数学公式均�?GitHub Markdown 可渲染的 `$...$` �?`$$...$$` 形式书写�?
- 如果你只想快速开始，请先确认 `data/processed/prosumer/` 下的数据已准备好，再按“复现顺序”依次运�?notebook�?

## Price Protocol

- wholesale_price is the forecast-layer wholesale electricity price.
- import_price is the settlement-layer import electricity price used by the environment, MPC, and cost accounting.
- The only valid conversion is import_price = wholesale_price + import_price_markup_eur_per_kwh.
- Bare price is only allowed at the external raw CSV boundary.
