...
from predictors.artifacts import DEFAULT_SUPPORTED_FORECAST_SIGNALS, get_default_lstm_artifact_dir, get_default_lstm_artifact_paths
from predictors.lstm_forecaster import BASELINE_MODE_LAST_VALUE, BASELINE_MODE_NONE, LSTMForecaster, LSTM_ARTIFACT_FORMAT, LSTM_LOAD_HYBRID_ARTIFACT_FORMAT, PHYSICAL_NORMALIZATION_LOAD_SCALE, PHYSICAL_NORMALIZATION_NONE, PHYSICAL_NORMALIZATION_PV_PEAK, POSTPROCESS_MODE_BASELINE_BLEND, POSTPROCESS_MODE_NONE, POSTPROCESS_MODE_PHYSICAL_CLIP, save_lstm_forecaster_artifacts
...
def _validate_choice(name: str, value: str | None, default: str, allowed: set[str]) -> str:
    ...
    if normalized not in allowed:
        ...
        raise ValueError(f"Unsupported {name} '{value}'. Only {allowed_text} are implemented.")
    return normalized
...
def _normalize_date_range(start_date, end_date) -> dict[str, str | None]:
    return {'start_date': None if start_date in (None, '') else str(start_date), 'end_date': None if end_date in (None, '') else str(end_date)}
...
def _resolve_signal_behavior(cfg, signal_name: str) -> dict[str, object]:
    normalized = _normalize_signal_name(signal_name)
    if normalized == 'load':
        postprocess_mode = _validate_choice('forecast.load_hybrid_mode', getattr(cfg.forecast, 'load_hybrid_mode', POSTPROCESS_MODE_BASELINE_BLEND), POSTPROCESS_MODE_BASELINE_BLEND, {POSTPROCESS_MODE_BASELINE_BLEND})
        return {'model_mode': _validate_choice('forecast.load_model_mode', getattr(cfg.forecast, 'load_model_mode', 'per_agent'), 'per_agent', {'per_agent'}), 'time_feature_mode': normalize_time_feature_mode(getattr(cfg.forecast, 'load_time_feature_mode', TIME_FEATURE_MODE_NONE)), 'postprocess_mode': postprocess_mode, 'baseline_mode': _validate_choice('forecast.load_baseline_mode', getattr(cfg.forecast, 'load_baseline_mode', BASELINE_MODE_LAST_VALUE), BASELINE_MODE_LAST_VALUE, {BASELINE_MODE_LAST_VALUE}), 'blend_candidates': _normalize_load_blend_candidates(getattr(cfg.forecast, 'load_blend_candidates', None)), 'scaler_type': str(getattr(cfg.forecast, 'load_scaler_type', 'standard')).strip().lower(), 'component_split': bool(getattr(cfg.forecast, 'load_component_split', False)), 'artifact_format': LSTM_LOAD_HYBRID_ARTIFACT_FORMAT if postprocess_mode == POSTPROCESS_MODE_BASELINE_BLEND else LSTM_ARTIFACT_FORMAT}
    if normalized == 'pv':
        return {'model_mode': 'shared', 'time_feature_mode': normalize_time_feature_mode(getattr(cfg.forecast, 'pv_time_feature_mode', TIME_FEATURE_MODE_HOUR_WEEK_YEAR)), 'postprocess_mode': _validate_choice('forecast.pv_postprocess_mode', getattr(cfg.forecast, 'pv_postprocess_mode', POSTPROCESS_MODE_PHYSICAL_CLIP), POSTPROCESS_MODE_PHYSICAL_CLIP, {POSTPROCESS_MODE_NONE, POSTPROCESS_MODE_PHYSICAL_CLIP}), 'baseline_mode': BASELINE_MODE_NONE, 'blend_candidates': (1.0,), 'scaler_type': 'standard', 'component_split': False, 'artifact_format': LSTM_ARTIFACT_FORMAT}
    return {'model_mode': 'shared', 'time_feature_mode': TIME_FEATURE_MODE_NONE, 'postprocess_mode': POSTPROCESS_MODE_NONE, 'baseline_mode': BASELINE_MODE_NONE, 'blend_candidates': (1.0,), 'scaler_type': 'standard', 'component_split': False, 'artifact_format': LSTM_ARTIFACT_FORMAT}
def build_lstm_source_signature(cfg, signal_name: str) -> dict[str, object]:
    ...
    if int(cfg.data.train_year) == int(cfg.data.test_year) and (not cfg.data.train_start_date) and (not cfg.data.train_end_date) and (cfg.data.test_start_date or cfg.data.test_end_date):
...
def _slice_scale_by_source_columns(values: Sequence[float] | np.ndarray | float | None, source: 'SignalCsvSource') -> np.ndarray | None:
    ...
    if any((index < 0 or index >= scale.size for index in indices)):
        raise IndexError(f'Signal source column_indices={indices} are out of bounds for scale size={scale.size}.')
...
def resolve_signal_physical_scale_from_source(source: SignalCsvSource, signal_name: str, *, split: str) -> np.ndarray | None:
    ...
    if mode == PHYSICAL_NORMALIZATION_LOAD_SCALE:
        return _coerce_physical_scale_by_column(_slice_scale_by_source_columns(dataset_kwargs.get('load_scale'), source), expected_size=len(source.value_columns))
...
def clone_config_with_signal_overrides(cfg, overrides: dict[str, object] | None=None):
    ...
    if unknown_keys:
        raise KeyError(f"Unknown signal training override keys: {unknown_keys}. Allowed keys: {sorted(SIGNAL_TRAINING_OVERRIDE_FIELDS)} + ['future_horizon', 'device'].")
    ...
    return cloned
def resolve_signal_training_settings(cfg, signal_name: str, overrides: dict[str, object] | None=None) -> tuple[object, dict[str, object]]:
    signal_name = _normalize_signal_name(signal_name)
    local_cfg = clone_config_with_signal_overrides(cfg, overrides=overrides)
    behavior = _resolve_signal_behavior(local_cfg, signal_name)
    settings = {'signal_name': signal_name, 'history_window': int(local_cfg.forecast.history_window), 'future_horizon': int(local_cfg.env.future_horizon), 'hidden_size': int(local_cfg.forecast.lstm_hidden_size), 'num_layers': int(local_cfg.forecast.lstm_num_layers), 'dropout': float(local_cfg.forecast.lstm_dropout), 'batch_size': int(local_cfg.forecast.lstm_batch_size), 'epochs': int(local_cfg.forecast.lstm_epochs), 'lr': float(local_cfg.forecast.lstm_lr), 'train_ratio': float(local_cfg.forecast.lstm_train_ratio), 'val_ratio': float(local_cfg.forecast.lstm_val_ratio), 'device': str(local_cfg.runtime.device), 'input_size': 1 + int(time_feature_dim(behavior['time_feature_mode'])), **behavior}
...
def _resolve_prosumer_dataset_kwargs(cfg, data_dir: Path, split: str) -> dict[str, object]:
    ...
    if len(agent_profiles) != n_agents:
        raise ValueError(f'Forecast prosumer config mismatch: cfg.env.num_agents={n_agents} but cfg.data.agent_profiles has {len(agent_profiles)} entry(ies). Keep forecast notebook/config values in sync, for example: cfg.env.num_agents = len(cfg.data.agent_profiles).')
    exclude_start_date = None
    exclude_end_date = None
    if split == 'train' and int(cfg.data.train_year) == int(cfg.data.test_year) and (not cfg.data.train_start_date) and (not cfg.data.train_end_date) and (cfg.data.test_start_date or cfg.data.test_end_date):
...
def _resolve_prosumer_signal_source(cfg, data_dir: Path, signal_name: str) -> SignalCsvSource | None:
    ...
    return SignalCsvSource(signal_name=signal_name, value_columns=value_columns, column_indices=tuple(range(len(value_columns))), source_kind='prosumer', dataset_kwargs={'train': _resolve_prosumer_dataset_kwargs(cfg, data_dir, 'train'), 'test': _resolve_prosumer_dataset_kwargs(cfg, data_dir, 'test')})
...
def _load_signal_matrix_from_source(source: SignalCsvSource, signal_name: str, *, split: str) -> tuple[pd.DataFrame, np.ndarray, tuple[str, ...]]:
...
def _load_signal_segment_frames_from_source(source: SignalCsvSource, signal_name: str, *, split: str, drop_warmup: bool=False) -> tuple[pd.DataFrame, list[pd.DataFrame], tuple[str, ...]]:
    ...
    if frame.empty:
        raise ValueError(f"Signal source split='{split}' does not contain any usable rows for signal '{signal_name}'.")
    working = frame.copy()
    ...
    if not segment_frames:
        raise ValueError(f"Signal source split='{split}' does not contain any segment frames for signal '{signal_name}'.")
    return (working.reset_index(drop=True), segment_frames, value_columns)
def _slice_copy(item, start: int, end: int):
    return item.iloc[start:end].copy() if isinstance(item, pd.DataFrame) else np.asarray(item[start:end], dtype=np.float32).copy()
def _temporal_split_items(items: Sequence[pd.DataFrame | np.ndarray], *, train_ratio: float=0.7, val_ratio: float=0.15, seq_len: int, pred_len: int) -> dict[str, object]:
    ...
    train_items: list[pd.DataFrame | np.ndarray] = []
    val_items: list[pd.DataFrame | np.ndarray] = []
    ...
    for segment_idx, item in enumerate(items):
        ...
        if train_end >= min_window:
            train_items.append(_slice_copy(item, 0, train_end))
        if val_end - train_end > 0 and val_end - val_context_start >= min_window:
            val_items.append(_slice_copy(item, val_context_start, val_end))
    ...
    if not val_items:
        fallback = max(train_items, key=len)
        fallback_tail = _slice_copy(fallback, max(0, len(fallback) - (2 * int(seq_len) + int(pred_len))), len(fallback))
...
def resolve_signal_csv_source(data_dir: str | Path, signal_name: str, cfg) -> SignalCsvSource | None:
    return _resolve_prosumer_signal_source(cfg, Path(data_dir), _normalize_signal_name(signal_name))
...
def fit_signal_scaler(values: np.ndarray | list[np.ndarray], *, physical_scale_by_column: Sequence[float] | np.ndarray | float | None=None, scaler_type: str='standard') -> StandardScaler | RobustScaler:
    if isinstance(values, list):
        flattened = np.concatenate([_apply_physical_normalization(np.asarray(chunk, dtype=np.float32), physical_scale_by_column).reshape(-1) for chunk in values], axis=0)
    else:
        flattened = _apply_physical_normalization(np.asarray(values, dtype=np.float32), physical_scale_by_column).reshape(-1)
...
def train_lstm_model(model, train_loader: DataLoader, val_loader: DataLoader, *, epochs: int, lr: float, device: str | torch.device | TorchRuntimeState, show_progress: bool=False, progress_label: str | None=None) -> dict[str, object]:
    ...
    progress = tqdm(epoch_iterator, desc=progress_label or 'forecast epochs', leave=False, disable=not show_progress)
    try:
        for _ in progress:
            model.train()
            train_losses = []
            for batch_x, batch_y in train_loader:
                ...
                autocast_context = torch.autocast(device_type='cuda', dtype=torch.bfloat16) if use_amp else nullcontext()
            ...
            if show_progress:
                progress.set_postfix({'train': f'{mean_train:.4f}', 'val': f'{mean_val:.4f}'})
    ...
    return {'model': model, 'history': history, 'best_val_loss': float(best_val_loss), 'best_state_dict': best_state_dict, 'runtime': runtime_state}
...
def _predict_supervised_windows(model, windows: np.ndarray, *, runtime_state: TorchRuntimeState, batch_size: int) -> np.ndarray:
    ...
    with torch.no_grad():
        for start in range(0, len(windows), max(int(batch_size), 1)):
            batch = torch.tensor(windows[start:start + max(int(batch_size), 1)], dtype=torch.float32, device=runtime_state.device)
...
def _build_validation_window_timestamps(segment_frames: Sequence[pd.DataFrame], *, seq_len: int, pred_len: int) -> pd.DatetimeIndex:
    total_window = int(seq_len) + int(pred_len)
    timestamps = [pd.DatetimeIndex(pd.to_datetime(frame['timestamp'].iloc[int(seq_len):int(seq_len) + int(len(frame) - total_window + 1)].tolist(), utc=True)) for frame in (segment_frame.reset_index(drop=True) for segment_frame in segment_frames) if len(frame) >= total_window]
    return pd.DatetimeIndex([]) if not timestamps else timestamps[0].append(timestamps[1:])
def _select_week_evaluation_slice(frame: pd.DataFrame, values: np.ndarray, *, seq_len: int) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    if 'week_id' in frame.columns and 'is_warmup' in frame.columns:
        ...
        if not non_warmup.empty:
            ...
            segment_mask = np.ones(len(frame), dtype=bool) if 'segment_id' not in frame.columns else frame['segment_id'].astype(int).to_numpy() == int(first_target_row['segment_id'])
            warmup_mask = segment_mask & frame['is_warmup'].astype(bool).to_numpy()
            target_mask = segment_mask & ~frame['is_warmup'].astype(bool).to_numpy() & (frame['week_id'].astype(int).to_numpy() == target_week_id)
            history_values, target_values = values[warmup_mask], values[target_mask]
    ...
    timestamps = frame.loc[seq_len:seq_len + len(target) - 1, 'timestamp'].to_numpy() if 'timestamp' in frame.columns else np.arange(len(target))
    return (history, np.asarray(target), np.asarray(history_timestamps), np.asarray(timestamps))
def compute_forecast_metrics(target: np.ndarray, prediction: np.ndarray) -> dict[str, float | None]:
    ...
    if target_array.shape != prediction_array.shape:
        raise ValueError(f'Forecast metric computation requires target/prediction to share the same shape, got {target_array.shape} vs {prediction_array.shape}.')
    error = prediction_array.reshape(-1) - target_array.reshape(-1)
    rmse = float(np.sqrt(np.mean(np.square(error))))
    mae = float(np.mean(np.abs(error)))
    non_zero_mask = np.abs(target_array.reshape(-1)) > 1e-08
    mape = None if not np.any(non_zero_mask) else float(np.mean(np.abs(error[non_zero_mask] / target_array.reshape(-1)[non_zero_mask])) * 100.0)
...
def _select_load_blend_weight_from_validation(*, model, x_val: np.ndarray, y_val: np.ndarray, scaler: object | None, physical_scale: float | None, runtime_state: TorchRuntimeState, batch_size: int, candidate_weights: Sequence[float], component: str | None=None, window_timestamps: pd.DatetimeIndex | None=None) -> dict[str, object]:
    raw_predictions = _predict_supervised_windows(model, x_val, runtime_state=runtime_state, batch_size=batch_size)
    raw_step1 = _restore_supervised_window_values(raw_predictions[:, 0], scaler=scaler, physical_scale=physical_scale)
    baseline_step1 = _restore_supervised_window_values(x_val[:, -1, 0], scaler=scaler, physical_scale=physical_scale)
    target_step1 = _restore_supervised_window_values(y_val[:, 0], scaler=scaler, physical_scale=physical_scale)
    best_weight = float(candidate_weights[0])
    best_mae = float('inf')
    optimized_metric = 'mae_step1'
    bias_guard_satisfied: bool | None = None
    if str(component or '').strip().lower() == 'heatpump' and window_timestamps is not None and (len(window_timestamps) == len(target_step1)):
        selection = _select_heatpump_blocked_blend_weight(target_step1=target_step1, baseline_step1=baseline_step1, raw_step1=raw_step1, window_timestamps=pd.DatetimeIndex(window_timestamps), candidate_weights=tuple((float(weight) for weight in candidate_weights)))
    ...
    return {'postprocess_mode': POSTPROCESS_MODE_BASELINE_BLEND, 'baseline_mode': BASELINE_MODE_LAST_VALUE, 'blend_weight': best_weight, 'optimized_metric': optimized_metric, 'bias_guard_satisfied': bias_guard_satisfied}
def _select_heatpump_blocked_blend_weight(*, target_step1: np.ndarray, baseline_step1: np.ndarray, raw_step1: np.ndarray, window_timestamps: pd.DatetimeIndex, candidate_weights: Sequence[float], bias_threshold_kw: float=HEATPUMP_BLOCKED_BIAS_GUARD_KW) -> dict[str, object]:
    if len(window_timestamps) != len(target_step1):
        raise ValueError(f'window_timestamps must align with target_step1 for heatpump blocked blend selection, got len(window_timestamps)={len(window_timestamps)} vs len(target_step1)={len(target_step1)}.')
    base_frame = pd.DataFrame({'timestamp': pd.DatetimeIndex(window_timestamps), 'target': np.asarray(target_step1, dtype=np.float32), 'baseline': np.asarray(baseline_step1, dtype=np.float32), 'raw': np.asarray(raw_step1, dtype=np.float32)})
    ...
    for candidate_weight in candidate_weights:
        ...
        for _, months in HEATPUMP_BLOCKED_MONTH_GROUPS:
            ...
            if block.empty:
                continue
            blended = block['baseline'].to_numpy(dtype=np.float32) + np.float32(candidate_weight) * (block['raw'].to_numpy(dtype=np.float32) - block['baseline'].to_numpy(dtype=np.float32))
    ...
    if viable.empty:
        ranked = candidate_frame.sort_values(['max_abs_block_bias', 'avg_abs_block_bias', 'avg_block_mae', 'candidate_weight']).reset_index(drop=True)
        satisfied = False
    else:
        ranked = viable.sort_values(['avg_block_mae', 'avg_abs_block_bias', 'candidate_weight']).reset_index(drop=True)
        satisfied = True
    selected = ranked.iloc[0]
    return {'selected_weight': float(selected['candidate_weight']), 'bias_guard_satisfied': bool(satisfied)}
def evaluate_signal_one_week(source: SignalCsvSource, model, *, mode: str, **kwargs):
    signal_name, scaler = kwargs['signal_name'], kwargs['scaler']
    test_frame, test_values, _ = _load_signal_matrix_from_source(source, signal_name, split='test')
    history_seed, target_values, history_timestamps, timestamps = _select_week_evaluation_slice(test_frame, test_values, seq_len=kwargs['seq_len'])
    forecaster = LSTMForecaster(model_path=None, hidden_size=kwargs['hidden_size'], num_layers=kwargs['num_layers'], dropout=kwargs['dropout'], pred_len=kwargs['pred_len'], seq_len=kwargs['seq_len'], device=kwargs['device'], scaler=scaler, input_size=kwargs.get('input_size', 1), time_feature_mode=kwargs.get('time_feature_mode', TIME_FEATURE_MODE_NONE), model_mode=kwargs.get('model_mode', 'shared'), physical_normalization_mode=kwargs.get('physical_normalization_mode', PHYSICAL_NORMALIZATION_NONE), physical_scale_by_column=kwargs.get('physical_scale_by_column'), postprocess_mode=kwargs.get('postprocess_mode', POSTPROCESS_MODE_NONE), baseline_mode=kwargs.get('baseline_mode', BASELINE_MODE_NONE), blend_weight=kwargs.get('blend_weight'), optimized_metric=kwargs.get('optimized_metric')).rename_default_signal(signal_name)
    runtime = forecaster.signal_runtimes[signal_name][0]
    runtime.model.load_state_dict(copy.deepcopy(model.state_dict()))
    runtime.model.eval()
    runtime.agent_index = None if kwargs.get('agent_index') is None else int(kwargs['agent_index'])
    runtime.agent_profile = None if kwargs.get('agent_profile') is None else str(kwargs['agent_profile'])
    target_matrix = _reshape_signal_values(target_values)
    target = target_values.reshape(-1).astype(np.float32) if target_matrix.shape[1] == 1 else target_matrix.T.astype(np.float32)
    if mode == 'open_loop':
        rollout = forecaster.predict(history_seed, horizon=target_values.shape[0] + 1, signal_name=signal_name, history_timestamps=history_timestamps)
        prediction = np.asarray(rollout, dtype=np.float32)[1:target_values.shape[0] + 1] if target_matrix.shape[1] == 1 else np.asarray(rollout, dtype=np.float32)[:, 1:target_values.shape[0] + 1]
        ...
        for step_idx in range(target_matrix.shape[0]):
            current_history = history_matrix if step_idx == 0 else np.concatenate([history_matrix, target_matrix[:step_idx]], axis=0)
            current_timestamps = np.asarray(history_timestamps) if step_idx == 0 else np.concatenate([np.asarray(history_timestamps), np.asarray(timestamps[:step_idx])], axis=0)
            rollout = forecaster.predict(current_history.reshape(-1) if current_history.ndim == 2 and current_history.shape[1] == 1 else current_history, horizon=max(int(kwargs['pred_len']) + 1, 2), signal_name=signal_name, history_timestamps=current_timestamps)
            rollout_array = np.asarray(rollout, dtype=np.float32)
            one_step_predictions.append(np.float32(rollout_array[1]) if target_matrix.shape[1] == 1 else rollout_array[:, 1].astype(np.float32))
    ...
    return SignalForecastEvaluation(signal_name=signal_name, evaluation_mode=mode, timestamps=np.asarray(timestamps), target=target, prediction=prediction, metrics=compute_forecast_metrics(target, prediction), source_columns=source.value_columns)
def _extract_segment_value_arrays(segment_frames: list[pd.DataFrame], value_columns: Sequence[str]) -> list[np.ndarray]:
...
def _aggregate_per_agent_evaluations(signal_name: str, evaluation_mode: str, evaluations: Sequence[SignalForecastEvaluation]) -> SignalForecastEvaluation:
    ...
    for evaluation in evaluations:
        ...
        if evaluation_timestamps.shape != base_timestamps.shape or not np.array_equal(evaluation_timestamps, base_timestamps):
        ...
        if target_array.ndim == 1:
            ...
            raise ValueError('Per-agent aggregation expects 1D evaluations or 2D evaluations with one source column.')
    ...
    return SignalForecastEvaluation(signal_name=signal_name, evaluation_mode=evaluation_mode, timestamps=base_timestamps, target=target, prediction=prediction, metrics=compute_forecast_metrics(target, prediction), source_columns=tuple(source_columns))
def _aggregate_training_history(agent_results: Sequence[dict[str, object]]) -> dict[str, list[float]]:
    if not agent_results:
        return {'train_loss': [], 'val_loss': []}
    train_curves = [np.asarray(result['training']['history']['train_loss'], dtype=np.float32) for result in agent_results]
    val_curves = [np.asarray(result['training']['history']['val_loss'], dtype=np.float32) for result in agent_results]
    return {'train_loss': np.mean(np.stack(train_curves, axis=0), axis=0).astype(np.float32).tolist(), 'val_loss': np.mean(np.stack(val_curves, axis=0), axis=0).astype(np.float32).tolist()}
def _summarize_per_agent_train_stats(agent_results: Sequence[dict[str, object]]) -> dict[str, object]:
    stats_by_agent = {str(result.get('agent_profile', result['columns'][0])): dict(result['train_stats']) for result in agent_results}
    return {'agent_stats': stats_by_agent, 'min': float(min((stat['min'] for stat in stats_by_agent.values()))), 'max': float(max((stat['max'] for stat in stats_by_agent.values()))), 'mean': float(np.mean([stat['mean'] for stat in stats_by_agent.values()])), 'std': float(np.mean([stat['std'] for stat in stats_by_agent.values()]))}
def _print_signal_evaluation_summary(signal_name: str, online_evaluation: SignalForecastEvaluation, open_loop_evaluation: SignalForecastEvaluation) -> None:
    print(f"[forecast] {signal_name} metrics: online_rmse={online_evaluation.metrics['rmse']:.6f}, online_mae={online_evaluation.metrics['mae']:.6f}, open_loop_rmse={open_loop_evaluation.metrics['rmse']:.6f}, open_loop_mae={open_loop_evaluation.metrics['mae']:.6f}")
def _build_signal_training_specs(local_cfg, signal_name: str, *, source: SignalCsvSource, settings: dict[str, object], data_dir: Path) -> list[dict[str, object]]:
    if signal_name != 'load' or str(settings['model_mode']) != 'per_agent':
        return [{'source': source, 'agent_index': None, 'agent_profile': None, 'component': None}]
    profiles = [str(profile) for profile in local_cfg.data.agent_profiles][:len(source.value_columns)]
    components = list(local_cfg.data.load_components) if bool(settings.get('component_split', False)) else [None]
    ...
    for agent_index, agent_profile in enumerate(profiles):
        for component in components:
            ...
            if component is not None:
                ...
                selected_source = resolve_signal_csv_source(data_dir, comp_signal, cfg=local_cfg)
...
def _train_single_signal_lstm(local_cfg, signal_name: str, *, source: SignalCsvSource, settings: dict[str, object], runtime_state: TorchRuntimeState, overrides: dict[str, object] | None=None, show_progress: bool=False, agent_index: int | None=None, agent_profile: str | None=None, component: str | None=None) -> dict[str, object]:
    ...
    online_evaluation = evaluate_signal_one_week(source, result['model'], mode='online_aligned', **evaluation_kwargs)
    open_loop_evaluation = evaluate_signal_one_week(source, result['model'], mode='open_loop', **evaluation_kwargs)
...
def train_signal_lstm(cfg, signal_name: str, *, device: str | torch.device | TorchRuntimeState | None=None, overrides: dict[str, object] | None=None, show_progress: bool=False) -> dict[str, object]:
    ...
    runtime_state = configure_torch_runtime(local_cfg, device=local_cfg.runtime.device if device is None else device, seed=local_cfg.runtime.seed)
    signal_name = _normalize_signal_name(signal_name)
    source = resolve_signal_csv_source(data_dir, signal_name, cfg=local_cfg)
    if source is None:
        raise FileNotFoundError(f"No train/test source found for forecast signal '{signal_name}' under '{data_dir}'.")
    if int(local_cfg.env.future_horizon) <= 0:
        raise ValueError('LSTM forecast training requires cfg.env.future_horizon > 0.')
    specs = _build_signal_training_specs(local_cfg, signal_name, source=source, settings=settings, data_dir=data_dir)
    results = [_train_single_signal_lstm(local_cfg, signal_name, settings=settings, runtime_state=runtime_state, overrides=overrides, show_progress=show_progress, **spec) for spec in specs]
    if len(results) == 1 and results[0].get('agent_index') is None and results[0].get('component') is None:
        return results[0]
    online_evaluation = _aggregate_per_agent_evaluations(signal_name, 'online_aligned', [result['online_evaluation'] for result in results])
    open_loop_evaluation = _aggregate_per_agent_evaluations(signal_name, 'open_loop', [result['open_loop_evaluation'] for result in results])
    return {'signal_name': signal_name, 'source': source, 'columns': tuple(source.value_columns), 'segment_summary': {str(result['agent_profile']): result['segment_summary'] for result in results}, 'train_stats': _summarize_per_agent_train_stats(results), 'artifact_paths': [result['artifact_paths'] for result in results], 'training': {'history': _aggregate_training_history(results), 'best_val_loss': float(np.mean([result['training']['best_val_loss'] for result in results], dtype=np.float32))}, 'evaluation': online_evaluation, 'online_evaluation': online_evaluation, 'open_loop_evaluation': open_loop_evaluation, 'settings': settings, 'runtime': runtime_state, 'agent_results': results, 'hybrid': {str(result['agent_profile']): dict(result['hybrid']) for result in results}}
def _expected_signal_optimized_metric(signal_name: str, *, postprocess_mode: str, component: str | None=None) -> str | None:
    normalized_signal = _normalize_signal_name(signal_name)
    if normalized_signal != 'load' or str(postprocess_mode) != POSTPROCESS_MODE_BASELINE_BLEND:
        return None
    return 'blocked_bias_guard_step1' if str(component or '').strip().lower() == 'heatpump' else 'mae_step1'
def expected_lstm_artifact_meta(cfg, signal_name: str, overrides: dict[str, object] | None=None, *, agent_index: int | None=None, agent_profile: str | None=None, component: str | None=None) -> dict[str, object]:
    ...
    if expected['artifact_format'] == LSTM_LOAD_HYBRID_ARTIFACT_FORMAT or str(settings['postprocess_mode']) != POSTPROCESS_MODE_NONE:
        expected.update({'postprocess_mode': str(settings['postprocess_mode']), 'baseline_mode': str(settings['baseline_mode']), 'blend_weight': None, 'optimized_metric': _expected_signal_optimized_metric(signal_name, postprocess_mode=str(settings['postprocess_mode']), component=component)})
    return expected
def compare_lstm_artifact_meta(actual_meta: dict[str, object] | None, expected_meta: dict[str, object]) -> dict[str, object]:
    ...
    for field in comparable_fields:
        ...
        matches = actual_value == expected_value if field not in {'dropout', 'blend_weight'} else actual_value is not None and bool(np.isclose(float(actual_value), float(expected_value))) if field == 'dropout' else field in actual_meta and (actual_value is None or 0.0 <= float(actual_value) <= 1.0)
        if not matches:
...
def validate_lstm_artifact(cfg, signal_name: str, paths: dict[str, str | Path], *, overrides: dict[str, object] | None=None, agent_index: int | None=None, agent_profile: str | None=None, component: str | None=None) -> dict[str, object]:
    normalized_signal = _normalize_signal_name(signal_name)
    expected = expected_lstm_artifact_meta(cfg, normalized_signal, overrides=overrides, agent_index=agent_index, agent_profile=agent_profile, component=component)
    resolved_paths = {name: Path(path) for name, path in paths.items()}
    required_files = {key: resolved_paths[key] for key in ('model_path', 'meta_path', 'scaler_path') if key in resolved_paths}
    ...
    return result
def _managed_lstm_artifact_paths(cfg, signal_name: str, *, agent_index: int | None=None, component: str | None=None) -> dict[str, Path]:
    artifact_root = cfg.forecast.lstm_artifact_root
    return get_default_lstm_artifact_paths(root=get_default_lstm_artifact_dir() if artifact_root in (None, '') else Path(artifact_root), signal_name=_normalize_signal_name(signal_name), future_horizon=int(cfg.env.future_horizon), agent_index=agent_index, component=component)
def _format_lstm_artifact_issue(validation: dict[str, object]) -> str:
    ...
    if issue_type in {'missing', 'incomplete'}:
        return f"signal='{signal_name}', artifact='{artifact_path}', {'artifact is incomplete' if issue_type == 'incomplete' else 'missing files'}: {', '.join(validation.get('missing_files', []))}. Delete stale artifacts or enable cfg.forecast.auto_train_missing=True to rebuild them automatically."
    if issue_type == 'invalid_meta':
        return f"signal='{signal_name}', artifact='{artifact_path}', {validation.get('error', 'meta is unreadable')}. Delete the stale artifact or enable cfg.forecast.auto_train_missing=True to rebuild it automatically."
    mismatch_text = ', '.join((f"{field}(expected={detail['expected']}, actual={detail['actual']})" for field, detail in sorted(validation.get('mismatches', {}).items())))
...
def _signal_artifact_specs(cfg, signal_name: str) -> list[dict[str, object]]:
    normalized_signal = _normalize_signal_name(signal_name)
    behavior = _resolve_signal_behavior(cfg, normalized_signal)
    if normalized_signal == 'load' and str(behavior['model_mode']) == 'per_agent':
        profiles = [str(profile) for profile in cfg.data.agent_profiles][:int(cfg.env.num_agents)]
        components = list(cfg.data.load_components) if bool(behavior['component_split']) else [None]
        return [{'signal_name': normalized_signal, 'agent_index': int(agent_index), 'agent_profile': profile, 'component': component, 'paths': _managed_lstm_artifact_paths(cfg, normalized_signal, agent_index=agent_index, component=component)} for agent_index, profile in enumerate(profiles) for component in components]
...
def _collect_lstm_artifact_inventory(cfg, *, overrides_by_signal: dict[str, dict[str, object]] | None=None) -> dict[str, object]:
    resolved_overrides_by_signal = dict(getattr(cfg.forecast, 'signal_training_overrides', {}) if overrides_by_signal is None else overrides_by_signal)
    ...
    for signal_name in configured_forecast_signals(cfg):
        compatible_artifacts, signal_missing, signal_invalid = [], [], []
        ...
        for spec in _signal_artifact_specs(cfg, signal_name):
            ...
            if validation['compatible']:
                compatible_artifacts.append((str(spec['paths']['model_path']), str(spec['paths']['meta_path']), str(spec['paths']['scaler_path'])))
            elif validation.get('issue_type') == 'missing':
                signal_missing.append(validation)
            else:
                signal_invalid.append(validation)
        if compatible_artifacts and not signal_missing and not signal_invalid:
            artifacts[signal_name] = compatible_artifacts if len(compatible_artifacts) > 1 else compatible_artifacts[0]
        if signal_missing:
...
def _raise_lstm_artifact_requirements_error(cfg, inventory: dict[str, object]) -> None:
    ...
    raise (RuntimeError if mismatch_count and missing_count else ValueError if mismatch_count else FileNotFoundError)('Managed LSTM forecast artifacts are missing or incompatible:\n' + '\n'.join(problem_lines))
def collect_available_lstm_artifacts(cfg, *, overrides_by_signal: dict[str, dict[str, object]] | None=None) -> dict[str, object]:
    return dict(_collect_lstm_artifact_inventory(cfg, overrides_by_signal=getattr(cfg.forecast, 'signal_training_overrides', {}) if overrides_by_signal is None else overrides_by_signal)['artifacts'])
def ensure_lstm_artifacts(cfg, *, device: str | torch.device | None=None, overrides_by_signal: dict[str, dict[str, object]] | None=None) -> dict[str, object]:
    artifact_root = cfg.forecast.lstm_artifact_root
    artifact_root = get_default_lstm_artifact_dir() if artifact_root in (None, '') else Path(artifact_root)
    artifact_root.mkdir(parents=True, exist_ok=True)
    resolved_overrides_by_signal = getattr(cfg.forecast, 'signal_training_overrides', {}) if overrides_by_signal is None else overrides_by_signal
    inventory_before = _collect_lstm_artifact_inventory(cfg, overrides_by_signal=resolved_overrides_by_signal)
    if not bool(cfg.forecast.auto_train_missing) and (inventory_before['missing_signals'] or inventory_before['invalid_artifacts']):
        _raise_lstm_artifact_requirements_error(cfg, inventory_before)
    trained_results, trained_signals, retrained_signals = [], [], []
    ...
    for signal_name in configured_forecast_signals(cfg):
        ...
        if resolve_signal_csv_source(data_dir, signal_name, cfg=cfg) is None:
        ...
        trained_results.append(train_signal_lstm(cfg, signal_name, device=device, overrides=dict(resolved_overrides_by_signal.get(signal_name) or {}) or None))
        (retrained_signals if invalid_validations else trained_signals).append(signal_name)
    inventory_after = _collect_lstm_artifact_inventory(cfg, overrides_by_signal=resolved_overrides_by_signal)
    if not bool(cfg.forecast.auto_train_missing) and (inventory_after['missing_signals'] or inventory_after['invalid_artifacts']):
