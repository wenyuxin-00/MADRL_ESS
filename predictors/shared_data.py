...
_FLOAT_PRECISION,_SCHEMA_VERSION=6,8
SHARED_DATA_SCHEMA_VERSION=_SCHEMA_VERSION
PRICE_OBSERVATION_CONTRACT='oracle_window_relative_price_v1'
WHOLESALE_PRICE_RANK_SEQ_FIELD='wholesale_price_rank_seq'
WHOLESALE_PRICE_RELATIVE_SEQ_FIELD='wholesale_price_relative_seq'
WHOLESALE_PRICE_SPREAD_SEQ_FIELD='wholesale_price_spread_seq'
...
def _shared_data_signature_payload(cfg,*,artifact_fingerprint:dict[str,object])->dict[str,object]:return{'schema_version':_SCHEMA_VERSION,'price_observation_contract':PRICE_OBSERVATION_CONTRACT,'num_agents':int(cfg.env.num_agents),'episode_limit':int(cfg.env.episode_limit),'train_episode_limit':int(resolve_train_episode_limit(cfg)),'test_episode_limit':int(resolve_test_episode_limit(cfg)),'train_window_days':int(getattr(cfg.env,'train_window_days',1)),'window_stride_days':int(getattr(cfg.env,'window_stride_days',1)),'future_horizon':int(cfg.env.future_horizon),'agent_profiles':list(cfg.data.agent_profiles),'agent_bus_ids':list(cfg.grid.agent_bus_ids),'train_year':int(cfg.data.train_year),'test_year':int(cfg.data.test_year),'train_start_date':cfg.data.train_start_date,'train_end_date':cfg.data.train_end_date,'same_year_has_explicit_train_range':bool(_same_year_has_explicit_train_range(cfg)),'test_manifest_scope':'full_year','load_components':list(cfg.data.load_components),'pv_reference':str(cfg.data.pv_reference),'pv_capacity_kw':list(cfg.data.pv_capacity_kw),'load_scale':list(cfg.data.load_scale),'pv_scale':list(cfg.data.pv_scale),'wholesale_price_spread_scale_eur_per_kwh':float(getattr(cfg.obs,'wholesale_price_spread_scale_eur_per_kwh',.20)),'price_protocol_version':int(PRICE_PROTOCOL_VERSION),'artifacts':artifact_fingerprint}
...
def _relative_price_sequence_matrix(values:np.ndarray)->np.ndarray:
	array=np.asarray(values,dtype=np.float32)
	if array.ndim==1:array=array.reshape(1,-1)
	if array.ndim!=2:raise ValueError(f"Expected 1D or 2D price sequence values for relative feature, got shape {array.shape}.")
	if int(array.shape[-1])<=0:return np.zeros_like(array,dtype=np.float32)
	minimum=np.min(array,axis=-1,keepdims=True);spread=np.max(array,axis=-1,keepdims=True)-minimum;scaled=2.*(array-minimum)/np.maximum(spread,np.float32(1e-06))-1.;return np.where(spread>np.float32(1e-06),scaled,np.zeros_like(array,dtype=np.float32)).astype(np.float32)
def _spread_price_sequence_matrix(values:np.ndarray,scale_eur_per_kwh:float)->np.ndarray:
	array=np.asarray(values,dtype=np.float32)
	if array.ndim==1:array=array.reshape(1,-1)
	if array.ndim!=2:raise ValueError(f"Expected 1D or 2D price sequence values for spread feature, got shape {array.shape}.")
	if int(array.shape[-1])<=0:return np.zeros_like(array,dtype=np.float32)
	spread=np.max(array,axis=-1,keepdims=True)-np.min(array,axis=-1,keepdims=True);scale=max(float(scale_eur_per_kwh),1e-06);value=np.clip(spread/np.float32(scale),.0,1.).astype(np.float32);return np.broadcast_to(value,array.shape).astype(np.float32,copy=False)
...
def _open_split_arrays(split_dir:Path,*,num_timestamps:int,n_agents:int,sequence_length:int)->tuple[dict[str,str],dict[str,np.ndarray]]:files={'calendar_time':'calendar_time.npy',WHOLESALE_PRICE_SEQ_FIELD:f"{WHOLESALE_PRICE_SEQ_FIELD}.npy",WHOLESALE_PRICE_RANK_SEQ_FIELD:f"{WHOLESALE_PRICE_RANK_SEQ_FIELD}.npy",WHOLESALE_PRICE_RELATIVE_SEQ_FIELD:f"{WHOLESALE_PRICE_RELATIVE_SEQ_FIELD}.npy",WHOLESALE_PRICE_SPREAD_SEQ_FIELD:f"{WHOLESALE_PRICE_SPREAD_SEQ_FIELD}.npy",'load_seq':'load_seq.npy','pv_seq':'pv_seq.npy','mu_t':'mu_t.npy','mu_next':'mu_next.npy'};shapes={'calendar_time':(num_timestamps,n_agents,4),WHOLESALE_PRICE_SEQ_FIELD:(num_timestamps,sequence_length),WHOLESALE_PRICE_RANK_SEQ_FIELD:(num_timestamps,sequence_length),WHOLESALE_PRICE_RELATIVE_SEQ_FIELD:(num_timestamps,sequence_length),WHOLESALE_PRICE_SPREAD_SEQ_FIELD:(num_timestamps,sequence_length),'load_seq':(num_timestamps,n_agents,sequence_length),'pv_seq':(num_timestamps,n_agents,sequence_length),'mu_t':(num_timestamps,),'mu_next':(num_timestamps,)};arrays={name:open_memmap(split_dir/file_name,mode='w+',dtype=np.float32,shape=shapes[name])for(name,file_name)in files.items()};return files,arrays
...
def _write_window_relative_price_cache(price_seq_array:np.ndarray,relative_array:np.ndarray,spread_array:np.ndarray,*,split:str,spread_scale_eur_per_kwh:float)->None:
	num_timestamps=int(price_seq_array.shape[0]);row_chunk_size=max(int(_SHARED_DATA_FORECAST_ROW_CHUNK_SIZE),1)
	for start in tqdm(range(0,num_timestamps,row_chunk_size),desc=f"shared_data[{split}] relative prices",unit='chunk',leave=True):
		end=min(start+row_chunk_size,num_timestamps);chunk=price_seq_array[start:end];relative_array[start:end]=_relative_price_sequence_matrix(chunk);spread_array[start:end]=_spread_price_sequence_matrix(chunk,spread_scale_eur_per_kwh)
def _write_split_shared_data(cfg,*,split:str,split_dir:Path)->dict[str,object]:
	...
	try:
		...
		if vectorized:
			...
			_write_window_relative_price_cache(arrays[WHOLESALE_PRICE_SEQ_FIELD],arrays[WHOLESALE_PRICE_RELATIVE_SEQ_FIELD],arrays[WHOLESALE_PRICE_SPREAD_SEQ_FIELD],split=str(split),spread_scale_eur_per_kwh=float(getattr(cfg.obs,'wholesale_price_spread_scale_eur_per_kwh',.20)))
		else:
	...
	manifest={'schema_version':_SCHEMA_VERSION,'cache_layout':SHARED_DATA_CACHE_LAYOUT,'price_protocol_version':int(PRICE_PROTOCOL_VERSION),'price_observation_contract':PRICE_OBSERVATION_CONTRACT,'split':str(split),'num_episodes':n_episodes,'num_timestamps':num_timestamps,'episode_length':episode_length,'num_agents':n_agents,'sequence_length':sequence_length,'split_controls':split_controls,'dropped_tail_steps':int(getattr(dataset,'dropped_tail_steps',0)),'files':files,'episodes':episode_manifest};(split_dir/'manifest.json').write_text(json.dumps(manifest,indent=2,default=_json_default),encoding='utf-8');return manifest
...
class PrecomputedObservationStore:
	def __init__(self,split_dir:str|Path):
		...
		missing_price_files=sorted({WHOLESALE_PRICE_RANK_SEQ_FIELD,WHOLESALE_PRICE_RELATIVE_SEQ_FIELD,WHOLESALE_PRICE_SPREAD_SEQ_FIELD}-set(dict(self.manifest.get('files',{}))))
...
def validate_madrl_shared_data_runtime_contract(cfg,*,shared_data_dir:str|Path,split_dir:str|Path,split_manifest:dict[str,object])->dict[str,object]:
	...
	if str(root_manifest.get('price_observation_contract',''))!=PRICE_OBSERVATION_CONTRACT:mismatches.append(f"old shared-data root manifest '{root_manifest_path}' declares price_observation_contract={root_manifest.get('price_observation_contract')!r}, but the new contract expects '{PRICE_OBSERVATION_CONTRACT}'")
	if str(split_manifest.get('price_observation_contract',''))!=PRICE_OBSERVATION_CONTRACT:mismatches.append(f"old split manifest '{split_manifest_path}' declares price_observation_contract={split_manifest.get('price_observation_contract')!r}, but the new contract expects '{PRICE_OBSERVATION_CONTRACT}'")
	missing_price_files=sorted({WHOLESALE_PRICE_RANK_SEQ_FIELD,WHOLESALE_PRICE_RELATIVE_SEQ_FIELD,WHOLESALE_PRICE_SPREAD_SEQ_FIELD}-set(dict(split_manifest.get('files',{}))))
	if missing_price_files:mismatches.append(f"old split manifest '{split_manifest_path}' is missing {missing_price_files}, but the new contract requires '{PRICE_OBSERVATION_CONTRACT}' window-relative wholesale price observations")
...
def ensure_madrl_shared_data(cfg,*,root:str|Path|None=None)->SharedDataResult:
	...
	try:
		temp_dir.mkdir(parents=True,exist_ok=False);train_manifest,test_manifest=_write_split_shared_data(cfg,split='train',split_dir=temp_dir/'train'),_write_split_shared_data(cfg,split='test',split_dir=temp_dir/'test');manifest={'schema_version':_SCHEMA_VERSION,'cache_layout':SHARED_DATA_CACHE_LAYOUT,'price_protocol_version':int(PRICE_PROTOCOL_VERSION),'price_observation_contract':PRICE_OBSERVATION_CONTRACT,'signature_hash':signature_hash,'signature':_normalize_for_signature(signature_payload),'created_at':datetime.now(timezone.utc).isoformat(timespec='seconds'),'shared_data_dir':str(shared_dir),'artifact_inventory':dict(artifact_info['fingerprint']),'data_controls':{'agent_profiles':list(cfg.data.agent_profiles),'agent_bus_ids':list(cfg.grid.agent_bus_ids),'train_year':int(cfg.data.train_year),'test_year':int(cfg.data.test_year),'train_start_date':cfg.data.train_start_date,'train_end_date':cfg.data.train_end_date,'same_year_has_explicit_train_range':bool(_same_year_has_explicit_train_range(cfg)),'test_manifest_scope':'full_year','test_selection_contract':'runtime_date_selection','train_window_days':int(getattr(cfg.env,'train_window_days',1)),'window_stride_days':int(getattr(cfg.env,'window_stride_days',1)),'train_episode_limit':int(resolve_train_episode_limit(cfg)),'test_episode_limit':int(resolve_test_episode_limit(cfg)),'load_scale':_normalize_for_signature(list(cfg.data.load_scale)),'pv_scale':_normalize_for_signature(list(cfg.data.pv_scale)),'pv_capacity_kw':_normalize_for_signature(list(cfg.data.pv_capacity_kw)),'load_components':list(cfg.data.load_components),'pv_reference':str(cfg.data.pv_reference),'future_horizon':int(cfg.env.future_horizon),'episode_limit':int(cfg.env.episode_limit),'price_observation_contract':PRICE_OBSERVATION_CONTRACT,'wholesale_price_spread_scale_eur_per_kwh':float(getattr(cfg.obs,'wholesale_price_spread_scale_eur_per_kwh',.20))},'splits':{'train':train_manifest,'test':test_manifest}};(temp_dir/'manifest.json').write_text(json.dumps(manifest,indent=2,default=_json_default),encoding='utf-8')
