...
def _shared_data_signature_payload(cfg,*,artifact_fingerprint:dict[str,object])->dict[str,object]:return{'schema_version':_SCHEMA_VERSION,'num_agents':int(cfg.env.num_agents),'episode_limit':int(cfg.env.episode_limit),'future_horizon':int(cfg.env.future_horizon),'agent_profiles':list(cfg.data.agent_profiles),'agent_bus_ids':list(cfg.grid.agent_bus_ids),'train_year':int(cfg.data.train_year),'test_year':int(cfg.data.test_year),'train_start_date':cfg.data.train_start_date,'train_end_date':cfg.data.train_end_date,'same_year_has_explicit_train_range':bool(_same_year_has_explicit_train_range(cfg)),'test_window_strategy':'cfg_window','test_start_date':cfg.data.test_start_date,'test_end_date':cfg.data.test_end_date,'load_components':list(cfg.data.load_components),'pv_reference':str(cfg.data.pv_reference),'pv_capacity_kw':list(cfg.data.pv_capacity_kw),'load_scale':list(cfg.data.load_scale),'pv_scale':list(cfg.data.pv_scale),'price_protocol_version':int(PRICE_PROTOCOL_VERSION),'artifacts':artifact_fingerprint}
def _resolve_shared_split_controls(cfg,split:str)->dict[str,object]:selected_year,start_date,end_date,exclude_start_date,exclude_end_date=_resolve_split_dates(cfg,split);return{'split':str(split),'year':int(selected_year),'start_date':start_date,'end_date':end_date,'exclude_start_date':exclude_start_date,'exclude_end_date':exclude_end_date,'window_strategy':'cfg_window'}
