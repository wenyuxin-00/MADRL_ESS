"""Time-feature helpers for forecast training and inference."""
...
def coerce_timestamp_index(
    timestamps: Sequence[str | pd.Timestamp] | pd.DatetimeIndex | None,
) -> pd.DatetimeIndex:
...
def pad_history_timestamps_left(
    timestamps: Sequence[str | pd.Timestamp] | pd.DatetimeIndex | None,
    total_length: int,
    *,
    ...
    if len(index) == 0:
        return pd.date_range(
            start=pd.Timestamp("2000-01-01 00:00:00+00:00"),
...
def encode_forecast_time_features(
    timestamps: Sequence[str | pd.Timestamp] | pd.DatetimeIndex,
    mode: str | None,
) -> np.ndarray:
    ...
    hour_of_day = (
        ...
        + index.second.to_numpy(dtype=np.float32) / np.float32(3600.0)
    )
