...
import pandas as pd
import torch
from predictors.lstm_forecaster import BASELINE_MODE_NONE, PHYSICAL_NORMALIZATION_NONE, POSTPROCESS_MODE_NONE
...
def _build_runtime_forecaster_from_model(model, *, signal_name: str, scaler: object | None, seq_len: int, pred_len: int, hidden_size: int, num_layers: int, dropout: float, device: str | torch.device, input_size: int=1, time_feature_mode: str=TIME_FEATURE_MODE_NONE, model_mode: str='shared', physical_normalization_mode: str=PHYSICAL_NORMALIZATION_NONE, physical_scale_by_column: Sequence[float] | np.ndarray | float | None=None, agent_index: int | None=None, agent_profile: str | None=None, postprocess_mode: str=POSTPROCESS_MODE_NONE, baseline_mode: str=BASELINE_MODE_NONE, blend_weight: float | None=None, optimized_metric: str | None=None):
    return _training_module().build_runtime_forecaster_from_model(model, signal_name=signal_name, scaler=scaler, seq_len=seq_len, pred_len=pred_len, hidden_size=hidden_size, num_layers=num_layers, dropout=dropout, device=device, input_size=input_size, time_feature_mode=time_feature_mode, model_mode=model_mode, physical_normalization_mode=physical_normalization_mode, physical_scale_by_column=physical_scale_by_column, agent_index=agent_index, agent_profile=agent_profile, postprocess_mode=postprocess_mode, baseline_mode=baseline_mode, blend_weight=blend_weight, optimized_metric=optimized_metric)

def _reshape_signal_values(values: np.ndarray) -> np.ndarray:
    return _training_module()._reshape_signal_values(values)

def _select_week_evaluation_slice(frame: pd.DataFrame, values: np.ndarray, *, signal_name: str, seq_len: int) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    _ = signal_name
...
def compute_forecast_metrics(target: np.ndarray, prediction: np.ndarray) -> dict[str, float | None]:
    ...
    non_zero_mask = np.abs(flat_target) > 1e-08
    mape = None
...
def evaluate_signal_open_loop_one_week(source: SignalCsvSource, model, *, signal_name: str, scaler: object | None, seq_len: int, pred_len: int, hidden_size: int, num_layers: int, dropout: float, device: str | torch.device, input_size: int=1, time_feature_mode: str=TIME_FEATURE_MODE_NONE, model_mode: str='shared', physical_normalization_mode: str=PHYSICAL_NORMALIZATION_NONE, physical_scale_by_column: Sequence[float] | np.ndarray | float | None=None, agent_index: int | None=None, agent_profile: str | None=None, postprocess_mode: str=POSTPROCESS_MODE_NONE, baseline_mode: str=BASELINE_MODE_NONE, blend_weight: float | None=None, optimized_metric: str | None=None):
    test_frame, test_values, _ = _load_signal_matrix_from_source(source, signal_name, split='test')
    history_seed, target_values, history_timestamps, timestamps = _select_week_evaluation_slice(test_frame, test_values, signal_name=signal_name, seq_len=seq_len)
    forecaster = _build_runtime_forecaster_from_model(model, signal_name=signal_name, scaler=scaler, seq_len=seq_len, pred_len=pred_len, hidden_size=hidden_size, num_layers=num_layers, dropout=dropout, input_size=input_size, time_feature_mode=time_feature_mode, model_mode=model_mode, physical_normalization_mode=physical_normalization_mode, physical_scale_by_column=physical_scale_by_column, agent_index=agent_index, agent_profile=agent_profile, postprocess_mode=postprocess_mode, baseline_mode=baseline_mode, blend_weight=blend_weight, optimized_metric=optimized_metric, device=device)
    prediction = forecaster.predict(history_seed, horizon=target_values.shape[0] + 1, signal_name=signal_name, history_timestamps=history_timestamps)
    ...
    return _signal_forecast_evaluation_cls()(signal_name=signal_name, evaluation_mode='open_loop', timestamps=np.asarray(timestamps), target=target, prediction=np.asarray(prediction, dtype=np.float32), metrics=compute_forecast_metrics(target, prediction), source_columns=source.value_columns)

def evaluate_signal_online_one_week(source: SignalCsvSource, model, *, signal_name: str, scaler: object | None, seq_len: int, pred_len: int, hidden_size: int, num_layers: int, dropout: float, device: str | torch.device, input_size: int=1, time_feature_mode: str=TIME_FEATURE_MODE_NONE, model_mode: str='shared', physical_normalization_mode: str=PHYSICAL_NORMALIZATION_NONE, physical_scale_by_column: Sequence[float] | np.ndarray | float | None=None, agent_index: int | None=None, agent_profile: str | None=None, postprocess_mode: str=POSTPROCESS_MODE_NONE, baseline_mode: str=BASELINE_MODE_NONE, blend_weight: float | None=None, optimized_metric: str | None=None):
    test_frame, test_values, _ = _load_signal_matrix_from_source(source, signal_name, split='test')
    history_seed, target_values, history_timestamps, timestamps = _select_week_evaluation_slice(test_frame, test_values, signal_name=signal_name, seq_len=seq_len)
    forecaster = _build_runtime_forecaster_from_model(model, signal_name=signal_name, scaler=scaler, seq_len=seq_len, pred_len=pred_len, hidden_size=hidden_size, num_layers=num_layers, dropout=dropout, input_size=input_size, time_feature_mode=time_feature_mode, model_mode=model_mode, physical_normalization_mode=physical_normalization_mode, physical_scale_by_column=physical_scale_by_column, agent_index=agent_index, agent_profile=agent_profile, postprocess_mode=postprocess_mode, baseline_mode=baseline_mode, blend_weight=blend_weight, optimized_metric=optimized_metric, device=device)
    ...
    for step_idx in range(target_matrix.shape[0]):
        ...
        if step_idx > 0:
            current_timestamps = np.concatenate([current_timestamps, np.asarray(timestamps[:step_idx])], axis=0)
        rollout = forecaster.predict(_to_forecaster_history_input(current_history), horizon=max(int(pred_len) + 1, 2), signal_name=signal_name, history_timestamps=current_timestamps)
    ...
    return _signal_forecast_evaluation_cls()(signal_name=signal_name, evaluation_mode='online_aligned', timestamps=np.asarray(timestamps), target=target, prediction=prediction, metrics=compute_forecast_metrics(target, prediction), source_columns=source.value_columns)

def evaluate_signal_one_week(source: SignalCsvSource, model, *, mode: str, signal_name: str, scaler: object | None, seq_len: int, pred_len: int, hidden_size: int, num_layers: int, dropout: float, device: str | torch.device, input_size: int=1, time_feature_mode: str=TIME_FEATURE_MODE_NONE, model_mode: str='shared', physical_normalization_mode: str=PHYSICAL_NORMALIZATION_NONE, physical_scale_by_column: Sequence[float] | np.ndarray | float | None=None, agent_index: int | None=None, agent_profile: str | None=None, postprocess_mode: str=POSTPROCESS_MODE_NONE, baseline_mode: str=BASELINE_MODE_NONE, blend_weight: float | None=None, optimized_metric: str | None=None):
    if mode == 'online_aligned':
        return evaluate_signal_online_one_week(source, model, signal_name=signal_name, scaler=scaler, seq_len=seq_len, pred_len=pred_len, hidden_size=hidden_size, num_layers=num_layers, dropout=dropout, device=device, input_size=input_size, time_feature_mode=time_feature_mode, model_mode=model_mode, physical_normalization_mode=physical_normalization_mode, physical_scale_by_column=physical_scale_by_column, agent_index=agent_index, agent_profile=agent_profile, postprocess_mode=postprocess_mode, baseline_mode=baseline_mode, blend_weight=blend_weight, optimized_metric=optimized_metric)
    if mode == 'open_loop':
        return evaluate_signal_open_loop_one_week(source, model, signal_name=signal_name, scaler=scaler, seq_len=seq_len, pred_len=pred_len, hidden_size=hidden_size, num_layers=num_layers, dropout=dropout, device=device, input_size=input_size, time_feature_mode=time_feature_mode, model_mode=model_mode, physical_normalization_mode=physical_normalization_mode, physical_scale_by_column=physical_scale_by_column, agent_index=agent_index, agent_profile=agent_profile, postprocess_mode=postprocess_mode, baseline_mode=baseline_mode, blend_weight=blend_weight, optimized_metric=optimized_metric)
...
def _plot_evaluation_curve(axis, evaluation) -> None:
    ...
    mode_title = {'online_aligned': 'Online aligned forecast', 'open_loop': 'Open-loop stress test'}.get(evaluation.evaluation_mode, str(evaluation.evaluation_mode))
...
def plot_weekly_forecasts(evaluations: list, *, save_path: str | Path) -> Path:
    if not evaluations:
        raise ValueError('plot_weekly_forecasts requires at least one evaluation result.')
    save_path = Path(save_path)
    save_path.parent.mkdir(parents=True, exist_ok=True)
    figure, axes = plt.subplots(len(evaluations), 1, figsize=(14, 4.2 * len(evaluations)), sharex=False)
...
def plot_signal_training_report(result: dict[str, object], *, figsize: tuple[float, float]=(18.0, 4.8)):
    ...
    figure, axes = plt.subplots(len(rows), subplot_count, figsize=(figsize[0], figsize[1] * len(rows)), squeeze=False)
