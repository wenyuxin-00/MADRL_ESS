...
import numpy as np
import torch
from predictors.artifacts import get_default_lstm_artifact_dir, get_default_lstm_artifact_paths, get_weekly_forecast_plot_path
from predictors.lstm_forecaster import LSTM_LOAD_HYBRID_ARTIFACT_FORMAT, POSTPROCESS_MODE_BASELINE_BLEND, POSTPROCESS_MODE_NONE
...
def _resolve_signal_training_settings(cfg, signal_name: str, overrides: dict[str, object] | None=None) -> tuple[object, dict[str, object]]:
...
def _train_signal_lstm(cfg, signal_name: str, *, device: str | torch.device | None=None, overrides: dict[str, object] | None=None):
    return _training_module().train_signal_lstm(cfg, signal_name, device=device, overrides=overrides)
...
def expected_lstm_artifact_meta(cfg, signal_name: str, overrides: dict[str, object] | None=None, *, agent_index: int | None=None, agent_profile: str | None=None, component: str | None=None) -> dict[str, object]:
    ...
    return {'artifact_format': _resolve_signal_artifact_format(signal_name, settings), 'signal_name': str(settings['signal_name']), 'future_horizon': future_horizon, 'pred_len': future_horizon, 'seq_len': int(settings['history_window']), 'hidden_size': int(settings['hidden_size']), 'num_layers': int(settings['num_layers']), 'dropout': float(settings['dropout']), 'input_size': int(settings['input_size']), 'time_feature_mode': str(settings['time_feature_mode']), 'model_mode': str(settings['model_mode']), 'normalization_mode': _resolve_signal_physical_normalization_mode(signal_name), 'source_signature': _build_lstm_source_signature(local_cfg, signal_name), 'agent_index': None if agent_index is None else int(agent_index), 'agent_profile': None if agent_profile is None else str(agent_profile), 'component': None if component is None else str(component), **({'postprocess_mode': str(settings['postprocess_mode']), 'baseline_mode': str(settings['baseline_mode']), 'blend_weight': None, 'optimized_metric': _expected_signal_optimized_metric(signal_name, postprocess_mode=str(settings['postprocess_mode']), component=component)} if _resolve_signal_artifact_format(signal_name, settings) == LSTM_LOAD_HYBRID_ARTIFACT_FORMAT or str(settings['postprocess_mode']) != POSTPROCESS_MODE_NONE else {})}
...
def compare_lstm_artifact_meta(actual_meta: dict[str, object] | None, expected_meta: dict[str, object]) -> dict[str, object]:
    ...
    for field in comparable_fields:
        ...
        if field == 'dropout':
            ...
            matches = field in actual_meta and (actual_value is None or 0.0 <= float(actual_value) <= 1.0)
        else:
...
def _managed_lstm_artifact_paths(cfg, signal_name: str, *, agent_index: int | None=None, component: str | None=None) -> dict[str, Path]:
    return get_default_lstm_artifact_paths(root=forecast_artifact_root(cfg), signal_name=_normalize_signal_name(signal_name), future_horizon=int(cfg.env.future_horizon), agent_index=agent_index, component=component)

def _artifact_tuple_from_paths(paths: dict[str, Path]) -> tuple[str, str, str]:
    return (str(paths['model_path']), str(paths['meta_path']), str(paths['scaler_path']))

def _format_lstm_artifact_mismatches(mismatches: dict[str, dict[str, object]]) -> str:
    return ', '.join((f"{field}(expected={detail['expected']}, actual={detail['actual']})" for field, detail in sorted(mismatches.items())))
...
def _format_lstm_artifact_issue(validation: dict[str, object]) -> str:
    ...
    if issue_type == 'missing':
        ...
        return f"signal='{signal_name}', artifact='{artifact_path}', missing files: {missing_files}. Delete stale artifacts or enable cfg.forecast.auto_train_missing=True to rebuild them automatically."
    if issue_type == 'incomplete':
        missing_files = ', '.join(validation.get('missing_files', []))
        return f"signal='{signal_name}', artifact='{artifact_path}', artifact is incomplete: {missing_files}. Delete the stale artifact and retry, or enable cfg.forecast.auto_train_missing=True to rebuild it automatically."
    ...
    if normalized_signal == 'load' and _resolve_signal_model_mode(cfg, normalized_signal) == 'per_agent':
        ...
        for agent_index, profile in enumerate(profiles):
            for component in components:
                specs.append({'signal_name': normalized_signal, 'agent_index': int(agent_index), 'agent_profile': profile, 'component': component, 'paths': _managed_lstm_artifact_paths(cfg, normalized_signal, agent_index=agent_index, component=component)})
        return specs
...
def _raise_lstm_artifact_requirements_error(cfg, inventory: dict[str, object]) -> None:
    ...
    raise exc_type('Managed LSTM forecast artifacts are missing or incompatible:\n' + '\n'.join(problem_lines))
...
def collect_available_lstm_artifacts(cfg, *, overrides_by_signal: dict[str, dict[str, object]] | None=None) -> dict[str, object]:
    ...
    return dict(_collect_lstm_artifact_inventory(cfg, overrides_by_signal=resolved_overrides_by_signal)['artifacts'])
...
def ensure_lstm_artifacts(cfg, *, device: str | torch.device | None=None, overrides_by_signal: dict[str, dict[str, object]] | None=None) -> dict[str, object]:
    ...
    for signal_name in _configured_forecast_signals(cfg):
        ...
        trained_result = _train_signal_lstm(cfg, signal_name, device=device, overrides=dict(resolved_overrides_by_signal.get(signal_name) or {}) or None)
    ...
    plot_path = get_weekly_forecast_plot_path(future_horizon=int(cfg.env.future_horizon), root=artifact_root)
    if evaluations:
        plot_path = _plot_weekly_forecasts(evaluations, save_path=plot_path)
    elif not plot_path.exists():
        plot_path = None
    return {'mode': 'managed_multi_signal', 'artifacts': dict(inventory_after['artifacts']), 'trained_signals': trained_signals, 'retrained_signals': retrained_signals, 'mismatched_signals': list(inventory_before['mismatched_signals']), 'invalid_artifacts': dict(inventory_before['invalid_artifacts']), 'missing_signals': list(inventory_after['missing_signals']), 'plot_path': str(plot_path) if plot_path is not None else None}
