...
def load_lstm_forecaster_artifacts(model_path, meta_path=None, scaler_path=None) -> tuple[dict, object]:
    ...
    if not meta_path.exists():
        raise FileNotFoundError(f"Missing LSTM meta artifact: '{meta_path}'. Expected '<stem>_meta.json' next to the model.")
    if not scaler_path.exists():
        raise FileNotFoundError(f"Missing LSTM scaler artifact: '{scaler_path}'. Expected '<stem>_scaler.pkl' next to the model.")
    meta = json.loads(meta_path.read_text(encoding='utf-8'))
    missing_fields = [field for field in LSTM_REQUIRED_META_FIELDS if field not in meta]
    if missing_fields:
        raise ValueError(f"LSTM meta artifact '{meta_path}' is missing required fields: {missing_fields}")
    if str(meta.get('signal_name', '')).strip().lower() == 'price':
        raise ValueError(f"LSTM meta artifact '{meta_path}' still uses legacy signal_name='price'. Rebuild the artifact with the wholesale/import price protocol cutover.")
    if int(meta.get('price_protocol_version', -1)) != int(PRICE_PROTOCOL_VERSION):
        raise ValueError(f"LSTM meta artifact '{meta_path}' has unsupported price_protocol_version={meta.get('price_protocol_version')!r}; expected {PRICE_PROTOCOL_VERSION}.")
...
class LSTMForecaster(Forecaster):
    ...
    def set_episode(self, episode_signals: dict[str, np.ndarray] | np.ndarray, episode_meta: dict[str, object] | None=None) -> None:
        ...
        if isinstance(episode_signals, dict):
            for key, value in episode_signals.items():
                if key.startswith('load_'):
                    self._episode_component_signals[key] = np.asarray(value, dtype=np.float32)
    ...
    def _build_padded_history_windows(values: np.ndarray, seq_len: int) -> np.ndarray:
        ...
        prefix = np.zeros((max(seq_len - 1, 0),), dtype=np.float32)
    ...
    def _append_prediction_chunk_to_windows(windows: np.ndarray, prediction_chunk: np.ndarray) -> np.ndarray:
        ...
        chunk = np.asarray(prediction_chunk, dtype=np.float32)
        if chunk.ndim == 1:
            chunk = chunk[:, None]
        if chunk.size == 0:
            return base
    ...
    def _build_time_window_view(timestamp_index: pd.DatetimeIndex, *, seq_len: int, horizon: int, time_feature_mode: str) -> np.ndarray | None:
        ...
        seq_len = int(seq_len)
        horizon = int(horizon)
        total_steps = len(timestamp_index)
        if total_steps == 0:
            return np.zeros((0, seq_len, 0), dtype=np.float32)
        step_delta = infer_timestamp_step(timestamp_index)
        prefix = pd.date_range(end=timestamp_index[0] - step_delta, periods=max(seq_len - 1, 0), freq=step_delta)
        extension = pd.date_range(start=timestamp_index[-1] + step_delta, periods=max(horizon - 1, 0), freq=step_delta)
        full_index = prefix.append(timestamp_index).append(extension)
    ...
    def _predict_model_chunk_batch(self, runtime: _SignalForecasterRuntime, history_windows: np.ndarray, *, time_windows: np.ndarray | None, column_idx: int | None, batch_size: int) -> np.ndarray:
        ...
        normalized_mode = normalize_time_feature_mode(runtime.time_feature_mode if int(runtime.input_size) > 1 else 'none')
        with torch.inference_mode():
            for start in range(0, len(histories), batch_size):
                end = start + batch_size
                history_batch = histories[start:end]
                normalized_history, scale = self._normalize_model_input(history_batch, runtime, column_idx=column_idx)
                if runtime.scaler is not None:
                    load_channel = runtime.scaler.transform(normalized_history.reshape(-1, 1)).reshape(normalized_history.shape).astype(np.float32)
                else:
                    load_channel = normalized_history.astype(np.float32)
                if int(runtime.input_size) <= 1 or normalized_mode == TIME_FEATURE_MODE_NONE:
                    features = load_channel
                else:
                    if time_windows is None:
                        raise ValueError('time_windows are required when runtime.input_size > 1.')
                ...
                prediction = runtime.model(tensor).detach().cpu().numpy().astype(np.float32)
                if runtime.scaler is not None:
                    prediction = runtime.scaler.inverse_transform(prediction.reshape(-1, 1)).reshape(prediction.shape).astype(np.float32)
    ...
    def _predict_univariate_episode_matrix(self, runtime: _SignalForecasterRuntime, history: np.ndarray, horizon: int, *, column_idx: int | None=None, history_timestamps: Sequence[str | pd.Timestamp] | None=None, batch_size: int=8192) -> np.ndarray:
        ...
        horizon = int(horizon)
        if horizon <= 0:
            return np.zeros((total_steps, 0), dtype=np.float32)
        if total_steps == 0:
        ...
        timestamp_index = self._coerce_episode_timestamp_index(history_timestamps, total_steps)
        rolling_windows = self._build_padded_history_windows(values, runtime.seq_len)
        time_window_view = self._build_time_window_view(timestamp_index, seq_len=runtime.seq_len, horizon=horizon, time_feature_mode=runtime.time_feature_mode if int(runtime.input_size) > 1 else TIME_FEATURE_MODE_NONE)
        remaining = horizon - 1
        if str(runtime.postprocess_mode) == POSTPROCESS_MODE_BASELINE_BLEND:
            ...
            future = np.empty((total_steps, remaining), dtype=np.float32)
            weight = np.float32(1.0 if runtime.blend_weight is None else float(runtime.blend_weight))
            for step_idx in range(remaining):
                current_time_windows = None if time_window_view is None else time_window_view[step_idx:step_idx + total_steps]
                raw_batch = self._predict_model_chunk_batch(runtime, rolling_windows, time_windows=current_time_windows, column_idx=column_idx, batch_size=batch_size)
                baseline_values = rolling_windows[:, -1].astype(np.float32, copy=False)
                next_values = (baseline_values + weight * (raw_batch[:, 0] - baseline_values)).astype(np.float32)
                future[:, step_idx] = next_values
                if step_idx + 1 < remaining:
                    rolling_windows = self._append_prediction_chunk_to_windows(rolling_windows, next_values[:, None])
        ...
        while produced < remaining:
            current_time_windows = None if time_window_view is None else time_window_view[produced:produced + total_steps]
            raw_batch = self._predict_model_chunk_batch(runtime, rolling_windows, time_windows=current_time_windows, column_idx=column_idx, batch_size=batch_size)
            take = min(int(runtime.pred_len), remaining - produced)
            prediction_chunk = np.asarray(raw_batch[:, :take], dtype=np.float32)
    ...
    def predict_episode_matrix(self, history: np.ndarray, horizon: int, *, signal_name: str=WHOLESALE_PRICE_SIGNAL, history_timestamps: Sequence[str | pd.Timestamp] | None=None, batch_size: int=8192) -> np.ndarray:
        ...
        if any((r.component is not None for r in runtimes)):
            return self._predict_component_split_episode_matrix(runtimes, history, horizon, history_timestamps=history_timestamps, batch_size=batch_size)
        if history.ndim == 1:
            runtime = runtimes[0]
            return self._predict_univariate_episode_matrix(runtime, history, horizon, column_idx=0, history_timestamps=history_timestamps, batch_size=batch_size)
        if history.ndim != 2:
            raise ValueError(f'LSTMForecaster expects 1D or 2D history, got shape {history.shape}')
        predictions: list[np.ndarray] = []
