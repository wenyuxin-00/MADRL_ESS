...
def build_mainline_forecast_controls(cfg) -> dict[str, object]:
    ...
    controls = {name: getattr(cfg.forecast, name) for name in _FORECAST_FIELDS}
    controls.update({"artifact_root": None if artifact_root in (None, "") else str(Path(artifact_root).resolve()), "target_signals": [str(value) for value in cfg.forecast.target_signals], "future_horizon": int(cfg.env.future_horizon), "signal_training_overrides": _normalize_signal_training_overrides(getattr(cfg.forecast, "signal_training_overrides", {}))})
    return controls
...
def resolve_mainline_forecast_controls(cfg, forecast_controls: Mapping[str, object] | None = None) -> dict[str, object]:
    ...
    if controls.get("future_horizon") not in (None, int(cfg.env.future_horizon)):
        raise ValueError(f"forecast_controls.future_horizon must match cfg.env.future_horizon, got {controls.get('future_horizon')} vs {cfg.env.future_horizon}.")
    incoming = _normalize_signal_training_overrides(controls.pop("signal_training_overrides", {}))
    merged = {**canonical, **controls, "signal_training_overrides": {name: {**dict(canonical["signal_training_overrides"].get(name, {}) or {}), **dict(incoming.get(name, {}) or {})} for name in set(canonical["signal_training_overrides"]) | set(incoming)}}
    artifact_root = merged.get("artifact_root", cfg.forecast.lstm_artifact_root)
    return {"target_signals": [str(value) for value in merged.get("target_signals", cfg.forecast.target_signals)], **{name: type(getattr(cfg.forecast, name))(merged.get(name, getattr(cfg.forecast, name))) for name in _FORECAST_FIELDS}, "artifact_root": None if artifact_root in (None, "") else str(Path(artifact_root).resolve()), "signal_training_overrides": _normalize_signal_training_overrides(merged.get("signal_training_overrides", getattr(cfg.forecast, "signal_training_overrides", {})))}

def get_mainline_forecast_controls(*, artifact_root: str | Path | None = None, auto_train_missing: bool = False) -> dict[str, object]:
    return {**build_mainline_forecast_controls(ExperimentConfig()), "artifact_root": str(Path(artifact_root).resolve()) if artifact_root is not None else str(get_default_lstm_artifact_dir()), "auto_train_missing": bool(auto_train_missing)}
