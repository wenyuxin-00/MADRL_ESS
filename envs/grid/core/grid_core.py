...
class GridStepResult:
    ...
    trafo_p_signed_kw: np.ndarray = field(default_factory=lambda: np.zeros(0, dtype=np.float32))
    psi_v_raw: float = 0.0
...
class GridCore:
    def __init__(self, deployments: list["AgentDeployment"], grid_cfg: Any) -> None:
        ...
        self._agent_bus_pos = np.asarray([bus_id_to_pos[bus_id] for bus_id in self.agent_bus_ids], dtype=np.int64)
    ...
    def _ensure_agent_bus_rows(self) -> list[tuple[int, int, int, float, float]]:
        ...
        for bus_id in self.agent_bus_ids:
            ...
            load_idx = int(self.net.load.index[load_bus][0]) if load_bus.any() else int(pp.create_load(self.net, bus=bus_id, p_mw=0.0, q_mvar=0.0))
            sgen_idx = int(self.net.sgen.index[sgen_bus][0]) if sgen_bus.any() else int(pp.create_sgen(self.net, bus=bus_id, p_mw=0.0, q_mvar=0.0))
            rows.append((int(bus_id), load_idx, sgen_idx, float(self.net.load.at[load_idx, "p_mw"]), float(self.net.sgen.at[sgen_idx, "p_mw"])))
        return rows
    ...
    def step(self, p_batt_kw: np.ndarray, base_load_kw: np.ndarray) -> GridStepResult:
        self._restore_agent_buses()
        self._apply_agent_injections(-(np.asarray(base_load_kw, dtype=np.float32) + np.asarray(p_batt_kw, dtype=np.float32)))
        try:
            self._runpp()
            self.last_pf_error = ""
    ...
    def _restore_agent_buses(self) -> None:
        for _, load_idx, sgen_idx, load_p_mw, sgen_p_mw in self._bus_rows:
            self.net.load.at[load_idx, "p_mw"] = load_p_mw
            self.net.sgen.at[sgen_idx, "p_mw"] = sgen_p_mw

    def _apply_agent_injections(self, p_inject_kw: np.ndarray) -> None:
        for (bus_id, load_idx, sgen_idx, _, _), p_kw in zip(self._bus_rows, np.asarray(p_inject_kw, dtype=np.float32), strict=False):
            p_mw = float(p_kw) / 1000.0
            self.net.sgen.at[sgen_idx, "p_mw"] = max(0.0, p_mw)
            self.net.sgen.at[sgen_idx, "q_mvar"] = 0.0
            self.net.load.at[load_idx, "p_mw"] = max(0.0, -p_mw)
            self.net.load.at[load_idx, "q_mvar"] = 0.0

    def _runpp(self) -> None:
        import pandapower as pp

        attempts = ({"algorithm": self.grid_cfg.pf_solver, "init": "flat"}, {"algorithm": self.grid_cfg.pf_solver, "init": "auto"}, {"algorithm": "bfsw", "init": "flat"})
    ...
    def _extract_result(self, *, converged: bool) -> GridStepResult:
        ...
        v_violation = (np.maximum(0.0, v_min - agent_vm_pu) + np.maximum(0.0, agent_vm_pu - v_max)).astype(np.float32)
        bus_v_excess = (np.maximum(0.0, v_min - vm_pu) + np.maximum(0.0, vm_pu - v_max)).astype(np.float32)
        line_excess = (np.maximum(0.0, line_loading_pct - limit) / 100.0).astype(np.float32)
        trafo_excess = (np.maximum(0.0, trafo_loading_pct - limit) / 100.0).astype(np.float32)
