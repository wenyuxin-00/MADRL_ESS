...
class GridCore:
    ...
    def __init__(self, deployments: list['AgentDeployment'], grid_cfg: Any) -> None:
        ...
        for bus_id in self.agent_bus_ids:
            if bus_id not in self._bus_id_to_pos:
                raise ValueError(f'agent_bus_id={bus_id} is not present in network bus index. Available bus ids: {bus_index_list}')
        ...
        for bus_id in self.agent_bus_ids:
            ...
            if load_mask.any():
                self._original_load_p_mw[bus_id] = float(self.net.load.at[load_index, 'p_mw'])
            ...
            if sgen_mask.any():
                self._original_sgen_p_mw[bus_id] = float(self.net.sgen.at[sgen_index, 'p_mw'])
    ...
    def step(self, p_batt_kw: np.ndarray, base_load_kw: np.ndarray) -> GridStepResult:
        ...
        bus_id_to_p_kw = {self.agent_bus_ids[i]: float(p_inject_kw[i]) for i in range(self.n_agents)}
    ...
    def _runpp_with_fallback(self) -> None:
        ...
        if self._last_success_kwargs is not None:
            ...
            try:
                ...
                pass
        attempts = ({'algorithm': self.grid_cfg.pf_solver, 'numba': False, 'lightsim2grid': False, 'calculate_voltage_angles': False, 'voltage_depend_loads': False, 'init': 'flat'}, {'algorithm': 'bfsw', 'numba': False, 'lightsim2grid': False, 'calculate_voltage_angles': False, 'voltage_depend_loads': False, 'init': 'flat'}, {'algorithm': self.grid_cfg.pf_solver, 'numba': False, 'lightsim2grid': False, 'calculate_voltage_angles': False, 'voltage_depend_loads': False, 'init': 'auto'}, {'algorithm': self.grid_cfg.pf_solver, 'numba': True, 'lightsim2grid': False, 'calculate_voltage_angles': False, 'voltage_depend_loads': False, 'init': 'auto'})
        ...
        with warnings.catch_warnings():
            ...
            for extra_kwargs in attempts:
                try:
                    ...
                    last_exc = exc
                    error_messages.append(f"{extra_kwargs.get('algorithm', self.grid_cfg.pf_solver)} / numba={extra_kwargs.get('numba')} / lightsim2grid={extra_kwargs.get('lightsim2grid', 'default')} -> {type(exc).__name__}: {exc}")
    ...
    def _extract_result(self, *, converged: bool) -> GridStepResult:
        ...
        if self.n_trafos > 0 and hasattr(self.net, 'res_trafo') and (not self.net.res_trafo.empty):
            ...
            trafo_p_signed_kw = np.zeros(self.n_trafos, dtype=np.float32)
        agent_vm_pu = np.array([float(vm_pu[self._bus_id_to_pos[bus_id]]) for bus_id in self.agent_bus_ids], dtype=np.float32)
        ...
        bus_v_excess = (np.maximum(0.0, v_min - vm_pu) + np.maximum(0.0, vm_pu - v_max)).astype(np.float32)
        ...
        return GridStepResult(converged=converged, vm_pu=vm_pu, va_degree=va_degree, line_loading_pct=line_loading_pct, trafo_loading_pct=trafo_loading_pct, p_mw_from=p_mw_from, agent_vm_pu=agent_vm_pu, v_violation=v_violation, line_violation=line_violation, trafo_violation=trafo_violation, l_violation=l_violation, n_buses=self.n_buses, n_lines=self.n_lines, n_trafos=self.n_trafos, trafo_p_signed_kw=trafo_p_signed_kw.astype(np.float32, copy=False), bus_v_excess=bus_v_excess, line_excess=line_excess_arr, trafo_excess=trafo_excess_arr, psi_v_raw=psi_v_raw, psi_line_raw=psi_line_raw, psi_trafo_raw=psi_trafo_raw)

    def _make_zero_result(self) -> GridStepResult:
        return GridStepResult(converged=False, vm_pu=np.ones(self.n_buses, dtype=np.float32), va_degree=np.zeros(self.n_buses, dtype=np.float32), line_loading_pct=np.zeros(self.n_lines, dtype=np.float32), trafo_loading_pct=np.zeros(self.n_trafos, dtype=np.float32), p_mw_from=np.zeros(self.n_lines, dtype=np.float32), agent_vm_pu=np.ones(self.n_agents, dtype=np.float32), v_violation=np.zeros(self.n_agents, dtype=np.float32), line_violation=0.0, trafo_violation=0.0, l_violation=0.0, n_buses=self.n_buses, n_lines=self.n_lines, n_trafos=self.n_trafos, trafo_p_signed_kw=np.zeros(self.n_trafos, dtype=np.float32), bus_v_excess=np.zeros(self.n_buses, dtype=np.float32), line_excess=np.zeros(self.n_lines, dtype=np.float32), trafo_excess=np.zeros(self.n_trafos, dtype=np.float32), psi_v_raw=0.0, psi_line_raw=0.0, psi_trafo_raw=0.0)
