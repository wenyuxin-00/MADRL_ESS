...
def _subproc_worker(address, authkey: bytes, cfg, mode: str, worker_rank: int, seed: int | None) -> None:
    ...
    parallel_episode_sampling = validate_parallel_episode_sampling_mode(getattr(getattr(cfg, 'train', None), 'parallel_episode_sampling', 'unique_active'))
    try:
        ...
        while True:
            ...
            if cmd == 'step':
                ...
                if episode_done:
                    ...
                    reset_obs, reset_info = env.reset(episode_idx=next_episode_idx) if next_episode_idx is not None else env.reset()
...
def _recv_worker_ready(remote, process, worker_rank: int) -> dict[str, int]:
    ...
    if status == _WORKER_READY:
        return {'num_agents': int(payload['num_agents']), 'num_available_episodes': int(payload['num_available_episodes']), 'episode_length': int(payload['episode_length'])}
...
class SubprocVecEnv:
    ...
    def __init__(self, num_envs: int, cfg, mode: str='train', seed: int | None=None):
        ...
        try:
            for worker_rank in range(self.num_envs):
                ...
                try:
                    process = ctx.Process(target=_subproc_worker, args=(listener.address, self.authkey, cfg, mode, worker_rank, seed), daemon=True)
                    process.start()
            ...
            for worker_rank, (remote, process) in enumerate(zip(self.remotes, self.processes)):
                ...
                if worker_rank == 0:
                    ...
                    raise RuntimeError(f'SubprocVecEnv workers reported inconsistent agent counts: worker 0 -> {self.num_agents}, worker {worker_rank} -> {worker_num_agents}.')
                elif int(worker_meta['num_available_episodes']) != self.num_available_episodes:
                    raise RuntimeError(f"SubprocVecEnv workers reported inconsistent num_available_episodes: worker 0 -> {self.num_available_episodes}, worker {worker_rank} -> {int(worker_meta['num_available_episodes'])}.")
                elif int(worker_meta['episode_length']) != self.episode_length:
                    raise RuntimeError(f"SubprocVecEnv workers reported inconsistent episode_length: worker 0 -> {self.episode_length}, worker {worker_rank} -> {int(worker_meta['episode_length'])}.")
    ...
    def step(self, actions_n_batched):
        ...
        done_flags = [bool(info.get('episode_done', False) or np.all(np.logical_or(np.asarray(terminated), np.asarray(truncated)))) for info, terminated, truncated in zip(info_list, terminated_list, truncated_list)]
        if self.parallel_episode_sampling == 'unique_active':
