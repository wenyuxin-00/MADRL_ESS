...
class NormalReward(RewardFn):
    ...
    def compute(self, env_state: dict) -> tuple[np.ndarray, dict[str, np.ndarray]]:
        ...
        p_max = np.asarray(env_state["p_max"], dtype=np.float32)
        ...
        r_action_pen = (
            self.w_action_pen * np.abs(e_bat_req - e_bat) / (p_max + 1e-6)
        ).astype(np.float32)
        ...
        total = (r_cost + r_throughput - r_action_pen - r_safe_v - r_safe_trafo).astype(
            np.float32
        )
        components = {
            "r_cost": r_cost,
