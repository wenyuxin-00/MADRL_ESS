    ...
    def _initialize_episode_sampler(self) -> None:
        ...
        for env_idx, env in enumerate(self.envs[1:], start=1):
            if int(getattr(env, "num_available_episodes", 0)) != num_available_episodes:
                raise RuntimeError(
                    "DummyVecEnv requires matching num_available_episodes across envs in "
                    "parallel_episode_sampling='unique_active' mode."
                )
            if int(getattr(env, "episode_length", 0)) != episode_length:
                raise RuntimeError(
                    "DummyVecEnv requires matching episode_length across envs in "
                    "parallel_episode_sampling='unique_active' mode."
                )
        self._episode_sampler = ParallelEpisodeSampler(
            num_available_episodes=num_available_episodes,
    ...
    def reset(self):
        ...
        for env_idx, env in enumerate(self.envs):
            obs, info = self._reset_env(
                env,
                episode_idx=episode_indices[env_idx],
                seed=self._consume_initial_seed(env_idx),
            )
            obs_list.append(obs)
            ...
            if any(done_flags):
                ...
                for env_idx, env in enumerate(self.envs):
                    reset_obs, reset_info = self._reset_env(
                        env,
                        episode_idx=current_wave_indices[env_idx],
                    )
