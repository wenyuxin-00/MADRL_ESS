...
from envs.observation.feature_blocks import broadcast_scalar_feature, build_time_features, reshape_agent_scalar_feature
from envs.observation.features import build_adjacency_field, resolve_local_features, resolve_sequence_features
...
class PrecomputedObservationBuilder(ObservationBuilder):
    ...
    def __init__(self, local_features: list[str], sequence_features: list[str], future_horizon: int, adjacency_type: str='identity', normalizer: ObservationNormalizer | None=None):
    ...
    def get_schema(self, n_agents: int) -> dict[str, tuple[int, ...]]:
        schema = {'local': (int(n_agents), self.local_dim), 'adjacency': (int(n_agents), int(n_agents)), 'safety_local': (int(n_agents), len(_SAFETY_LOCAL_FIELDS))}
    ...
    def get_layout(self, n_agents: int) -> dict[str, dict]:
        layout = {'local': {'group': 'local', 'scope': 'per_agent', 'dim': int(self.local_dim), 'fields': list(self.local_feature_names)}, 'adjacency': {'group': 'graph', 'scope': 'shared', 'dim': int(n_agents), 'fields': ['adjacency']}, 'safety_local': {'group': 'projector', 'scope': 'per_agent', 'dim': len(_SAFETY_LOCAL_FIELDS), 'fields': list(_SAFETY_LOCAL_FIELDS)}}
        schema = self.get_schema(n_agents)
        for spec in self.sequence_specs:
            field_name = spec.field_name()
            layout[field_name] = {**spec.layout(), 'shape': schema[field_name]}
        return layout
    ...
    def _build_safety_local(self, env) -> np.ndarray:
        return np.column_stack([np.asarray(env.soc, dtype=np.float32), np.asarray(env.get_signal_step('load'), dtype=np.float32), np.asarray(env.get_signal_step('pv'), dtype=np.float32), np.asarray(env.agent_c_bat, dtype=np.float32), np.asarray(env.agent_p_max, dtype=np.float32)]).astype(np.float32)
    ...
    def _build(self, env, *, apply_normalization: bool) -> dict[str, np.ndarray]:
        ...
        obs = {'local': local, 'adjacency': build_adjacency_field(env.n, self.adjacency_type), 'safety_local': self._build_safety_local(env)}
        for spec in self.sequence_specs:
