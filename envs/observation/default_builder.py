...
def _adjacency(n_agents: int, adjacency_type: str) -> np.ndarray:
    ...
    if adjacency_type != "fully_connected_no_self":
        raise ValueError(f"Unknown adjacency_type '{adjacency_type}', expected 'identity' or 'fully_connected_no_self'.")
...
def _calendar_features(timestamp_value: str, n_agents: int) -> np.ndarray:
    ...
    phases = np.asarray([np.sin(2.0 * np.pi * hour_of_day / 24.0), np.cos(2.0 * np.pi * hour_of_day / 24.0), np.sin(2.0 * np.pi * day_of_year / 365.25), np.cos(2.0 * np.pi * day_of_year / 365.25)], dtype=np.float32)
    return np.repeat(phases.reshape(1, -1), int(n_agents), axis=0)


def _pad_sequence(values, start: int, length: int) -> np.ndarray:
    array = np.asarray(values, dtype=np.float32)
    tail = array[int(start) : int(start) + int(length)]
    pad_rows = int(length) - int(tail.shape[0])
    if pad_rows > 0:
...
def _signal_history(env, signal_name: str) -> np.ndarray:
    ...
    return signal[: env.cur_step + 1].copy() if prefix.size == 0 else np.concatenate([prefix, signal[: env.cur_step + 1]], axis=0).astype(np.float32, copy=False)
...
class DefaultObservationBuilder:
    ...
    def get_schema(self, n_agents: int) -> dict[str, tuple[int, ...]]:
        return {
            ...
            **{
                f"{name}_seq": (self.sequence_length,) if _SEQUENCE_SCOPES[name] == "shared" else (int(n_agents), self.sequence_length)
    ...
    def get_layout(self, n_agents: int) -> dict[str, dict]:
        layout = {
            "local": {"group": "local", "scope": "per_agent", "dim": int(self.local_dim), "fields": list(self.local_feature_names)},
            "adjacency": {"group": "graph", "scope": "shared", "dim": int(n_agents), "fields": ["adjacency"]},
            "safety_local": {"group": "projector", "scope": "per_agent", "dim": len(_SAFETY_LOCAL_FIELDS), "fields": list(_SAFETY_LOCAL_FIELDS)},
        }
        schema = self.get_schema(n_agents)
        for name in self.sequence_feature_names:
            layout[f"{name}_seq"] = {"feature_name": name, "group": "sequence", "scope": _SEQUENCE_SCOPES[name], "dim": 1, "shape": schema[f"{name}_seq"]}
        return layout
    ...
    def build_raw(self, env) -> dict[str, np.ndarray]:
        return self._build(env, normalize=False)

    def build(self, env) -> dict[str, np.ndarray]:
        return self._build(env, normalize=True)
    ...
    def _local_feature(self, env, name: str) -> np.ndarray:
        ...
        if name == "calendar_time":
            ...
            timestamp = str(timestamps[int(env.cur_step)]) if 0 <= int(env.cur_step) < len(timestamps) else str(_TS_FALLBACK + pd.Timedelta(minutes=15 * int(env.cur_step)))
    ...
    def _sequence_feature(self, env, name: str) -> np.ndarray:
        ...
        if getattr(env, "forecaster", None) is None:
            return _pad_sequence(env.get_signal(name), env.cur_step, self.sequence_length)
        ...
        return np.asarray(env.forecaster.predict(history, self.sequence_length, signal_name=name, history_timestamps=history_timestamps), dtype=np.float32)

    def _build(self, env, *, normalize: bool) -> dict[str, np.ndarray]:
        local_parts = [self._normalize(name, self._local_feature(env, name), normalize=normalize, sequence=False) for name in self.local_feature_names]
        obs = {
            "local": np.concatenate(local_parts, axis=1).astype(np.float32) if local_parts else np.zeros((env.n, 0), dtype=np.float32),
            "adjacency": _adjacency(env.n, self.adjacency_type),
            "safety_local": env._current_safety_local().astype(np.float32),
        }
        for name in self.sequence_feature_names:
            obs[f"{name}_seq"] = self._normalize(name, self._sequence_feature(env, name), normalize=normalize, sequence=True)
        return obs
