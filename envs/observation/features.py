...
import numpy as np
import pandas as pd
from envs.observation.feature_blocks import broadcast_scalar_feature, build_adjacency, build_calendar_time_features, build_time_features, pad_sequence_1d, pad_sequence_2d, reshape_agent_scalar_feature
...
class ObservationFeatureSpec:
    ...
    def schema(self, n_agents: int, sequence_length: int) -> tuple[int, ...]:
        ...
        if self.scope == 'per_agent':
            return (int(n_agents), sequence_length) if self.dim == 1 else (int(n_agents), sequence_length, int(self.dim))
        raise ValueError(f"Unknown feature scope '{self.scope}'.")

    def layout(self) -> dict[str, str | int]:
        return {'feature_name': self.name, 'group': self.group, 'scope': self.scope, 'dim': int(self.dim), 'description': self.description}
...
def _current_shared_signal_feature(signal_name: str, description: str) -> ObservationFeatureSpec:
    return ObservationFeatureSpec(name=signal_name, group='local', dim=1, scope='shared', builder=lambda env, _: broadcast_scalar_feature(env.get_signal_step(signal_name), env.n), description=description)

def _current_per_agent_signal_feature(signal_name: str, description: str) -> ObservationFeatureSpec:
    return ObservationFeatureSpec(name=signal_name, group='local', dim=1, scope='per_agent', builder=lambda env, _: reshape_agent_scalar_feature(env.get_signal_step(signal_name)), description=description)

def _shared_signal_sequence_feature(signal_name: str, description: str, *, use_forecaster: bool=False) -> ObservationFeatureSpec:

    def builder(env, sequence_length: int) -> np.ndarray:
        if use_forecaster:
            return env.forecaster.predict(env.get_signal_history(signal_name), sequence_length, signal_name=signal_name, history_timestamps=_history_timestamps(env)).astype(np.float32)
        return pad_sequence_1d(env.get_signal(signal_name), env.cur_step, sequence_length).astype(np.float32)
    return ObservationFeatureSpec(name=signal_name, group='sequence', dim=1, scope='shared', builder=builder, description=description)

def _per_agent_signal_sequence_feature(signal_name: str, description: str, *, use_forecaster: bool=False) -> ObservationFeatureSpec:

    def builder(env, sequence_length: int) -> np.ndarray:
        if use_forecaster:
            return env.forecaster.predict(env.get_signal_history(signal_name), sequence_length, signal_name=signal_name, history_timestamps=_history_timestamps(env)).astype(np.float32)
        return pad_sequence_2d(env.get_signal(signal_name), env.cur_step, sequence_length).T.astype(np.float32)
    return ObservationFeatureSpec(name=signal_name, group='sequence', dim=1, scope='per_agent', builder=builder, description=description)
register_local_feature(ObservationFeatureSpec(name='time', group='local', dim=2, scope='shared', builder=lambda env, _: build_time_features(env.cur_step, env.episode_length, env.n), description='Episode progress encoded as one sin/cos pair.'))
register_local_feature(ObservationFeatureSpec(name='calendar_time', group='local', dim=4, scope='shared', builder=lambda env, _: build_calendar_time_features(_current_timestamp_value(env), env.n), description='Absolute hour-of-day and day-of-year sin/cos encodings.'))
...
register_local_feature(ObservationFeatureSpec(name='soc', group='local', dim=1, scope='per_agent', builder=lambda env, _: reshape_agent_scalar_feature(env.soc), description='Current battery state of charge per agent.'))
register_sequence_feature(_shared_signal_sequence_feature('wholesale_price', 'Future shared wholesale price window.', use_forecaster=True))
register_sequence_feature(_per_agent_signal_sequence_feature('load', 'Future per-agent load window.', use_forecaster=True))
register_sequence_feature(_per_agent_signal_sequence_feature('pv', 'Future per-agent PV window.', use_forecaster=True))
