"""Fast-lab subprocess vector environment for cached-observation training."""
...
def _make_worker_env(cfg, mode: str, *, worker_rank: int, seed: int | None):
    ...
    from scripts.builder_fastlab import build_fastlab_env
    ...
    return build_fastlab_env(worker_cfg, mode=mode)
...
def _recv_worker_ready(remote, process, worker_rank: int) -> int:
    ...
    if status == _WORKER_INIT_ERROR:
        raise RuntimeError(
            f"SubprocVecEnvFastLab worker {worker_rank} failed during initialization:\n{payload}"
        )

    raise RuntimeError(
        f"SubprocVecEnvFastLab worker {worker_rank} sent unexpected init status {status!r}."
    )


class SubprocVecEnvFastLab:
    """Run cached-observation training envs in parallel subprocesses."""

    def __init__(self, num_envs: int, cfg, mode: str = "train", seed: int | None = None):
        self.num_envs = int(num_envs)
        self.closed = False
        self.authkey = b"madrl_subproc_vec_env_fastlab"
        ...
        try:
            ...
            for worker_rank, (remote, process) in enumerate(zip(self.remotes, self.processes)):
                ...
                if worker_rank == 0:
                    ...
                    raise RuntimeError(
                        "SubprocVecEnvFastLab workers reported inconsistent agent counts: "
