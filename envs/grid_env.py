...
class GridEnv(gym.Env):
	...
	def __init__(self,cfg:Any,mode:str='train',dataset:Any|None=None,reward_fn:Any|None=None,forecaster:Any|None=None,obs_builder:Any|None=None,grid_core:Any|None=None,precomputed_data_dir:str|Path|None=None)->None:
		super().__init__();self.cfg=cfg;self.mode=mode;env_cfg=cfg.env;self.n=int(env_cfg.num_agents);self.future_horizon=int(env_cfg.future_horizon);self.eff=float(env_cfg.efficiency);self.init_soc=float(env_cfg.init_soc);self.dt=float(env_cfg.dt);fixed_capacity_kwh,_,fixed_p_max_kw=resolve_fixed_battery_spec(env_cfg.battery_capacity,env_cfg.max_charge_rate,n_agents=self.n);self._fixed_capacity_kwh=np.asarray(fixed_capacity_kwh,dtype=np.float32);self._fixed_p_max_kw=np.asarray(fixed_p_max_kw,dtype=np.float32);runtime_seed=getattr(getattr(cfg,'runtime',None),'seed',None);self._default_seed=None if runtime_seed is None else int(runtime_seed);self._seeded_once=False;self.soc_min=float(env_cfg.soc_min);self.soc_max=float(env_cfg.soc_max);self.soc_target=float(env_cfg.soc_target)
		if not .0<=self.soc_min<=self.soc_max<=1.:raise ValueError(f"Invalid SoC range: soc_min={self.soc_min}, soc_max={self.soc_max}.")
		self.init_soc=float(np.clip(self.init_soc,self.soc_min,self.soc_max));self.train_init_soc_low=float(getattr(env_cfg,'train_init_soc_low',self.init_soc));self.train_init_soc_high=float(getattr(env_cfg,'train_init_soc_high',self.init_soc))
		if not self.soc_min<=self.train_init_soc_low<=self.train_init_soc_high<=self.soc_max:raise ValueError(f"Invalid train initial SoC range: train_init_soc_low={self.train_init_soc_low}, train_init_soc_high={self.train_init_soc_high}, expected within [{self.soc_min}, {self.soc_max}].")
		self._grid_cfg=cfg.grid;from envs.rewards.NormalReward import NormalReward;self.reward_fn=reward_fn if reward_fn is not None else NormalReward(cfg);self.import_price_markup_eur_per_kwh=get_import_price_markup(cfg)
		if dataset is None:from data.loaders.registry import build_dataset;dataset=build_dataset(cfg,mode=mode)
		self._dataset=dataset;self.episode_length=int(getattr(self._dataset,'episode_length',env_cfg.episode_limit))
		...
		self._grid_core=grid_core;self.num_available_episodes=self._dataset.num_episodes()
		if self.num_available_episodes<=0:raise ValueError(f"GridEnv requires at least one episode for mode='{self.mode}', but dataset '{type(self._dataset).__name__}' resolved zero episodes with episode_length={self.episode_length}. Expected a current split contract; adjust the date range or regenerate the shared-data package/notebook inputs.")
		self.cur_step=0;self.soc=np.full((self.n,),self.init_soc,dtype=np.float32);(self.signals):dict[str,np.ndarray]={};(self.history_signals):dict[str,np.ndarray]={};(self.bootstrap_signals):dict[str,np.ndarray]={};(self.history_timestamps):list[str]=[];(self.bootstrap_timestamps):list[str]=[];(self.episode_meta):dict[str,Any]={};self._episode_has_next_active=False;self._precomputed_data_dir=None if precomputed_data_dir is None else Path(precomputed_data_dir).resolve();self._precomputed_store=None if self._precomputed_data_dir is None else PrecomputedObservationStore(self._precomputed_data_dir);(self._episode_precomputed):dict[str,np.ndarray]={};(self._local_mpc_solver_cache):dict[tuple[object,...],Any]={};self._train_step_calls=0;self._train_num_envs=max(1,int(getattr(getattr(cfg,'train',None),'num_envs',1)));self._target_global_interaction_steps=max(1,int(getattr(getattr(cfg,'train',None),'resolved_max_train_steps',lambda episode_limit:int(episode_limit))(self.episode_length)));self._apply_episode_storage_config();self.action_space=[spaces.Box(low=-1.,high=1.,shape=(2,),dtype=np.float32)for _ in range(self.n)];self.observation_space=spaces.Dict({key:spaces.Box(low=-np.inf,high=np.inf,shape=shape,dtype=np.float32)for(key,shape)in self.observation_schema.items()})
	...
	def _initial_soc_for_reset(self)->np.ndarray:
		if self.mode=='train':
			if np.isclose(self.train_init_soc_low,self.train_init_soc_high):value=np.full((self.n,),self.train_init_soc_low,dtype=np.float32)
			else:value=np.asarray(self.np_random.uniform(self.train_init_soc_low,self.train_init_soc_high,size=self.n),dtype=np.float32)
			return np.clip(value,self.soc_min,self.soc_max).astype(np.float32)
		return np.full((self.n,),self.init_soc,dtype=np.float32)
	def _load_episode(self,episode_idx:int)->None:
		...
		if 'next_active_idx' not in self.episode_meta:raise ValueError("GridEnv received an episode without meta['next_active_idx']. The new multi-day MADRL training contract requires explicit window continuation metadata. Rebuild the dataset/shared-data through notebooks/forecast/forecast_lstm.ipynb.")
		self._episode_has_next_active=self.episode_meta.get('next_active_idx')is not None;raw_bootstrap_signals=dict(episode_data.get('bootstrap_signals',{}));self.bootstrap_signals={};self.bootstrap_timestamps=[str(value)for value in list(self.episode_meta.get('bootstrap_timestamps')or[])]
		if self._episode_has_next_active:
			if len(self.bootstrap_timestamps)!=1:raise ValueError("GridEnv expected exactly one bootstrap timestamp when meta['next_active_idx'] is present. Rebuild the dataset/shared-data through notebooks/forecast/forecast_lstm.ipynb.")
			for(name,signal)in self.signals.items():
				if name not in raw_bootstrap_signals:raise KeyError(f"GridEnv expected bootstrap_signals['{name}'] for truncated bootstrap. Rebuild the dataset/shared-data through notebooks/forecast/forecast_lstm.ipynb.")
				bootstrap=np.asarray(raw_bootstrap_signals[name],dtype=np.float32)
				if bootstrap.shape!=(1,*signal.shape[1:]):raise ValueError(f"bootstrap signal '{name}' shape should be {(1,*signal.shape[1:])}, got {bootstrap.shape}")
				self.bootstrap_signals[name]=bootstrap
		self.history_timestamps=[str(value)for value in list(episode_data.get('history_timestamps')or[])];history_length=int(episode_data.get('history_length',len(self.history_timestamps)))
	...
	def reset(self,episode_idx:int|None=None,*,seed:int|None=None,options:dict[str,Any]|None=None)->tuple[dict[str,np.ndarray],dict[str,Any]]:
		...
		self._load_episode(int(episode_idx));self._episode_precomputed={}if self._precomputed_store is None else self._precomputed_store.episode(int(episode_idx));self.cur_step=0;self.soc=self._initial_soc_for_reset()
	...
	def _training_progress(self)->float:
		if self.mode!='train':return 1.
		global_interaction_step=min((self._train_step_calls+1)*self._train_num_envs,self._target_global_interaction_steps)
		return float(global_interaction_step/max(float(self._target_global_interaction_steps),1.))
	def _build_truncated_bootstrap_obs(self)->dict[str,np.ndarray]:
		if not self._episode_has_next_active:return self.obs_builder.zeros(self.n)
		if self._episode_precomputed:
			for name,array in self._episode_precomputed.items():
				if int(np.asarray(array).shape[0])<=int(self.episode_length):raise ValueError(f"Old shared-data object '{self._precomputed_data_dir}' does not contain a bootstrap row for precomputed field '{name}'. New contract expects timeline-cache shared-data with next_active_idx support. Re-run notebooks/forecast/forecast_lstm.ipynb.")
		original_signals,original_meta=self.signals,self.episode_meta
		try:
			self.signals={name:np.concatenate([signal,self.bootstrap_signals[name]],axis=0).astype(np.float32,copy=False)for(name,signal)in original_signals.items()};extended_meta=dict(original_meta);extended_meta['timestamps']=[*list(original_meta.get('timestamps')or[]),*self.bootstrap_timestamps];self.episode_meta=extended_meta
			if self.forecaster is not None:self.forecaster.reset();self.forecaster.set_episode({name:np.asarray(signal,dtype=np.float32).copy()if np.asarray(self.history_signals.get(name),dtype=np.float32).size==0 else np.concatenate([np.asarray(self.history_signals.get(name),dtype=np.float32),signal],axis=0).astype(np.float32,copy=False)for(name,signal)in self.signals.items()},self.episode_meta)
			return self.obs_builder.build(self)
		finally:
			self.signals,self.episode_meta=original_signals,original_meta
	def step(self,actions:list[np.ndarray])->tuple[dict[str,np.ndarray],list[float],list[bool],list[bool],dict[str,Any]]:
		t=self.cur_step;battery_action,pv_action=self._split_action_components(actions);validate_executed_actions_numpy(self._current_safety_local(),np.column_stack([battery_action,pv_action]),efficiency=self.eff,dt_hours=self.dt,soc_min=self.soc_min,soc_max=self.soc_max);storage_state=self._apply_storage_dynamics(battery_action);signal_state=self._build_signal_state(t,pv_action);pf_result,pf_error=self._run_power_flow(storage_state,signal_state);p_max=self.agent_p_max.astype(np.float32);e_min=self.agent_e_min.astype(np.float32);e_max=self.agent_e_max.astype(np.float32);battery_capacity_kwh=self.agent_c_bat.astype(np.float32);reward_state={'import_price_t':signal_state['import_price_t'],'storage_price_t':signal_state['import_price_t'],'battery_power_t':storage_state['e_bat'],'actual_grid_power_t':signal_state['net_load'],'dt':self.dt,'v_violation':pf_result.v_violation,'psi_v_raw':float(pf_result.psi_v_raw),'psi_line_raw':float(pf_result.psi_line_raw),'psi_trafo_raw':float(pf_result.psi_trafo_raw),'soc_t':storage_state['soc_t'],'soc_next':storage_state['soc_next'],'soc_min':float(self.soc_min),'soc_max':float(self.soc_max),'battery_capacity_kwh':battery_capacity_kwh,'training_progress':self._training_progress()};reward_per_agent,components=self.reward_fn.compute(reward_state);reward=np.asarray(reward_per_agent,dtype=np.float32);throughput_bonus_weight=float(getattr(self.reward_fn,'_throughput_bonus_weight')(reward_state));throughput_kwh=(np.abs(storage_state['e_bat'])*np.float32(self.dt)).astype(np.float32);self.soc=storage_state['soc_next'];self.cur_step+=1;done=self.cur_step>=self.episode_length;terminated_done=bool(done and not self._episode_has_next_active);truncated_done=bool(done and self._episode_has_next_active);terminated_n=[terminated_done]*self.n;truncated_n=[truncated_done]*self.n;obs=self._build_truncated_bootstrap_obs()if truncated_done else self.obs_builder.zeros(self.n)if done else self.obs_builder.build(self)
		if self.mode=='train':self._train_step_calls+=1
		if self.mode=='train':
			info:dict[str,Any]={'episode_done':bool(done),'madrl_throughput_bonus_weight':float(throughput_bonus_weight),'madrl_throughput_kwh':throughput_kwh,'pf_converged':bool(pf_result.converged),'pf_error':pf_error}
