...
class GridEnv(gym.Env):
    ...
    def _load_episode(self, episode_idx: int) -> None:
        ...
        for name, signal in self.signals.items():
            ...
            if history.ndim != signal.ndim or (signal.ndim > 1 and history.shape[1:] != signal.shape[1:]):
                raise ValueError(f"history signal '{name}' shape should match active signal trailing dimensions, got {history.shape} vs {signal.shape}")
        ...
        if self.get_signal(WHOLESALE_PRICE_SIGNAL).ndim != 1:
            raise ValueError(f"signals['{WHOLESALE_PRICE_SIGNAL}'] must have shape (T,), got {self.get_signal(WHOLESALE_PRICE_SIGNAL).shape}")
        if self.get_signal('load').shape != (self.episode_length, self.n):
            raise ValueError(f"signals['load'] must have shape {(self.episode_length, self.n)}, got {self.get_signal('load').shape}")
        pv_signal = self.signals.get('pv', np.zeros((self.episode_length, self.n), dtype=np.float32))
        if np.asarray(pv_signal, dtype=np.float32).shape != (self.episode_length, self.n):
            raise ValueError(f"signals['pv'] must have shape {(self.episode_length, self.n)}, got {np.asarray(pv_signal, dtype=np.float32).shape}")
        self._apply_episode_storage_config()
    ...
    def reset(self, episode_idx: int | None=None, *, seed: int | None=None, options: dict[str, Any] | None=None) -> tuple[dict[str, np.ndarray], dict[str, Any]]:
        ...
        if self.forecaster is not None:
            ...
            self.forecaster.set_episode({name: np.asarray(signal, dtype=np.float32).copy() if np.asarray(self.history_signals.get(name), dtype=np.float32).size == 0 else np.concatenate([np.asarray(self.history_signals.get(name), dtype=np.float32), signal], axis=0).astype(np.float32, copy=False) for name, signal in self.signals.items()}, self.episode_meta)
        pv_signal = self.signals.get('pv', np.zeros((self.episode_length, self.n), dtype=np.float32))
        self._grid_core.reset(np.asarray(self.get_signal('load')[0], dtype=np.float32), np.asarray(pv_signal[0], dtype=np.float32))
        return (self.obs_builder.build(self), {'episode_idx': int(episode_idx), 'battery_capacity_kwh': self.agent_c_bat.astype(np.float32).copy(), 'p_max': self.agent_p_max.astype(np.float32).copy(), IMPORT_PRICE_MARKUP_KEY: float(self.import_price_markup_eur_per_kwh), 'episode_meta': dict(self.episode_meta)})
    ...
    def _apply_storage_dynamics(self, battery_action: np.ndarray) -> dict[str, np.ndarray]:
        ...
        return {'e_bat_req': e_bat_req.astype(np.float32), 'e_bat': e_bat, 'soc_t': soc_t, 'soc_next': soc_next, 'e_t': e_t.astype(np.float32), 'e_next': e_next}
    def _build_signal_state(self, t: int, pv_action: np.ndarray) -> dict[str, Any]:
        ...
        pv_signal = self.signals.get('pv')
        pv_raw = np.zeros((self.n,), dtype=np.float32) if pv_signal is None else np.asarray(pv_signal[t], dtype=np.float32)
        pv_effective_req = (pv_raw * (0.5 * (np.asarray(pv_action, dtype=np.float32) + 1.0))).astype(np.float32)
        base_net_load_raw = (load_t - pv_raw).astype(np.float32)
        pv_effective = np.clip(pv_effective_req, 0.0, pv_raw).astype(np.float32)
        pv_curtail = (pv_raw - pv_effective).astype(np.float32)
        pv_utilization = np.ones_like(pv_raw, dtype=np.float32)
        valid_mask = pv_raw > 1e-06
        pv_utilization[valid_mask] = (pv_effective[valid_mask] / pv_raw[valid_mask]).astype(np.float32)
        base_net_load_effective = (load_t - pv_effective).astype(np.float32)
        return {'wholesale_price_t': wholesale_price_t, 'import_price_t': import_price_t, 'load_t': load_t, 'pv_raw': pv_raw, 'pv_effective': pv_effective, 'pv_curtail': pv_curtail, 'pv_utilization': pv_utilization.astype(np.float32), 'base_net_load_raw': base_net_load_raw, 'base_net_load_effective': base_net_load_effective}
    ...
    def step(self, actions: list[np.ndarray]) -> tuple[dict[str, np.ndarray], list[float], list[bool], list[bool], dict[str, Any]]:
        ...
        pv_action_exec = (2.0 * signal_state['pv_utilization'] - 1.0).astype(np.float32)
        line_limit = float(self._grid_cfg.line_max_loading_pct)
        info = {
            'episode_done': done,
            't': int(t),
            'wholesale_price': float(signal_state['wholesale_price_t']),
            'import_price': float(signal_state['import_price_t']),
            'load': signal_state['load_t'].astype(np.float32),
            'pv': signal_state['pv_raw'].astype(np.float32),
            'pv_effective': signal_state['pv_effective'].astype(np.float32),
            'pv_curtail': signal_state['pv_curtail'].astype(np.float32),
            'pv_utilization': signal_state['pv_utilization'].astype(np.float32),
            'pv_action_req': pv_action.astype(np.float32),
            'pv_action': pv_action_exec,
            'net_load': signal_state['net_load'].astype(np.float32),
            'grid_import_kw': signal_state['grid_import_kw'].astype(np.float32),
            'grid_export_kw': signal_state['grid_export_kw'].astype(np.float32),
            'e_bat_req': storage_state['e_bat_req'].astype(np.float32),
            'e_bat': storage_state['e_bat'].astype(np.float32),
            'p_max': p_max,
            'e_min': e_min,
            'e_max': e_max,
            ...
            'soc_t': storage_state['soc_t'],
            'soc_next': storage_state['soc_next'],
            **components,
            ...
            'v_violation': pf_result.v_violation,
            'n_v_violations': int(np.sum(pf_result.v_violation > 0.0)),
            'n_line_violations': int(np.any(np.asarray(pf_result.line_loading_pct, dtype=np.float32) > line_limit)),
            'n_trafo_violations': int(np.any(np.asarray(pf_result.trafo_loading_pct, dtype=np.float32) > line_limit)),
            'psi_v_raw': float(pf_result.psi_v_raw),
            'psi_line_raw': float(pf_result.psi_line_raw),
            'psi_trafo_raw': float(pf_result.psi_trafo_raw),
        }
