...
def _resolve_dataset_dir(data_dir: str | Path) -> Path:
    root = Path(data_dir)
    candidates = [root, root / DEFAULT_PROCESSED_SUBDIR, root / 'prosumer']
...
class ProsumerDataset(BaseEpisodeDataset):
    ...
    def _validate_init_args(self) -> None:
        ...
        if self.exclude_start_date is not None and self.exclude_end_date is not None and (self.exclude_start_date > self.exclude_end_date):
            raise ValueError(f'exclude_start_date should be <= exclude_end_date, got {self.exclude_start_date} > {self.exclude_end_date}')
    ...
    def _load_components(self) -> tuple[pd.DataFrame, dict[str, pd.DataFrame], np.ndarray, np.ndarray]:
        ...
        for component in self.load_components:
            ...
            if frame.empty:
                raise ValueError(f'Processed prosumer data does not contain any accessible rows for year={self.year}, exclude_start_date={self.exclude_start_date}, exclude_end_date={self.exclude_end_date}.')
        ...
        return (merged, component_frames, active_mask_on_accessible.astype(bool, copy=False), accessible_source_positions.astype(np.int64, copy=False))
    ...
    def _load(self) -> None:
        ...
        if active_positions.size == 0:
            raise ValueError(f'Processed prosumer data does not contain any active rows for year={self.year}, start_date={self.start_date}, end_date={self.end_date}, exclude_start_date={self.exclude_start_date}, exclude_end_date={self.exclude_end_date}.')
        ...
        if self._num_episodes <= 0:
            history_msg = f'history_warmup_steps={self.history_warmup_steps}, ' if self.history_warmup_steps > 0 else ''
            raise ValueError(f'Processed prosumer data does not contain any usable episodes after split filtering: {history_msg}episode_length={self.episode_length}, total_steps={total_steps}.')
    ...
    def get_episode(self, episode_idx: int) -> dict:
        ...
        episode_signals = {'wholesale_price': self._signals['wholesale_price'][start:end].copy(), 'load': self._signals['load'][start:end, :].copy(), 'pv': self._signals['pv'][start:end, :].copy()}
        history_signals = {'wholesale_price': self._signals['wholesale_price'][history_start:start].copy(), 'load': self._signals['load'][history_start:start, :].copy(), 'pv': self._signals['pv'][history_start:start, :].copy()}
        for comp_name in self.load_components:
